 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/2/5/practice/contest/269656/problem/B
*/
...
void solve() {
    cin >> s >> t;
    string tmp = s + "%" + t;
    auto p = suffix_array_construction(tmp);
    auto lcp = lcp_construction(tmp, p);
    int mx = 0, ll = -1, len;
    for (int i = 0; i < SZ(tmp) - 1; i++) {
        int a = p[i], b = p[i + 1];
        if (a > b) swap(a, b);
        if (a < SZ(s) && b >= SZ(s) + 1) {
            if (maximize(mx, lcp[i])) {
                ll = a; len = lcp[i];
            }
        }
    }
    if (ll != -1) cout << s.substr(ll, len) << nline;
} 
