/*
    Solution for: https://atcoder.jp/contests/abc236/tasks/abc236_e
*/
...
// state = 0: choose or skip
// state = 1: must choose
double maxSum(int i, int state, vector<double>& c) {
    if (i >= n) return 0;
    if (dp[i][state] != -1.0) return dp[i][state];

    double res;
    if (state == 0) {
        res = max(c[i] + maxSum(i + 1, 0, c), maxSum(i + 1, 1, c));
    } else {
        res = c[i] + maxSum(i + 1, 0, c);
    }
    return dp[i][state] = res;
}
bool check(double x) {
    vector<double> c(n, 0);
    dp = vector<vector<double>>(n, vector<double>(2, -1.0));
    for (int i = 0; i < n; i++) {
        c[i] = a[i] - x;
    }
    return  maxSum(0, 0, c) >= 0;
}
bool check2(int x) {
    vector<double> c(n, 0);
    dp = vector<vector<double>>(n, vector<double>(2, -1.0));
