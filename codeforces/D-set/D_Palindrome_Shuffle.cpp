 ...
 * Solution for: https://codeforces.com/contest/2069/problem/D
*/
...
bool smaller(int l1, int r1, int l2, int r2) {
    for (int c = 0; c < 26; c++) {
        int cnt1 = freq[r1 + 1][c] - freq[l1][c];
        int cnt2 = freq[r2 + 1][c] - freq[l2][c];
        if (!(cnt1 <= cnt2)) return false;
    }
    return true;
}
bool can(int l, int r) {
    int take = min(l, n - (r + 1));
    if (!(border >= take)) return false;

    if (r <= n/2 - 1 || l >= n/2) {
        int d_len = max(n/2 - r - 1, l - n/2);
        if (!(d_len <= d)) {
            return false;
        }
    }
    
    int len = min(r - l + 1, max(l - take, n - (r + 1) - take));
    if (len == 0) return true;
    int ll = take, rr = ll + len - 1;
    if (l <= ll && rr <= r) {
        ll = n - 1 - ll;
        rr = n - 1 - rr;
        swap(ll, rr);
