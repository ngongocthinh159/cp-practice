 ...
 * Solution for: https://codeforces.com/problemset/problem/580/D
*/
...
void solve() {
    ...
    vector<vector<int>> plus(n, vector<int>(n, 0));
    ll x, y, c;
    for (int i = 0; i < k; i++) {
        cin >> x >> y >> c;
        x--; y--;
    ...
    for (int mask = maxMask - 1; mask >= 1; mask--) {
        for (int prev = 0; prev < n; prev++) {
            ...
            for (int j = 0; j < n; j++) {
                if (!((mask >> j) & 1)) {
                    dp[mask][prev] = max(dp[mask][prev], dp[mask | (1 << j)][j] + a[j] + plus[prev][j]);
                }
            }
        }
    }
    ...
    for (int mask = 0; mask <= maxMask; mask++) {
        if (n - __builtin_popcount(mask) != m - 1) continue;

        for (int prev = 0; prev < n; prev++) {
            if (!((mask >> prev) & 1)) continue;

            ans = max(ans, dp[mask][prev] + a[prev]);
        }
    }
