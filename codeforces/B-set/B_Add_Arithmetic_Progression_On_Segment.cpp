 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/5/4/practice/contest/280801/problem/B
*/
...
void apply(int idx, int s, int e, const Lazy &operation) {
    if (operation.type == ADD) {
        segtree[idx].sum += 1LL*(e - s + 1)*(operation.A + operation.C) + 1LL*(e - s + 1)*(e + s)/2*operation.B;
        lazy[idx].A += operation.A;
        lazy[idx].B += operation.B;
        lazy[idx].C += operation.C;
        lazy[idx].type = ADD;
    }
};
...
Data query(int idx, int s, int e, int pos) { // range sum query
...
void solve() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        if (t == 1) {
            cin >> l >> r >> a >> d;
            l--, r--;
            // update(1, 0, n - 1, l, r, Lazy({ADD, 1LL*(r + l + 2)*(r - l + 1)/2*d}));
            // update(1, 0, n - 1, l, r, Lazy({ADD, 1LL*(r - l + 1)*(a - d*(l + 1))}));
            update(1, 0, n - 1, l, r, Lazy({ADD, a, d, -1LL*d*l}));
        } else {
            cin >> pos;
            pos--;
            cout << query(1, 0, n - 1, pos).sum << nline;
        }
    }
}
