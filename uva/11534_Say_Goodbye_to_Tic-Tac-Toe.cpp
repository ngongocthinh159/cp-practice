 ...
 * Solution for: https://onlinejudge.org/index.php?option=com_onlinejudge&Itemid=8&page=show_problem&problem=2529
*/
            ...
            for (int r = 0; r < 3; r++) {
                ...
                for (int j = 1; j <= i; j++)
                    for (int k = 1; k <= 2; k++) {
                        if (j == 1 && k == l) continue;
                        if (j == i && k == r) continue;
                        last[dp[j - 1][l][k]^dp[i - j][k][r]] = id;
                    }
                while (last[dp[i][l][r]] == id) dp[i][l][r]++;
            }
    int tot = 0;
    {
        int i = 0;
        while (i < n) {
            ...
            if (end + 1 < n) 
                ...
                r = 0;
            tot ^= dp[end - start + 1][l][r];
        }
    }
    int player = 0;
    for (auto x : s) if (x != '.') player ^= 1;
    if ((tot && player == 0) || (!tot && player == 1))
        cout << "Possible." << nline;
    else
