 ...
 * Solution for: https://codeforces.com/contest/1702/problem/E
*/
...
int n;
pair<int,int> d[MAX];
void dfs(int i, int& cnt, vector<bool> &visited, vector<vector<int>> &g) {
    visited[i] = true;
    cnt++;
    for (auto v : g[i]) if (!visited[v]) dfs(v, cnt, visited, g);
}
void solve() {
    cin >> n;
    vector<int> freq(n + 1);
    for (int i = 0; i < n; i++) cin >> d[i].first >> d[i].second, freq[d[i].first]++, freq[d[i].second]++;
    for (int i = 0; i < n; i++) if (d[i].first == d[i].second || freq[d[i].first] != 2 || freq[d[i].second] != 2) {
        cout << "NO" << nline;
        return;
    }
    vector<vector<int>> g(n + 1);
    for (int i = 0; i < n; i++) g[d[i].first].push_back(d[i].second),
        g[d[i].second].push_back(d[i].first);

    vector<bool> visited(n + 1);
    for (int i = 1; i <= n; i++) if (!visited[i]) {
        int ver_cnt = 0;
        dfs(i, ver_cnt, visited, g);
        // cout << i << " " << ver_cnt << nline;
        if (ver_cnt&1) {
            cout << "NO" << nline;
            return;
        }
