 ...
 * Solution for: https://vjudge.net/problem/UVALive-6439
*/
...
string s;
int n;
ll bp[MAX], ibp[MAX], pref[MAX], base = 33, cases = 0;
void init_hash() {
    for (int i = 0; i < n; i++) {
        pref[i + 1] = (pref[i] + (s[i] - 'a' + 1)*bp[i]%MOD) % MOD;
    }
}
ll query(int l, int r) {
    ll res = (pref[r + 1] - pref[l]) % MOD;
    if (res < 0) res += MOD;
    res = (res * ibp[l]) % MOD;
    return res;
}
void pre_compute() {
    bp[0] = 1;
    for (int i = 1; i < MAX; i++) bp[i] = bp[i - 1]*base % MOD;
    ibp[MAX - 1] = expo(bp[MAX - 1], MOD - 2, MOD);
    for (int i = MAX - 2; i >= 0; i--) ibp[i] = ibp[i + 1]*base %MOD;
}
void solve() {
    cin >> s;
    n = SZ(s);
    init_hash();
    int start = 0, ans = 0;
    bool extra_last = true;
    for (int i = 0; i <= (n + 1)/2 - 1; i++) {
        ll h1 = query(start, i);
        ll h2 = query(n - i - 1, n - start - 1);
        if (h1 == h2) {
            ans++;
            start = i + 1;
            if (i == (n + 1)/2 - 1) extra_last = false;
        }
    }
    ans = 2*ans + (extra_last);
    cout << "Case #" << (++cases) << ": " << ans << nline;
}
