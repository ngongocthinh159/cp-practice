...
void solve() {
    ...
    f(i,0,n) {
        ...
        if (action.compare("posted") == 0 || action.compare("commented") == 0) {cin >> saf;}

        int point;
        string secondName;
        if (words[1].compare("posted") == 0) {point = 15; secondName = words[3].substr(0,words[3].size() - 2);}
        if (words[1].compare("commented") == 0) {point = 10; secondName = words[3].substr(0,words[3].size() - 2);}
        if (words[1].compare("likes") == 0) {point = 5; secondName = words[2].substr(0,words[2].size() - 2);}
        string firstName = words[0];
        if (secondName.compare(name) == 0) {
            string tmp2 = firstName;
            firstName = secondName;
            secondName = tmp2;
        }

        set1.insert(firstName); set1.insert(secondName);

        if (firstName.compare(name) == 0) {
            if (map1.find(firstName+","+secondName) == map1.end()) map1[firstName+","+secondName] = point;
            else map1[firstName+","+secondName] += point;
            set2.insert(secondName);
        }
    }
    for (auto [name12,point] : map1) {
        string name2 = name12.substr(name.size()+1);
        vector<string>& list = map2[point];
        list.pb(name2);
    }
    vector<string> listTmp;
    for (auto [point, list] : map2) {
        sort(list.begin(), list.end(), greater<string>());
        fe(val,list)
            listTmp.pb(val);
    }
    rf(i,listTmp.size()-1,0)
        cout << listTmp[i] << "\n";
    set2.insert(name);
    vector<string> list;
    for (auto n1 : set1) {
