 ...
 * Solution for: https://codeforces.com/gym/102644/problem/F
*/
...
#define MAXN 105
ll dist[MAXN][MAXN], res[MAXN][MAXN], buf[MAXN][MAXN], n, m, k;
...
void mul(ll a[MAXN][MAXN], ll b[MAXN][MAXN]) {
    ...
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++)
            for (int k = 0; k < n; k++) if (a[i][k] != LINF && b[k][j] != LINF)
                minimize(buf[i][j], a[i][k] + b[k][j]);
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++)
            a[i][j] = buf[i][j];
}
