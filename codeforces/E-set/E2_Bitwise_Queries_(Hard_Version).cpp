 ...
 * Solution for: https://codeforces.com/contest/1451/problem/E2
*/
...
int query(char c, int i, int j) {
    i++; j++;
    string oper;
    if (c == '^') oper = "XOR";
    else if (c == '|') oper = "OR";
    else oper = "AND";
    string query = oper + " " + to_string(i) + " " + to_string(j);
    cout << query << nline << flush;
    int res; cin >> res;
    return res;

    // if (c == '^') return arr[i]^arr[j];
    // else if (c == '&') return arr[i]&arr[j];
