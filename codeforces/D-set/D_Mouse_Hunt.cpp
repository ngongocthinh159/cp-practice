 ...
 * Solution for: https://codeforces.com/contest/1027/problem/D
*/
...
void dfs(int u, vector<bool> &vis, unordered_set<int> &instack, int &vertice) {
    vis[u] = true;
    instack.insert(u);
    for (auto v : g[u]) {
        if (!vis[v]) {
            dfs(v, vis, instack, vertice);
...
int dfs2(int u, int root) {
    int res = cost[u];
    for (auto v : g[u]) {
        if (v != root) {
            res = min(res, dfs2(v, root));
        }
    }
    return res;
}
void solve() {
    ...
    for (int i = 1; i<=n ;i++) {
        if (!vis[i]) {
            int vertice = -1;
            dfs(i, vis, instack, vertice);
            // cout << i << " " << vertice << nline;
            if (vertice != -1) {
                int mn = dfs2(vertice, vertice);
                ans += mn;
            }
        }
    }
