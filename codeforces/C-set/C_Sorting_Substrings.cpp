 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/2/5/practice/contest/269656/problem/C
*/
...
int compare_substr(pair<int,int> &p1, pair<int,int> &p2, vector<int> &rank) {
    int l = rank[p1.fi], r = rank[p2.fi];
    if (l > r) swap(l, r);
    int len;
    if (l == r) {
        len = min(p1.se - p1.fi + 1, p2.se - p2.fi + 1);
    } else {
        r--;
        len = query_rmq(l, r);
        len = min(len, p1.se - p1.fi + 1);
        len = min(len, p2.se - p2.fi + 1);
    }

    int idx1 = p1.fi + len;
    int idx2 = p2.fi + len;
    if (idx1 <= p1.se && idx2 <= p2.se) {
        if (s[idx1] < s[idx2]) return -1;
        return 1;
    } else if (idx1 <= p1.se) return 1;
    else if (idx2 <= p2.se) return -1;
    return 0;
}
void solve() {
    cin >> s >> m;
    vector<pair<int,int>> sub(m);
    for (int i = 0; i < m; i++) cin >> sub[i].first >> sub[i].second, sub[i].first--, sub[i].second--;
    auto p = suffix_array_construction(s);
    auto lcp = lcp_construction(s, p);
    for (int i = 0; i < SZ(lcp); i++) rmq[0][i] = lcp[i];
    for (int j = 1; j <= LOG; j++)
        for (int i = 0; i + MASK(j) - 1 < SZ(lcp); i++)
            rmq[j][i] = RELAX(rmq[j - 1][i], rmq[j - 1][i + MASK(j - 1)]);
    vector<int> rank(s.size());
    for (int i = 0; i < SZ(s); i++)
        rank[p[i]] = i;
    sort(ALL(sub), [&](pair<int,int> &p1, pair<int,int> &p2) {
        int res = compare_substr(p1, p2, rank);
        if (res == 0) return p1 < p2;
        return res == -1;
    });
    for (int i = 0; i < m; i++) cout << sub[i].first + 1 << " " << sub[i].second + 1 << nline;
}
