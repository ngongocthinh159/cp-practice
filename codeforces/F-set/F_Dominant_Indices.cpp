 ...
 * Solution for: https://codeforces.com/contest/1009/problem/F
*/
...
#define MAXN 1000005
int n, sz[MAXN], big[MAXN], tin[MAXN], tout[MAXN], ver[MAXN], ans[MAXN], height[MAXN], h[MAXN], T = -1;
vector<int> g[MAXN];
int cur_j, cur_freq;
void pre_compute() {
...
void add(int v, int u) {
    if (++h[height[v]] == cur_freq) {
        minimize(cur_j, height[v]);
    } else if (h[height[v]] > cur_freq) {
        cur_freq = h[height[v]];
        cur_j = height[v];
    }
}
