 ...
 * Solution for: https://codeforces.com/problemset/problem/1163/C2
*/
...
int main() {
    ...
    for (int i = 0; i < n; i++)
        ...
        ll A = b;
        ll B = -a;
        ll C = -(A * p[0][i] + B * p[1][i]);
        if (A < 0 || (A == 0 && B < 0)) A = -A, B = -B, C = -C;
        ll gab = gcd(abs(A), abs(B));
        ll gabc = gcd(gab, abs(C));
        ll _A = A / gab;
        ll _B = B / gab;
        A /= gabc;
        B /= gabc;
    ...
    ll dif = S.size();
        ll ans = 1LL * dif * (dif - 1) / 2;
    for (auto &[_, cnt] : freq) if (cnt >= 2) {
        ans -= 1LL * cnt * (cnt - 1) / 2;
    }
    cout << ans << "\n";
    return 0;
