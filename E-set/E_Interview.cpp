...
struct Interactor {
    vector<int> piles;
    int specialPilePos = 2;
    Interactor(vector<int> piles, int specialPilePos) {
        this->piles = piles;
        this->specialPilePos = specialPilePos;
    }
    int query(string s) {
        stringstream ss(s);
        char c; ss >> c;
    ...
    int answer() {
        return specialPilePos;
    }
};
int askSumLToPos(int l, int pos) {
    string query = "? " + to_string(pos - l + 1);
    for (int i = l; i <= pos; i++) {
        query += " " + to_string(i);
    }
    cout << query << endl;
...
int askSum0ToPos_inter(int l, int pos, Interactor &inter) {
    string query = "? " + to_string(pos - l + 1);
    for (int i = l; i <= pos; i++) {
        query += " " + to_string(i);
    }
    return inter.query(query);
}
void solve() {
    ...
    while (l <= r) {
        int m = l + (r - l)/2;
        int sum = askSumLToPos(l + 1, m + 1);
        // int sum = askSum0ToPos_inter(l + 1, m + 1, inter);
        if (sum > pre[m] - pre[l] + a[l]) {
            ans = m + 1;
            r = m - 1;
        } else {
            l = m + 1;
    ...
    // cout << (ans == inter.answer() ? "passed" : "not passed") << "\n";
    cout << "! " << ans << endl;
}
