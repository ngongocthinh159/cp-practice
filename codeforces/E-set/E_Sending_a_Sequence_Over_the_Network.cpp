 ...
 * Solution for: https://codeforces.com/contest/1741/problem/E
*/
...
void solve() {
    ...
    dp[0] = true;
    for (int i = 1; i <= n; i++) {
        if (!dp[i]) { // if from [0,i] we can not form into segments => Try backward DP
            if (i - b[i] - 1 >= 0 && dp[i - b[i] - 1]) dp[i] = true;
        }

        if (dp[i - 1]) { // if [0,i-1] can form valid segments => Use forward DP for future state
            if (i + b[i] <= n) dp[i + b[i]] = true;
        }
    }
    cout << (dp[n] ? "YES" : "NO") << nline;
}
