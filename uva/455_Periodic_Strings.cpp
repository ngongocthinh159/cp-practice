 ...
 * Solution for: https://vjudge.net/problem/UVA-455/origin
*/
...
string s;
bool check(int len) {
    auto z = z_function(s.substr(0, len) + "#" + s);
    for (int i = len + 1; i < len + 1 + sz(s); i += len) {
        if (!(z[i] >= len)) return false;
    }
    return true;
}
void solve() {
    ...
    for (int i = 1; i <= sqrt(n); i++) {
        ...
        if (n%len1 == 0) {
            ...
            int len2 = (n/len1);
            if (len2 != len1) {
                if (check(len2)) ans = min(ans, len2);
            }
        }
    }
    if (ans == INT_MAX) cout << n << nline;
    else cout << ans << nline;
}
