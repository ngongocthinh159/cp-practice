...
ll dfs(int i, int j, int k, int ok, vector<vector<vector<vector<int>>>> &dp) {
    if (i == n) return ok;
    ...
    ll res = 0;
    if (j == 2 || k == 2) {
        res = (res + dfs(i + 1, k, x, 0, dp)) % MOD1;
        res = (res + dfs(i + 1, j, k, 0, dp)) % MOD1;
    } else {
        bool canChoose = false;
        if ((j + k + x) % 2 == 0) canChoose = true;
        if (canChoose) res = (res + dfs(i + 1, k, x, 1, dp)) % MOD1;
        res = (res + dfs(i + 1, j, k, ok, dp)) % MOD1;
    }
...
void solve() {
    ...
    vector<vector<vector<vector<int>>>> dp(n, vector<vector<vector<int>>>(3,
                                                vector<vector<int>>(3,
                                                vector<int>(2, -1))));
    cout << dfs(0, 2, 2, 0, dp) << nline;
}
