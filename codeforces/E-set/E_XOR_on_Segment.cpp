 ...
 * Solution for: https://codeforces.com/problemset/problem/242/E
*/
...
ll lazy[4*MAX];
ll a[MAX];
Data make_data(ll val, ll sum) {
    Data res;
    for (int bit = 0; bit <= MXBIT; bit++) {
        if ((val >> bit) & 1) res.freq[bit] = 1;
        else res.freq[bit] = 0;
    }
    res.sum = sum;
    return res;
...
Data merge(Data &l, Data &r) {
    Data res;
    for (int bit = 0; bit <= MXBIT; bit++) res.freq[bit] = l.freq[bit] + r.freq[bit];
    res.sum = l.sum + r.sum;
    return res;
}
...
void apply(int idx, int s, int e, ll val) {
    ...
    for (int bit = 0; bit <= MXBIT; bit++) {
        if ((val >> bit) & 1) {
            segtree[idx].sum -= 1LL*segtree[idx].freq[bit]*(1 << bit);
            int remain = (e - s + 1) - segtree[idx].freq[bit];
            segtree[idx].sum += 1LL*remain*(1 << bit);
            segtree[idx].freq[bit] = remain;
        }
    }
