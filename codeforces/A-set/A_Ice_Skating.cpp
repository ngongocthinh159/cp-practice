 ...
 * Solution for: https://codeforces.com/problemset/problem/217/A
*/
...
void solve() {
    ...
    for (int i = 0; i < n; i++) 
        for (int j = i + 1; j < n; j++) {
            if (p[i].first == p[j].first || p[i].second == p[j].second) dsu.unite(i, j);
        }
    int cnt = 0;
    for (int i = 0; i < n; i++) if (i == dsu.find(i)) cnt++;
    cout << cnt - 1 << nline;
}
