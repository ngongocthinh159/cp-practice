 ...
 * Solution for: https://www.hackerearth.com/practice/algorithms/graphs/strongly-connected-components/practice-problems/algorithm/gcd-on-directed-graph-1122228a/
*/
...
void dfs2(int u, vector<int> &compo, int id) {
	visited[u] = true;
	compo.push_back(u);
	for (auto v : tg[u]) {
		if (!visited[v]) {
...
void dfs3(int u, vector<int> &gcd_compo) {
    if (dp[u].size()) return;

    dp[u].insert(gcd_compo[u]);
	for (auto v : cong[u]) {
        dfs3(v, gcd_compo);
        for (auto x : dp[v]) dp[u].insert(gcd(x, gcd_compo[u]));
	}
}
void pre_compute() {
...
void solve() {
    ...
    // for (auto x : topo) cout << x << nline;
	vector<vector<int>> compos;
	vector<int> gcd_compo;
	gcd_compo.push_back(0);
	compos.push_back({});
	int id = 1;
    // int tmp = INT_MAX;
	for (auto u : topo) {
		if (!visited[u]) {
			vector<int> compo;
			dfs2(u, compo, id);
			int cur_gcd = cost[compo[0]];
			for (auto x : compo) compo_id[x] = id, cur_gcd = gcd(cur_gcd, cost[x]);
			gcd_compo.push_back(cur_gcd);
    ...
    for (int u = 1; u <= n; u++) {
        for (auto v : g[u]) {
            if (compo_id[v] != compo_id[u]) {
                cong[compo_id[u]].push_back(compo_id[v]);
            }
        }
    }
    dp.resize(id);
	int ans = INT_MAX;
	for (int i = 1; i < sz(cong); i++) {
        dfs3(i, gcd_compo);
		if (dp[i].size()) ans = min(ans, *dp[i].begin());
	}
	cout << ans << "\n";
}
