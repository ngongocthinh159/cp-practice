 ...
 * Solution for: https://codeforces.com/gym/102644/problem/I
*/
...
void solve() {
    ...
    int msb = 32 - __builtin_clz((int)1e9) - 1;
    for (int i = 1; i <= msb; i++) {
        base_pow.push_back(base_pow[i - 1] * base_pow[i - 1]);
    }
    for (int i = 0; i < q; i++) {        
        int u, v, k;
        cin >> u >> v >> k;
