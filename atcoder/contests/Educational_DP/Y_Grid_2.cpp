 ...
 * Solution for: https://atcoder.jp/contests/dp/tasks/dp_y
*/
...
#define MAXK 3005
int n, m, k;
ll fact[MAXN], inv[MAXN], f[MAXK];

void pre_compute() {
    fact[0] = 1;
    for (int i = 1; i < MAXN; i++) fact[i] = fact[i - 1] * i % MOD;
    inv[MAXN - 1] = expo(fact[MAXN - 1], MOD - 2, MOD);
...
void solve() {
    ...
    sort(ALL(p));
    for (int i = 0; i <= k; i++) {
        f[i] = nCk(p[i].fi + p[i].se - 2, p[i].fi - 1);
        for (int j = 0; j < i; j++) if (p[j].fi <= p[i].fi && p[j].se <= p[i].se) {
            f[i] = ((f[i] - f[j] * nCk(p[i].fi + p[i].se - p[j].fi - p[j].se, p[i].fi - p[j].fi)) % MOD + MOD) % MOD;
        }
    }
    cout << f[k] << nline;
}
