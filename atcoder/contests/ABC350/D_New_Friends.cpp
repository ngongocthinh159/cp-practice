 ...
 * Solution for: https://atcoder.jp/contests/abc350/tasks/abc350_d
*/
...
vector<vector<int>> g;  
vector<bool> visited;
void dfs(int u, vector<vector<int>> &g, set<pair<int,int>> &S, ll &vCnt) {
    vCnt++;
    for (auto &v : g[u]) {
...
void solve() {
    ...
    for (int i = 1; i <= n; i++) {
        if (!visited[i]) {
            ...
            dfs(i, g, S, verCnt);
            edgeCnt = S.size();
            ans += 1LL*verCnt*(verCnt-1)/2 - edgeCnt;
        }
    }
    cout << ans << nline;
}
