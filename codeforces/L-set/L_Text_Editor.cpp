 ...
 * Solution for: https://codeforces.com/gym/508693/problem/L
*/
...
struct RH {
    ...
    void pre_compute() {
        ...
        i_base[0] = 1;
        if (n >= 2) {
            i_base[1] = expo(b, m - 2, m);
            for (int i = 2; i <= n; i++) {
                i_base[i] = (i_base[i - 1]*i_base[1]) % m;
            }
        }
        for (int i = 1; i <= n; i++) {
            prf[i] = (prf[i - 1] + (((int) s[i - 1])*base[i - 1])%m) % m;
        }
    }

    ll get_hash(int l, int r) {
        return ((((prf[r + 1] - prf[l])%m + m) % m) * i_base[l]) % m;
    }
};
...
string p;
int x;
bool check(RH &hs, RH &hp, int len, int s_len, int p_len) {
    int cnt = 0;
    for (int r = len - 1; r < s_len; r++) {
        int l = r - len + 1;
        if (hs.get_hash(l, r) == hp.get_hash(0, len - 1)) cnt++;
    }
    return cnt >= x;
}
void solve() {
    ...
    while (r - l > 1) {
        int m = l + (r - l)/2;
        if (check(hs, hp, m, s.size(), p.size())) {
            l = m;
        } else
            r = m;
    }
    cout << (l == 0 ? "IMPOSSIBLE" : p.substr(0, l)) << nline;
}
