...
int n, m;
char mat[MAXN][MAXN];
ll pref[MAXN][MAXN], basepow[MAXN], v[2*MAXN], base = 299;
int mask[MAXN][MAXN], d[2*MAXN], freq[26];
void pre_compute() {
    basepow[0] = 1;
    for (int i = 1; i < 26; i++) 
        basepow[i] = basepow[i - 1] * base % MOD;
}
ll _hash(int i, int l, int r) {
    ll res = (pref[i][r + 1] - pref[i][l]) % MOD;
    if (res < 0) res += MOD;
    return res;
}
bool can_order(int i, int l, int r) {
    return __builtin_popcount(mask[i][r + 1] ^ mask[i][l]) <= 1;
}
void solve() {
    cin >> n >> m;
    for (int i = 0; i < n; i++)
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = 0; j < m; j++) {
            freq[mat[i][j] - 'a']++;
            for (int c = 0; c < 26; c++) {
                pref[i][j + 1] = (pref[i][j + 1] + (freq[c] + 1)*basepow[c]%MOD) % MOD;
            }

            mask[i][j + 1] = mask[i][j] ^ (1 << (mat[i][j] - 'a'));
        }
    }
    ll ans = 0;
    for (int l = 0; l < m; l++) {
        for (int r = l; r < m; r++) {
            int i = 0;
            while (i < n) {
                while (i < n && !can_order(i, l, r)) i++;
                if (i < n) {
                    int v_sz = 0;
                    while (i < n && can_order(i, l, r)) {
                        v[v_sz++] = -1;
                        v[v_sz++] = _hash(i, l, r);
                        i++;
                    }
                    ...
                    // Manacher
                    for (int j = 0, ll = -1, rr = -1; j < v_sz; j++) {
                        d[j] = 0;
                        if (j <= rr) d[j] = min(d[ll + rr - j], rr - j + 1);
                        while (0 <= j - d[j] && j + d[j] < v_sz && v[j - d[j]] == v[j + d[j]]) ++d[j];
                        if (j + d[j] - 1 > rr) ll = j - d[j] + 1, rr = j + d[j] - 1;
                    }
