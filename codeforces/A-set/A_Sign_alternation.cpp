 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/4/4/practice/contest/274684/problem/A
*/
...
struct Updater {
    ll value_to_update; // might change  (info required to update segtree node)
    Updater(ll _value_to_update) { // pass in information require to update segtree node
        value_to_update = _value_to_update; // might change  (save info)
    }
    void apply(Node &node) { // update logic
        node.val = value_to_update;
    }
};
...
void solve() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        if (t == 0) {
            ...
            pos--;
            if (pos&1) tree.update(pos, -val);
            else tree.update(pos, val);
        } else {
            int l, r; cin >> l >> r;
            l--, r--;
            if (l&1) cout << (-1LL*tree.query(l, r).val) << nline;
            else cout << tree.query(l, r).val << nline;
        }
    }
}
