 ...
 * Solution for: https://codeforces.com/contest/375/problem/D
*/
...
int n, m, ans[MAXN], c[MAXN], cf[MAXN], sz[MAXN], big[MAXN], tin[MAXN], tout[MAXN], ver[MAXN], T = -1;
...
void dfs2(int u, int par, bool keep) {
    ...
    for (auto &q : queries[u]) {
        auto it = S.lower_bound({q.k, -1});
        int idx = S.size();
        if (it != S.end()) idx = S.order_of_key(*it);
        ans[q.idx] = S.size() - idx;
    }
    if (!keep) {
