 ...
 * Solution for: https://atcoder.jp/contests/dp/tasks/dp_x
*/
...
#define MAXN 1005
#define MAXW 10005
int n, w[MAXN], s[MAXN], v[MAXN], order[MAXN];
ll dp[MAXN][2*MAXW];
void pre_compute() {
...
void solve() {
    ...
    for (int i = 0; i <= n; i++)
        for (int j = 0; j < 2*MAXW; j++) dp[i][j] = -LINF;
    ...
    for (int i = 1; i <= n; i++) {
        ...
        for (int j = 0; j < 2*MAXW; j++) {
            if (j <= s[idx] + w[idx] && j >= w[idx] && dp[i - 1][j - w[idx]] != -LINF) 
                dp[i][j] = max(dp[i][j], dp[i - 1][j - w[idx]] + v[idx]);
            dp[i][j] = max(dp[i][j], dp[i - 1][j]);
        }
    }
    ll ans = -LINF;
    for (int j = 0; j < 2*MAXW; j++) ans = max(ans, dp[n][j]);
    cout << ans << nline;
}
