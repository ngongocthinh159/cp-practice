...
void solve() {
    ll n, count = 0;
    cin >> n;
    vector<pair<int, int>> dp(1e5 + 1, mp(-1, -1));
    f(i,0,n) {
        ll tmp;
        cin >> tmp;
        if (dp[tmp].fi == -1) dp[tmp].fi = i; // fisrt time see => dp[tmp].fi >= 0, dp[tmp].se = -1 
        else if (dp[tmp].fi >= 0 ) { // >= Second time see
            if (dp[tmp].se == -1) { // Second time see
                dp[tmp].se = i - dp[tmp].fi; // Update d for second time
                dp[tmp].fi = i;
            } else {
                if (dp[tmp].se != i - dp[tmp].fi) { // >= Third time see
                    dp[tmp].fi = -2;
                } else dp[tmp].fi = i;
            }
        }
