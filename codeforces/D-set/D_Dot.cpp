 ...
 * Solution for: https://codeforces.com/problemset/problem/69/D
*/
...
int dfs(int x, int y) {
    ...
    for (int i = 0; i < n; i++) {
        int X = x + p[i][0];
        int Y = y + p[i][1];
        if (dist_ok(X, Y) && !dfs(X, Y)) {
            res = 1;
            break;
        }
    }
    return dp[x + OFFSET][y + OFFSET] = res;
}
void solve() {
    cin >> x_0 >> y_0 >> n >> d;
    for (int i = 0; i < n; i++) {
