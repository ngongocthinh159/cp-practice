 ...
 * Solution for: https://codeforces.com/contest/1937/problem/C
*/
...
struct Interactor {
    ...
    string ask(int a, int b, int c, int d) {
        int x = (v[a] | v[b]);
        int y = (v[c] | v[d]);
        // if (a==1 && b==0 && c==1 && d==3) cout << x << " " << y << nline;
        string res;
        if (x > y) res = ">";
        else if (x < y) res = "<";
        else res = "=";
        // cout << "(" << a << "," << b << "," << c << "," << d << ") = " << res << nline; 
        return res;
    }
};
string ask(int a, int b, int c, int d) {
...
void solve() {
    ...
    int mx_idx = 0;
    for (int i = 1; i < n; i++) {
        string res = ask(mx_idx,mx_idx,i,i);
        // string res = it.ask(mx_idx,mx_idx,i,i); 
        if (res == "<") {
            mx_idx = i;
        }
    }
    int second_idx;
    if (mx_idx==0) second_idx = 1;
    else second_idx = 0;
    vector<int> v;
    v.push_back(second_idx);
    for (int i = 0; i < n; i++) {
        if (i == mx_idx || i == second_idx) continue;
        string res = ask(mx_idx,second_idx,mx_idx,i);
        // string res = it.ask(mx_idx,second_idx,mx_idx,i);
        if (res == "<") {
            second_idx = i;
    ...
    for (int i = 1; i < v.size(); i++) {
        if (v[i] == mx_idx || v[i] == mn_idx) continue;
        string res = ask(mn_idx, mn_idx, v[i], v[i]);
        // string res = it.ask(mn_idx, mn_idx, v[i], v[i]);
        if (res == ">") {
            mn_idx = v[i];
        }
    }
