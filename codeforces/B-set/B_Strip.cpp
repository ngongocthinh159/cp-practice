 ...
 * Solution for: https://codeforces.com/contest/487/problem/B
*/
...
int query(int idx, int s, int e, int l, int r) {
    ...
    if (l <= s && e <= r) return segtree[idx].mn;
    int m = (s + e)/2;
    int la = query(2*idx, s, m, l, r);
    int ra = query(2*idx + 1, m + 1, e, l, r);
    return min(la, ra);
}
...
void solve() {
    ...
    for (int r = 1, l = 1; r <= n; r++) {
        while (mnq.size() && a[mnq.back()] >= a[r]) mnq.pop_back();
        mnq.push_back(r);
        while (mxq.size() && a[mxq.front()] <= a[r]) mxq.pop_front();
        mxq.push_front(r);
        while (mnq.size() && !(l <= mnq.front() && mnq.front() <= r)) mnq.pop_front();
        while (mxq.size() && !(l <= mxq.back() && mxq.back() <= r)) mxq.pop_back();
        int dif = a[mxq.back()] - a[mnq.front()];
        while (l < r && dif > S) {
            l++;
            while (mnq.size() && !(l <= mnq.front() && mnq.front() <= r)) mnq.pop_front();
            while (mxq.size() && !(l <= mxq.back() && mxq.back() <= r)) mxq.pop_back();
            dif = a[mxq.back()] - a[mnq.front()];
        }

        int len = r - l + 1;
        if (len >= L) {
            int _l = r - len + 1, _r = r - L + 1;
            int res = query(1, 0, n, _l - 1, _r - 1);
            if (res != IINF)
                dp[r] = res + 1;
            else
                dp[r] = IINF;
