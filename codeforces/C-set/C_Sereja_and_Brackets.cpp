 ...
 * Solution for: https://codeforces.com/contest/380/problem/C
*/
...
struct SegmentTree {
    struct Data {
        int open, close, valid_pairs;
    };
    vector<Data> segtree;
    int n;
    SegmentTree(int n, string &t) {
        this->n = n;
        segtree.resize(4*n);
        build(1, 0, n - 1, t);
    }
    void build(int idx, int s, int e, string &t) {
        if (s == e) {
            segtree[idx] = make_data(t[s] == '(', t[s] == ')', 0);
            return;
        }
        int m = (s + e)/2;
        build(2*idx, s, m, t);
        build(2*idx + 1, m + 1, e, t);
        segtree[idx] = merge(segtree[2*idx], segtree[2*idx + 1]);
    }
    Data make_data(int open, int close, int valid_pairs) {
        Data res;
        res.open = open;
        res.close = close;
    ...
    Data merge(Data l, Data r) {
        ...
        res.valid_pairs = l.valid_pairs + r.valid_pairs + min(l.open - l.valid_pairs, r.close - r.valid_pairs);
        return res;
    }
    Data _query(int idx, int s, int e, int l, int r) {
        if (s > r || e < l) return make_data(0, 0, 0);
        if (l <= s && e <= r) return segtree[idx];
        int m = (s + e)/2;
        return merge(_query(2*idx, s, m, l, r), _query(2*idx + 1, m + 1, e, l, r));
    }
    ll query(int l, int r) {
        return _query(1, 0, n - 1, l, r).valid_pairs;
    }
};
string s;
int q;
