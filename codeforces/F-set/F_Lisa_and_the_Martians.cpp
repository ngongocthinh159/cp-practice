 ...
 * Solution for: https://codeforces.com/problemset/problem/1851/F
*/
...
void insert(node* root, int x, int ii) {
    node* p = root;
    int idx;
...
array<int, 3> find(node* root, int ai) {
    int x = 0, mx = 0, idx;
    node* p = root;
    for (int bit = k; bit >= 0; bit--) {
        idx = (ai >> bit) & 1;
...
void solve() {
    ...
    node* root = new node;
    insert(root, a[0], 0);
    int ans = INT_MIN, ii, jj, xx;
    for (int i = 1; i < n; i++) {
        auto [mx, x, j] = find(root, a[i]);
        insert(root, a[i], i);
