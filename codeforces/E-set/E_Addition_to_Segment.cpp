 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/4/3/practice/contest/274545/problem/E
*/
...
private:
    ...
    void _update(int idx, int s, int e, int l, int r, Updater &updater) {
        if (s > r || e < l) return;
        if (l <= s && e <= r) {
        ...
        int m = (s + e)/2;
        _update(2*idx, s, m, l, r, updater);
        _update(2*idx + 1, m + 1, e, l, r, updater);
    }
    ll _query(int idx, int s, int e, int pos) {
        if (s == e) return tree[idx].val + tree[idx].extra;
        int m = (s + e)/2;
        int ans = tree[idx].extra;
        if (pos <= m) {
            ans += _query(2*idx, s, m, pos);
        } else {
            ans += _query(2*idx + 1, m + 1, e, pos);
        }
        return ans;
    }
public:
