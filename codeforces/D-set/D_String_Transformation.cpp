 ...
 * Solution for: https://codeforces.com/problemset/problem/119/D
*/
...
void solve() {
    ...
    char SEPA = (char) 1;
    string ba = b + SEPA + a;
    vector<int> z_ba = z_function(ba);
    string rba = rb + SEPA + a;
    vector<int> z_rba = z_function(rba);;
    string rab = ra + SEPA + b;
    vector<int> kmp_rab = kmp_function(rab);
    ...
    for (int i = 0; i < len_a; i++) {
        ...
        int len_x = i + 1, len_y, len_z;
        if (len_x <= z_rba[len_b + 1]) ok_x = true;
        if (kmp_rab[len_a + len_b - len_x] > 0) {
            if (kmp_rab[len_a + len_b - len_x] + z_ba[len_b + 1 + len_x] >= len_a - len_x) 
                ok_y = true, ok_z = true;
        }
        if (ok_x && ok_y && ok_z) {
            ii = i; jj = len_a - kmp_rab[len_a + len_b - len_x];
        }
    }
    cout << ii << " " << jj << nline;
}
