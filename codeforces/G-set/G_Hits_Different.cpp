 ...
 * Solution for: https://codeforces.com/problemset/problem/1829/G
*/
...
ll dp[MAX + 1];
void pre_compute() {
    for (int i = 1, level = 1; i <= MAX; i += level, ++level) {
        int j = i + level;
        for (int k = i; k <= i + level - 1; k++, j++) {
            if (k <= MAX) dp[k] += 1LL*k*k;
            if (k <= MAX && k != i && k != i + level - 1) {
                int left = k - level;
                if (left >= 2) {
                    int left_right = left - (level - 1) + 1;
                    if (left_right >= 1) dp[k] -= dp[left_right];
                }
            }
            if (j <= MAX) dp[j] += dp[k];
            if (j + 1 <= MAX) dp[j + 1] += dp[k];
        }
    }
}
int n;
