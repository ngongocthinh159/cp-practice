 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/2/3/practice/status
*/
...
string s, t;
int q;
int compare(string &s, int start, string &t) {
    int i = start, j = 0;
    while (i < SZ(s) && j < SZ(t)) {
        if (s[i] > t[j]) return 1;
        if (s[i] < t[j]) return -1;
...
void solve() {
    ...
    for (int i = 0; i < q; i++) {
        ...
        cout << (upper_bound_idx - lower_bound_idx) << nline;
    }
}
