...
void pre_compute() {
    ...
    inv[N - 1] = expo(fact[N - 1], MOD1 - 2, MOD1);
    for (int i = N - 2; i >= 0; i--) inv[i] = inv[i + 1] * (i + 1) % MOD1;
}
ll nCk(ll n, ll k) {
    if (k > n) return 0;
    return fact[n] * inv[n - k] % MOD1 * inv[k] % MOD1;
}
void solve() {
    ll x1, x2, x3; cin >> x1 >> x2 >> x3;
    ll ans = 0;

    auto cnt = [](int i, ll x1, ll x2, ll x3) -> ll {
        ll c1 = i/2 + 1;
        if (x1 >= c1) {
            ll c3 = i/2 + (i&1);
            if (x3 >= c3) {
                ...
                ll r3 = x3 - c3;
                ll r2 = x2 - i;
                return nCk(r1 + c1 - 1, c1 - 1) * nCk(r3 + c3 - 1, c3 - 1) % MOD1 * nCk(r2 + x1 + x3, x1 + x3) % MOD1;
            }
        }
        return 0;
    };
