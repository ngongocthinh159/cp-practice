 ...
 * Solution for: https://vjudge.net/problem/UVA-10246
*/
...
void solve() {
    ...
    vector<int> v(n + 1);
    for (int i = 1; i <= n; i++) v[i] = cost[i].second;
    for (int i = 1; i <= n; i++) {
        dp[i][i][0] = 0;
    }
    for (int i = 0; i < r; i++) {
        int u, v, w; cin >> u >> v >> w;
        dp[u][v][0] = dp[v][u][0] = min(dp[u][v][0], w);
    }
    for (int k = 1; k <= n; k++) {
        for (int i = 1; i <= n; i++) {
            for (int j = 1; j <= n; j++) {
                dp[i][j][k] = min(dp[i][j][k], dp[i][j][k - 1]);
                if (dp[i][v[k]][k - 1] != IINF && dp[v[k]][j][k - 1] != IINF) {
                    dp[i][j][k] = min(dp[i][j][k], dp[i][v[k]][k - 1] + dp[v[k]][j][k - 1]);
                }
            }
        }
    }
    
    if (test > 1) cout << nline;
    cout << "Case #" << test++ << nline;
    for (int m = 0; m < q; m++) {
        int u1, u2; cin >> u1 >> u2;
        int ans = IINF;
        for (int i = 0; i <= n; i++) {
            ans = min(ans, dp[u1][u2][i] + max(ocost[u1], max(ocost[u2], ocost[v[i]])));
        }
        if (ans == IINF) cout << -1 << nline;
        else cout << ans << nline;
    }
}
