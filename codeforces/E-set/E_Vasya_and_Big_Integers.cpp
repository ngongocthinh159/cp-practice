 ...
 * Solution for: https://codeforces.com/contest/1051/problem/E
*/
...
#define MAX 1000005
string a, l, r;
int len_a, len_l, len_r, zl[2*MAX], zr[2*MAX];
ll dp[MAX], pref[MAX];
void z_func(string &s, int len, int *z) {
...
int compare(string &a, string &b, int i, int j, int *zba) { // compare substring a[i..j] with string b based on z(b + '#' + a)
    int compare_len = j - i + 1;
    int len_b = b.size();
    if (compare_len < len_b) return -1;
    if (compare_len > len_b) return 1;

    i += len_b + 1;
    if (zba[i] == len_b) return 0;
    if (b[zba[i]] < a[i - (len_b + 1) + zba[i]]) return 1;
    return -1;
}
void pre_compute() {
...
void solve() {
    ...
    string tmpl = l + "#" + a;
    string tmpr = r + "#" + a;
    z_func(tmpl, SZ(tmpl), zl);
    z_func(tmpr, SZ(tmpr), zr);

    int j = len_a - 1, k = len_a - 1, bestJ = INT_MAX;
    dp[len_a] = 1;
    for (int i = len_a - 1; i >= 0; i--) {
        if (a[i] == '0') {
            dp[i] = l == "0" ? dp[i + 1] : 0;
            pref[i] = (dp[i] + pref[i + 1]) % MOD1;
            continue;
        }
        while (j >= i && compare(a, l, i, j, zl) >= 0) {
            bestJ = j;
            --j;
        }
        while (k >= i && compare(a, r, i, k, zr) == 1) {
            --k;
        }
        if (bestJ <= k) {
            dp[i] = (((pref[bestJ + 1] - pref[k + 1]) % MOD1) + MOD1) % MOD1; 
            dp[i] = (dp[i] + dp[k + 1]) % MOD1;
        }
        pref[i] = (dp[i] + pref[i + 1]) % MOD1;
    }
    cout << dp[0] << nline;
