 ...
 * Solution for: https://codeforces.com/problemset/problem/128/B
*/
...
struct SuffixArray {
    ...
    int query_lcp_substr(int i, int j) {
        assert(i < n);
        assert(j < n);
...
void solve() {
    ...
    ll k; cin >> k;
    if (k > n*(n + 1)/2) {
        cout << "No such line." << nline;
        return;
    }
    SuffixArray sa(s);
    auto p = sa.get_p_array();
    auto lcp = sa.get_lcp_array();
    // sa._print();
    vector<int> next_idx(n, 0);
    // Idead: traverse from smallest substring and gradually increasing lexicographically,
    // for each substring, we need to count its occurrences and remove its contribution from k
    // + To count occurences of a substring, we need to count from it starting index in the p array, then as long as the lcp >= substring len then we found 1 occurence
    //      => For each substr just traverse the lcp array to count it contribution, when count we also the mark the next index of the substring that not yet count of the suffix (next_idx array)
    //          otherwise we will deplicate the count
    // + To traverse from smallest substring and gradually increasing, just traverse each prefix of the smallest surffix that has prefixes that not yet been counted
    for (int i = 0; i < n; i++) {
        for (int j = next_idx[i]; j < n - p[i]; j++) {
            int len = j + 1;
            k--;
            if (!k) {
                cout << s.substr(p[i], len) << nline;
                return;
            }
            for (int ii = i + 1; ii < n; ii++) {
                if (lcp[ii - 1] < len) break;
                k--;
                if (!k) {
                    cout << s.substr(p[ii], len) << nline;
                    return;
                }
                next_idx[ii] = len;
            }
