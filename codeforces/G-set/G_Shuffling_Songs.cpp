 ...
 * Solution for: https://codeforces.com/problemset/problem/1950/G
*/
...
#define MAXN 16
int n, w[MAXN], c[MAXN];
bool edge[MAXN][MAXN], dp[MAXN][1 << MAXN];
...
void solve() {
    ...
    string wr, ca;
    int id = 0;
    unordered_map<string,int> w_to_id, c_to_id;
    for (int i = 0; i < n; i++) {
        cin >> wr >> ca;
        int wid, caid;
        if (w_to_id.count(wr)) 
            ...
            wid = id++, w_to_id[wr] = wid;
        if (c_to_id.count(ca)) 
            caid = c_to_id[ca];
        else 
            caid = id++, c_to_id[ca] = caid;
        w[i] = wid;
        c[i] = caid;
    }
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++) if (i != j)
            edge[i][j] = (w[i] == w[j] || c[i] == c[j]);
    for (int u = 0; u < n; u++)
    ...
    for (int mask = 0; mask < (1 << n); mask++) {
        for (int u = 0; u < n; u++) if ((mask >> u) & 1) {
            ...
            for (int v = 0; v < n; v++) if (((mask >> v) & 1) && edge[u][v] && v != u) {
    ...
    int ans = IINF;
    for (int u = 0; u < n; u++)
        for (int mask = (1 << n) - 1; mask >= 0; mask--) if ((mask >> u) & 1) {
            if (dp[u][mask]) {
