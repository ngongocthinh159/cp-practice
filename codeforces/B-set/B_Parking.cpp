 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/7/2/practice/contest/289391/problem/B
*/
...
int n, m;
vector<int> nextUnoccupied;
int dfs(int x) {
    if (nextUnoccupied[x] == x) {
        nextUnoccupied[x] = x == n ? nextUnoccupied[1] : nextUnoccupied[x + 1];
        return x;
    }
    return nextUnoccupied[x] = dfs(nextUnoccupied[x]);
}
void solve() {
    cin >> n;
    nextUnoccupied.resize(n + 1);
    for (int i = 1; i <= n; i++) nextUnoccupied[i] = i;
    for (int i = 1; i <= n; i++) {
        int x; cin >> x;
        cout << dfs(x) << nline;
    }
