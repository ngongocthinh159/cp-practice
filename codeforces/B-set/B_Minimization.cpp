 ...
 * Solution for: https://codeforces.com/problemset/problem/571/B
*/
...
void solve() {
    ...
    ll dp[S + 1][L + 1] = {LINF};
    for (int i = 0; i <= S; i++)
        for (int j = 0; j <= L; j++) dp[i][j] = LINF;
    dp[0][0] = 0;
    for (int i = 0; i <= S; i++)
        for (int j = 0; j <= L; j++) {
            if (i == 0 && j == 0) continue;
            int last = i*size_s + j*size_l;
            if (i > 0)
                dp[i][j] = min(dp[i][j], dp[i - 1][j] + a[last] - a[last - size_s + 1]);
            if (j > 0)
                dp[i][j] = min(dp[i][j], dp[i][j - 1] + a[last] - a[last - size_l + 1]);
        }
    cout << dp[S][L] << nline;
}
