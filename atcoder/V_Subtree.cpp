...
void dfs_pre_suf(int u, int parent) {
    pre[u].resize(g[u].size());
    sur[u].resize(g[u].size());
    for (int i = 0; i < g[u].size(); i++) {
        ll prev = i - 1 < 0 ? 1 : pre[u][i - 1];
        ll cur = g[u][i] == parent ? 1 : in[g[u][i]];
        pre[u][i] = (prev*cur)%m;
    }
    for (int i = g[u].size() - 1; i >= 0; i--) {
        ll next = i + 1 >= g[u].size() ? 1 : sur[u][i + 1];
        ll cur = g[u][i] == parent ? 1 : in[g[u][i]];
        sur[u][i] = (next*cur)%m;
    }
    for (auto &v : g[u]) {
...
void dfs2(int u, int parent, ll out_u) {
    ll tmp1 = mod_sub(out_u, 1, m);
    ll tmp2 = mod_sub(in[u], 1, m);
    ll tmp3 = mod_mul(tmp1, tmp2, m);
    ans[u] = tmp3;
    for (int i = 0; i < g[u].size(); i++) {
        int v = g[u][i];
        if (v != parent) {
            ll pre_mul = i - 1 < 0 ? 1 : pre[u][i - 1];
            ll suf_mul = i + 1 >= g[u].size() ? 1 : sur[u][i + 1];
            ll tmp = mod_mul(pre_mul, suf_mul, m);
            tmp = mod_mul(tmp, tmp1, m);
            ll out_v = mod_add(tmp, 2, m);
            dfs2(v, u, out_v);
        }
    }
}
void solve() {
    ...
    pre.assign(n + 1, vector<ll>());
    sur.assign(n + 1, vector<ll>());
    for (int i = 0; i < n - 1; i++) {
        int u, v; cin >> u >> v;
        g[u].push_back(v);
        g[v].push_back(u);
