 ...
 * Solution for: https://codeforces.com/contest/1629/problem/D
*/
...
bool is_palindrome(string &s) {
    for (int i = 0, j = SZ(s) - 1; i < j; i++, j--) if (s[i] != s[j])
        return false;
    return true;
}
void solve() {
    ...
    // check len 2 and 3
    for (int i = 0; i < n; i++) if (is_palindrome(s[i])) {
        cout << "YES" << nline;
        return;
    }
    unordered_set<string> S, S3;
    {
        for (int i = 0; i < n; i++) {
            if (SZ(s[i]) == 2) {
                S.insert(s[i]);
    ...
    
    {
        // check len 4 and 6
        for (int i = 0; i < n; i++) {
            if ((SZ(s[i]) == 2 && S.count(rs[i])) || 
                (SZ(s[i]) == 3 && S3.count(rs[i]))) {
                cout << "YES" << nline;
                return;
            }
        }
