 ...
 * Solution for: https://codeforces.com/contest/741/problem/D
*/
...
#define MAXN 500005
int n, cur_ans, sz[MAXN], big[MAXN], height[MAXN], ans[MAXN], vmask[MAXN], mask[1 << 22];
...
void del_sub(int u, int par) {
    mask[vmask[u]] = 0;
    for (auto [v, c] : g[u]) if (v != par)
        del_sub(v, u);
}
void relax(int u, int v, int r) {
    cur_ans = max(cur_ans, height[u] + height[v] - 2*height[r]);
}
void cal(int v, int root) {
    int tmask = 0^vmask[v];
    if (mask[tmask])
        relax(mask[tmask], v, root);
    for (int j = 0; j < 22; j++) {
        tmask = (1 << j)^vmask[v];
        if (mask[tmask])
            relax(mask[tmask], v, root);
...
void cal_sub(int u, int par, int root) {
    cal(u, root);
    for (auto [v, c] : g[u]) if (v != par) {
        cal_sub(v, u, root);
    }
...
void add(int v) {
    if (!mask[vmask[v]] || height[mask[vmask[v]]] < height[v]) 
        mask[vmask[v]] = v;
}
void add_sub(int u, int par) {
    add(u);
    for (auto [v, c] : g[u]) if (v != par)
        add_sub(v, u);
}
void dfs2(int u, int par, bool keep) {
    for (auto [v, c] : g[u]) if (v != par && v != big[u])
        dfs2(v, u, 0), maximize(ans[u], ans[v]);
    if (big[u] != -1)
        dfs2(big[u], u, 1), maximize(ans[u], ans[big[u]]);
