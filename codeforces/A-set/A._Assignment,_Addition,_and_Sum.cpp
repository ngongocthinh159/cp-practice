 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/5/4/practice/contest/280801/problem/A
*/
...
void apply(int idx, int s, int e, const Lazy &apply_lazy) {
    if (apply_lazy.type == ADD) {
        segtree[idx].sum += apply_lazy.val*(e - s + 1);
        if (lazy[idx].type == NONE) {
            lazy[idx] = apply_lazy;
        } else if (lazy[idx].type == ADD || lazy[idx].type == SET) {
            lazy[idx].val += apply_lazy.val;
        }
    } else if (apply_lazy.type == SET) {
        segtree[idx].sum = apply_lazy.val*(e - s + 1);
        if (lazy[idx].type == NONE) {
            lazy[idx] = apply_lazy;
...
void update(int idx, int s, int e, int l, int r, const Lazy &lazy) { // range update + val
    if (s > r || e < l) return;
    if (l <= s && e <= r) {
        apply(idx, s, e, lazy);
        return;
    ...
    push_down(idx, s, e);
    int m = (s + e)/2;
    update(2*idx, s, m, l, r, lazy);
    update(2*idx + 1, m + 1, e, l, r, lazy);
    segtree[idx] = merge(segtree[2*idx], segtree[2*idx + 1]);
}
...
void solve() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        if (t == 1) {
            cin >> l >> r >> val;
            r--;
            update(1, 0, n - 1, l, r, Lazy({val, SET}));
        } else if (t == 2) {
            cin >> l >> r >> val;
            r--;
            update(1, 0, n - 1, l, r, Lazy({val, ADD}));
        } else {
            cin >> l >> r;
            r--;
