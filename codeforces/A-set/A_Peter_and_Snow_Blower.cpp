...
struct Line {
    ...
    double distFromPoint(const Point& p, Point &projectedPoint) const { // projectedPoint is projection of point q into current line
...
void solve() {
    ...
    x[n] = x[0], y[n] = y[0];
    for (int i = 0; i <= n; i++) {
        mx = max(P.dist(Point(x[i], y[i])), mx);
        mn = min(mn, P.dist(Point(x[i], y[i])));
        if (i >= 1) {
            Point A(x[i], y[i]);
            Point B(x[i - 1], y[i - 1]);
            Line l(A, B);
            Point C;
            double d = l.distFromPoint(P, C);
            if (cmp((A - C).len() + (C - B).len(), (A - B).len()) == 0) 
                mn = min(mn, d);
        }
    }
