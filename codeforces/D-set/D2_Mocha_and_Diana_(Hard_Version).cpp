 ...
 * Solution for: https://codeforces.com/contest/1559/problem/D2
*/
...
void solve() {
    ...
    for (int i = 2; i <= n; i++) {
        if (!d1.same_set(1, i) && !d2.same_set(1, i)) {
            ans.pb({1, i});
            d1.unite(1, i);
            d2.unite(1, i);
        }
    ...
    for (int i = 2; i <= n; i++) {
        if (!d1.same_set(1, i)) s1.insert(d1.find(i));
        else if (!d2.same_set(1, i)) s2.insert(d2.find(i));
    }
    if (sz(s1) > sz(s2)) swap(s1,s2);
    vector<int> v1(all(s1));
    vector<int> v2(all(s2));
    for (int i = 0; i < v1.size(); i++) ans.pb({v1[i], v2[i]});
    cout << ans.size() << nline;
    for (auto p : ans) cout << p.first << " " << p.second << nline;
}
