 ...
 * Solution for: https://codeforces.com/problemset/problem/1198/D
*/
...
ll dfs(int t, int b, int l, int r) {
    int H = b - t + 1;
    int W = r - l + 1;
    if (!H || !W)
        return 0;
    ...
    ll C = max(H, W);
    if (H == max(H, W)) {
        for (int i = t; i <= b; i++) if (!query(i, l, i, r))
            C = min(C, dfs(t, i - 1, l, r) + dfs(i + 1, b, l, r));
    }
    if (W == max(H, W)) {
        for (int i = l; i <= r; i++) if (!query(t, i, b, i))
            C = min(C, dfs(t, b, l, i - 1) + dfs(t, b, i + 1, r));
    }
    return dp[t][b][l][r] = C;
}
void solve() {
    ...
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++) {
            char x; cin >> x;
            int a = x == '#' ? 1 : 0;
            pref[i + 1][j + 1] = a + pref[i + 1][j] + pref[i][j + 1] - pref[i][j];
        }
    cout << dfs(0, n - 1, 0, n - 1) << nline;
}
