...
// dp[i][j][_k]: min paint if we stand at i (choose paint i with any color or not), and last color is j and we already create _k groups
ll dfs(int i, int j, int _k, vector<vector<vector<ll>>> &dp) {
    if (i == n) {
        if (_k != k) return INF;
        return 0;
    }
    if (_k > k) return INF;
    if (dp[i][j][_k] != -1) return dp[i][j][_k];

    ll res = INF;
    if (c[i] != 0) {
        res = dfs(i + 1, c[i], (c[i] != j ? _k + 1 : _k), dp);
    } else {
        for (int q = 1; q <= m; q++) {
            ll tmp = dfs(i + 1, q, (j != q ? _k + 1 : _k), dp);
            if (tmp != (ll) INF) {
                tmp = tmp + p[i][q - 1];
                res = min(res, tmp);
            }
