 ...
 * Solution for: https://codeforces.com/contest/2085/problem/E
*/
...
#define MAXN 10005
#define MAXA 1000005
int n, a[MAXN], b[MAXN], cnt1[MAXA], cnt2[MAXA];

void pre_compute() {

}
bool check(int x) {
    for (int i = 0; i < n; i++) cnt1[a[i]%x]++, cnt2[b[i]]++;
    bool res = true;
    for (int i = 0; i < n; i++) if (cnt1[b[i]] != cnt2[b[i]]) {
        res = false;
        break;
    }
    for (int i = 0; i < n; i++) cnt1[a[i]%x]--, cnt2[b[i]]--;
...
void solve() {
    cin >> n;
    ll delta = 0;
    for (int i = 0; i< n; i++) cin >> a[i], delta += a[i];
    for (int i = 0; i <n; i++) cin >> b[i], delta -= b[i];
    if (delta == 0) {
        int k = 1e6 + 1;
        if (check(k))
            cout << k << nline;
