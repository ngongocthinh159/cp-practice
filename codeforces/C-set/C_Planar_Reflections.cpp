 ...
 * Solution for: https://codeforces.com/contest/1498/problem/C
*/
...
int n, k;
ll dp[MAX][MAX][2];
ll dfs(int decay, int right_cnt, int dir) {
    // cout << decay << " " << right_cnt << " " << dir << nline;
    if (decay == 0) return 0;
    if (dir == 1 && right_cnt == 0) return 1;
    if (dir == 0 && n - right_cnt == 0) return 1;
    if (dp[decay][right_cnt][dir] != -1) return dp[decay][right_cnt][dir];

    int reverse_dir = dir^1^0;
    ll res = 0;
    res = dfs(decay, dir == 1 ? right_cnt - 1 : right_cnt + 1, dir);
    res = (res + dfs(decay - 1, right_cnt, reverse_dir)) % MOD;
    return dp[decay][right_cnt][dir] = res;
}
void solve() {
    cin >> n >> k;
    for (int i = 0; i <= k; i++) {
