 ...
 * Solution for: https://codeforces.com/contest/19/problem/B
*/
...
void solve() {
    ...
    vector<vector<ll>> dp(n + 1,
        vector<ll>(2*n + 1, INF)); // dp[i][time]: min money spent if only buy product from i -> n - 1 and have 'time' free time due to previous buy
    vector<ll> surfixSum(n + 1);
    for (int i = n; i >= 1; i--) {
        surfixSum[i] = i == n ? t[i - 1] : surfixSum[i + 1] + t[i - 1];
    }
    dp[0][n] = 0;
    for (int i = 0; i < n; i++) {
        for (int time = 0; time <= n + n; time++) {
            ...
            // pay
            if (time + t[i] > 2*n) dp[i + 1][2*n] = min(dp[i + 1][2*n], dp[i][time] + p[i]);
            else dp[i + 1][time + t[i]] = min(dp[i + 1][time + t[i]], dp[i][time] + p[i]);
            
            // steal
            if (time > 0) dp[i + 1][time - 1] = min(dp[i + 1][time - 1], dp[i][time]);
        }
    }
    ll ans = LLONG_MAX; 
    for (int time = n; time <= 2*n; time++) {
        ans = min(ans, dp[n][time]);
    }
    cout << ans << nline;
}
