 ...
 * Solution for: https://codeforces.com/problemset/problem/104/C
*/
...
void solve() {
    cin >> n >> m;
    for (int i = 1; i <= n; i++) visited[i] = active[i] = false;
    for (int i = 0; i < m; i++) {
        int u, v; cin >> u >> v;
        g[u].push_back(v);
        g[v].push_back(u);
