 ...
 * Solution for: https://open.kattis.com/problems/scenes
*/
...
void solve() {
    ...
    for (int i = 0; i <= N; i++) {
        for (int w = 0; w <= W; w++) {
            ll can = min(H, W - w);
            dp[i + 1][w + 0] = (dp[i + 1][w + 0] + dp[i][w]) % MOD;
            dp[i + 1][w + can + 1] = (dp[i + 1][w + can + 1] - dp[i][w]) % MOD;
            if (dp[i + 1][w + can + 1] < 0) dp[i + 1][w + can + 1] += MOD;
        }
        for (int w = 1; w <= W; w++) 
            dp[i + 1][w] += dp[i + 1][w - 1], dp[i + 1][w] %= MOD;
    }
    ll ans = 0;
    for (int w = 0; w <= W; w++) ans = (ans + dp[N][w]) % MOD;
