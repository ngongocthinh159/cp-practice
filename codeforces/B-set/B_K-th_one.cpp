 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/4/2/practice/contest/273278/problem/B
*/
...
private:
    ...
    int _query(int idx, int s, int e, int k) {
        if (s == e) return s;
        int m = (s + e)/2;
        Node l_ans, r_ans, ans;
        if (tree[2*idx].val >= k) {
            return _query(2*idx, s, m, k);
        } else {
            return _query(2*idx + 1, m + 1, e, k - tree[2*idx].val);
        }
    }
public:
    vector<Node> tree;
