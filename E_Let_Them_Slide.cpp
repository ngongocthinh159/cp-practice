...
#define N 1000005
int n, w, l, startIndex, endIndex, len, len2, prevEnd;
int nums[N];
ll res[N];
bool isFirst = true;
...
void solve() {
    ...
    f(k,0,n) {
        ...
        f(i,0,w) {
            len = w - i;
            if (l <= len) startIndex = 0;
            else startIndex = l - len;
            len2 = w - len + 1;
            if (l<=len2) endIndex = l - 1; 
            else endIndex = len2 - 1;

            int j;
            if (!isFirst) {j = prevEnd + 1; isFirst = false;}
            else j = startIndex;
            for (; j <= endIndex; j++) {
                while (dq.size() > 0 && nums[j] >= nums[dq.front()]) dq.pop_front();
                dq.push_front(j);
            }
            while (dq.size() > 0 && !(startIndex <= dq.back())) { dq.pop_back();}

            if (nums[dq.back()] >= 0) res[i] += nums[dq.back()];
            else {
                if (l <= i || (l <= w-i-1)) res[i] += 0;
