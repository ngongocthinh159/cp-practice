 ...
 * Solution for: https://lightoj.com/problem/counting-perfect-bst
*/
...
unordered_set<ll> S;
vector<ll> perfect;
ll fac[2*MAXN], inv[2*MAXN], low, hi;
void pre_compute() {
    for (ll i = 2; i < MAXN; i++) {
        ll cur = i*i;
        while (cur <= MAXA) {
...
void solve() {
    cin >> low >> hi;
    auto it1 = lower_bound(ALL(perfect), low);
    auto it2 = upper_bound(ALL(perfect), hi);
    if (it1 == it2) {
        cout << 0 << nline;
        return;
    }
