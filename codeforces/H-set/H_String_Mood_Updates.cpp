 ...
 * Solution for: https://codeforces.com/gym/102644/problem/H
*/
...
Data segtree[4*MAXN];
Data make_data(ll c1, ll c2, ll c3, ll c4) {
    Data res({c1, c2, c3, c4});
    return res;
}
Data get_data(char c) {
    Data res;
    if (c == '?') 
        res = make_data(19, 6, 7, 20);
    else if (c == 'S' || c == 'D')
        res = make_data(0, 0, 1, 1);
    else if (c == 'H')
        res = make_data(1, 1, 0, 0);
    else if (is_vowel(c))
        res = make_data(0, 1, 1, 0);
    else
        res = make_data(1, 0, 0, 1);
    return res;
}
Data merge(Data &l, Data &r) {
    Data res;
    res.c1 = (r.c1 * l.c1 % MOD + r.c2 * l.c3 % MOD) % MOD;
    res.c2 = (r.c1 * l.c2 % MOD + r.c2 * l.c4 % MOD) % MOD;
    res.c3 = (r.c3 * l.c1 % MOD + r.c4 * l.c3 % MOD) % MOD;
    res.c4 = (r.c3 * l.c2 % MOD + r.c4 * l.c4 % MOD) % MOD;
    return res;
}
void build(int idx, int s, int e, string &mood) {
    if (s == e) {
        segtree[idx] = get_data(mood[s]);
        return;
    }
    int m = (s + e)/2;
    build(2*idx, s, m, mood);
    build(2*idx + 1, m + 1, e, mood);
    segtree[idx] = merge(segtree[2*idx], segtree[2*idx + 1]);
}
void update(int idx, int s, int e, int pos, char c) {
    if (s == e) {
        segtree[idx] = get_data(c);
        return;
    }
    int m = (s + e)/2;
    if (pos <= m) 
...
void solve() {
    ...
    build(1, 0, n - 1, s);
    ll h0 = 1, s0 = 0;
    ans[0] = (segtree[1].c1 * h0 + segtree[1].c2 * s0) % MOD;
    for (int i = 0; i < q; i++) {
        int pos;
        char c;
        cin >> pos >> c;
        ...
        ans[i + 1] = (segtree[1].c1 * h0 + segtree[1].c2 * s0) % MOD;
    }
    for (int i = 0; i < q + 1; i++)
        cout << ans[i] << nline;
}
