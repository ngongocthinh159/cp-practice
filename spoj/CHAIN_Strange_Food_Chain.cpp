 ...
 * Solution for: https://www.spoj.com/problems/CHAIN/
*/
...
struct DSU {
    vector<int> parent, size, minus_par;
    DSU(int n) {
        parent.resize(n + 1);
        size.resize(n + 1);
        minus_par.resize(n + 1);
        iota(ALL(parent), 0);
        fill(ALL(size), 1);
        fill(ALL(minus_par), 0);
    }
    int find(int x) {
        if (x == parent[x]) return x;
        int par = find(parent[x]);
        minus_par[x] = ((minus_par[x] + minus_par[parent[x]]) % 3 + 3) % 3;
        return parent[x] = par;
    }
    bool unite(int a, int b, int val) {
        ...
        if (pa != pb) {
            ...
            minus_par[pb] = (((minus_par[a] + val) % 3 - minus_par[b]) % 3 + 3) % 3;
            size[pa] += size[pb];
            parent[pb] = pa;
            return true;
        } else {
            int current_val = ((minus_par[b] - minus_par[a]) % 3 + 3) % 3;
            if (current_val != val)
                return false;
            return true;
        }
