 ...
 * Solution for: https://codeforces.com/contest/1944/problem/C
*/
...
void solve() {
    ...
    if (mn != 0) {cout << 0 << nline; return;}
    vector<pair<int,int>> v;
    for (auto [key,val] : mp) {
        if (v.size() == 0) v.push_back({key,val});
        else {
            if (key != v.back().first + 1) break;
            else {
                v.push_back({key,val});
            }
        }
    ...
    cout << (idx2 == -1 ? v.back().first : idx2-1) + 1 << nline;
}
