 ...
 * Solution for: https://codeforces.com/gym/102644/problem/E
*/
...
struct Matrix {
    ...
    Matrix(int _n, int _m) : n(_n), m(_m) {
        a.assign(n, vector<unsigned int>(m, 0));
    }
    Matrix(vector<vector<unsigned int>> &v) {
        n = v.size();
        m = v[0].size();
        a.assign(n, vector<unsigned int>(m, 0));
        for (int i = 0; i < n; i++)
    ...
    Matrix(const vector<vector<unsigned int>> &v) {
        n = v.size();
        m = v[0].size();
        a.assign(n, vector<unsigned int>(m, 0));
        for (int i = 0; i < n; i++)
    ...
    vector<unsigned int>& operator[](int i) {
        return a[i];
    }
    const vector<unsigned int>& operator[](int i) const {
        return a[i];
    }
