...
#define LOG 20
#define MAX 100005
#define MIN_HIGH(x, y) (hei[x] < hei[y] ? (x) : (y))

int n, m;
vector<int> g[MAX];
vector<int> lazy_nodes;
int min_dist[MAX], hei[MAX], etour[MAX << 1], pos[MAX], cnt = -1;
int rmq[LOG + 1][MAX << 1]; // min height in range
void pre_compute() {
...
void update_dist() {
    ...
    while (q.size()) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            min_dist[u] = min(min_dist[u], level);
            for (auto v : g[u]) {
                if (!visited[v]) {
                    visited[v] = true;
                    q.push_front(v);
...
void change_color(int u) {
    lazy_nodes.push_back(u);
    if (lazy_nodes.size() >= THRESH) {
        update_dist();
        lazy_nodes.clear();
    }
}
...
int query_rmq(int u, int v) {
    ...
    int pv = pos[v];
    if (pu > pv) swap(pu, pv);
    int msb = 32 - __builtin_clz(pv - pu + 1) - 1;
    return MIN_HIGH(rmq[msb][pu], rmq[msb][pv - (1 << msb) + 1]);
}
int dist(int u, int v) {
    int lca_uv = query_rmq(u, v);
    return hei[u] + hei[v] - 2*hei[lca_uv];
}
int query_min(int u) {
    int res = min_dist[u];
    for (auto v : lazy_nodes) {
...
void solve() {
    ...
    dfs(1, -1);
    for (int i = 0; i <= cnt; i++) rmq[0][i] = etour[i];
    for (int j = 1; j <= LOG; j++) {
        for (int i = 0; i <= cnt; i++) {
            if (i + (1 << j) - 1 <= cnt) {
