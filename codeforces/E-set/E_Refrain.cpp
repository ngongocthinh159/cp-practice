 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/2/5/practice/contest/269656/problem/E
*/
...
void solve() {
    ...
    ll len = n, product = n, idx = 0;
    vector<int> st, L(n - 1, -1), R(n - 1, lcp.size());
    for (int i = 0; i < n - 1; i++) {
    ...
    for (int i = 0; i < n - 1; i++) {
        if (maximize(product, 1LL*(R[i] - L[i])*lcp[i])) {
            idx = p[i];
            len = lcp[i];
        }
    }
    cout << product << nline;
    cout << len << nline;
    for (int j = idx; j < idx + len; j++) cout << (s[j] - 'a') << " ";
}
