...
#define MAXN 200005
#define next __next
int next[MAXN], parent[MAXN], sz[MAXN];
int n, q;
int find(int x) {
...
int get_next(int x, int cur_set) {
    if (x == next[x] || find(x) != cur_set) return x;
    return next[x] = get_next(next[x], cur_set);
}
void pre_compute() {
...
void solve() {
    ...
    for (int i = 0; i < q; i++) {
        ...
        if (type == 1) {
            ...
            cin >> x >> y;
            x--, y--;
            if (find(x) == find(y)) cout << "YES" << nline;
            else cout << "NO" << nline;
        }
    }
}
