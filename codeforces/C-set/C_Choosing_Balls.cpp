 ...
 * Solution for: https://codeforces.com/contest/264/problem/C
*/
...
ll cal(int a, int b) {
    ...
    pair<ll,int> mx1 = {-LINF, -1};
    pair<ll,int> mx2 = {-LINF, -1};
    ll ans = 0;
    for (int i = 0; i < n; i++) {
        int x = c[i];
        if (dp[x] != -LINF)
            dp[x] = max(dp[x], dp[x] + 1LL * a * v[i]);
        dp[x] = max(dp[x], 1LL * b * v[i]);
        if (mx1.fi != -LINF) {
            if (mx1.se == c[i]) {
                if (mx2.fi != -LINF) {
                    dp[x] = max(dp[x], mx2.fi + 1LL * b * v[i]);
                }   
            } else {
                dp[x] = max(dp[x], mx1.fi + 1LL * b * v[i]);
            }
        }
        if (c[i] == mx1.se || c[i] == mx2.se) {
            if (mx1.se == c[i]) mx1.fi = max(mx1.fi, dp[x]);
            if (mx2.se == c[i]) mx2.fi = max(mx2.fi, dp[x]);
            if (mx1.fi < mx2.fi)
                swap(mx1, mx2);
        } else {
            if (dp[x] > mx1.fi) {
