 ...
 * Solution for: https://codeforces.com/contest/1042/problem/B
*/
...
#define MAXN 1005
int n, vit[MAXN], c[MAXN], dp[8][MAXN];
...
void solve() {
    ...
    dp[0][0] = 0;
    for (int j = 0; j < 8; j++)
        for (int i = 0; i < n; i++) if (dp[j][i] != IINF) {
            dp[j | vit[i + 1]][i + 1] = min(dp[j | vit[i + 1]][i + 1], dp[j][i] + c[i + 1]);
            dp[j][i + 1] = min(dp[j][i + 1], dp[j][i]);
        }
    cout << (dp[0b111][n] != IINF ? dp[0b111][n] : -1) << nline; 
}
