 ...
 * Solution for: https://oj.vnoi.info/problem/catalan
*/
...
void solve() {
    ...
    for (int i = m - 2; i >= 0; i--) {
        for (int h = 0; h <= m; h++) {
            if (h - 1 >= 0) dp[i][h] += dp[i + 1][h - 1];
            if (h + 1 < m) dp[i][h] += dp[i + 1][h + 1];
        }
    }
    ...
    for (int i = 1; i < m; i++) {
        ll down = 0;
        if (prev - 1 >= 0) down = dp[i][prev - 1];
        bool go_down = false;
        if (prev - 1 == a[i]) go_down = true;
        if (!go_down) kth += down;
        prev = a[i];
    }
    cout << (kth + 1) << nline;
