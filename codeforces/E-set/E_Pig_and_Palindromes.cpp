 ...
 * Solution for: https://codeforces.com/contest/570/problem/E
*/
...
void solve() {
    ...
    for (int i = 1; i <= steps; i++) {
        ...
        int c1_s = 1 + max(0, i - (n - 1)), c1_e = min(i + 1, m);
        int c2_s = max(1, m - i), c2_e = m - max(0, i - (n - 1));

        for (int y1 = c1_s; y1 <= c1_e; y1++)
            for (int y2 = c2_s; y2 <= c2_e; y2++) {
                int x1 = i - y1 + 2; // x1 + y1 - 2 = i => x1 = i - y1 + 2
                int x2 = n + m - y2 - i; // n - x2 + m - y2 = i => x2 = n + m - y2 - i
                if (x1 <= x2 && y1 <= y2 && s[x1][y1] == s[x2][y2]) {
                    add(dp[t][y1][y2], dp[!t][y1][y2]);
                    add(dp[t][y1][y2], dp[!t][y1][y2 + 1]);
                    add(dp[t][y1][y2], dp[!t][y1 - 1][y2]);
                    add(dp[t][y1][y2], dp[!t][y1 - 1][y2 + 1]);
                }
            }
        t = !t;
    }
    int ans = 0;
    for (int j = 1; j <= m; j++) {
        add(ans, dp[steps&1][j][j]);
        if (n+m & 1) add(ans, dp[steps&1][j][j + 1]);
    }
    cout << ans << nline;
}
