 ...
 * Solution for: https://codeforces.com/gym/101992/problem/D
*/
...
int n;
int a[mxn + 1];
ll dp[mxn + 1], cnt[mxn + 1], inv[mxn + 1], sur[mxn + 1];
void pre_compute() {
    inv[0] = inv[1] = 1;
	for (int i = 2; i <= mxn; i++) {
...
void solve() {
    ...
    for (int i = mx; i >= 1; i--) {
        ...
        // Total sum of multiplying all pairs (include itself):
        // (cnt[1]*a[1] + cnt[2]*a[2] + cnt[3]*a[3] + ...) ^ 2
        for (int j = start; j <= end; j++) {
            sum = (sum + cnt[i*j]*(i * j)%MOD) % MOD;
        }
        sum = sum * sum % MOD;
