...
bool isOk(pair<int,int>& p1, pair<int,int>& p2, vector<vector<char>>& matrix) {
    bool okX1 = true, okX2 = true;
    cf(i,min(p1.fi,p2.fi),max(p1.fi,p2.fi)) {
        if (matrix[i][p1.se] == 'W') okX1 = false;
        if (matrix[i][p2.se] == 'W') okX2 = false;
        if (!okX1 && !okX2) return false;
    }

    bool okY1 = true, okY2 = true;
    cf(j,min(p1.se,p2.se),max(p1.se,p2.se)) {
        if (matrix[p1.fi][j] == 'W') okY1 = false;
        if (matrix[p2.fi][j] == 'W') okY2 = false;
...
void solve() {
    ...
    f(i,0,v.size() - 1) {
        for (int j = i + 1; j < v.size(); j++) {
            if (!isOk(v[i],v[j],matrix)) {cout << "NO\n"; return;}
        }
    }
    cout << "YES\n";
}
