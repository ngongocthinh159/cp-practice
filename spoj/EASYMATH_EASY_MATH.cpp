 ...
 * Solution for: https://www.spoj.com/problems/EASYMATH/
*/
...
/*---------------------------------------------------------------------------------------------------------------------------*/
ll __gcd__(ll a, ll b) {if (!a || !b) return a | b; unsigned shift = __builtin_ctzll(a | b); a >>= __builtin_ctzll(a); do { b >>= __builtin_ctzll(b); if (a > b) swap(a, b); b -= a; } while (b); return a << shift;} // only a >= 0 && b >= 0
...
ll f() {
    ...
    for (int mask = 1; mask < (1 << 5); mask++) {
        ...
        ll cur_lcm = 1;
        for (int bit = 5 - 1; bit >= 0; bit--) if ((mask >> bit) & 1) {
        	ll cur_gcd = __gcd(cur_lcm, a + bit*d);
        	if (cur_lcm / cur_gcd <= m / (a + bit*d)) {
            cur_lcm = cur_lcm / cur_gcd * (a + bit*d);
          } else {
            cur_lcm = m + 1;
            break;
          }
        ...
        int sign = len&1 ? -1 : 1;
        res += sign * (m / cur_lcm - (n - 1) / cur_lcm);
    }
    return res;
}
void solve() {
