 ...
 * Solution for: https://codeforces.com/contest/2090/problem/C
*/
...
#define MAXN 50005
int n;
int t[MAXN], cnt[MAXN], x[4*MAXN], y[4*MAXN], tbl[4*MAXN], dist[4*MAXN], order[4*MAXN], used[4*MAXN];
vector<pair<int,int>> moves = {{0,0},{1,0},{0,1},{1,1}};
void pre_compute() {
...
void solve() {
    ...
    int tot = 0, top = 0;
    for (int level = 0; tot < n; level++) {
        for (int i = 1, j = 3*level + 1, tbls = 1; tbls <= level + 1 && tot < n; i += 3, j -= 3, tbls++) {
            for (auto &m : moves) {
                int I = i + m.fi;
                int J = j + m.se;
                x[top] = I;
                y[top] = J;
                tbl[top] = tot;
                dist[top] = I + J + (m.fi == 1 && m.se == 1 ? 2 : 0);
                top++;
            }
            tot++;
        }
    ...
    iota(order, order + top, 0);
    sort(order, order + top, [&](int i, int j) {
        if (dist[i] < dist[j]) return true;
        if (dist[i] == dist[j] && x[i] < x[j]) return true;
        if (dist[i] == dist[j] && x[i] == x[j] && y[i] < y[j]) return true;
        return false;
    });
    int i = 0, j = 0;
    for (int k = 0; k < n; k++) {
        if (t[k]) {
            while (i < top && used[i]) i++;
            used[i] = true;
            cout << x[order[i]] << " " << y[order[i]] << nline;
            cnt[tbl[order[i]]]++;
            i++;
        } else {
            while (j < top && (used[j] || cnt[tbl[order[j]]] != 0)) j++;
            used[j] = true;
            cout << x[order[j]] << " " << y[order[j]] << nline;
            cnt[tbl[order[j]]]++;
            j++;
        }
    }
