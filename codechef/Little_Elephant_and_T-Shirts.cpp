 ...
 * Solution for: https://www.codechef.com/problems/TSHIRTS
*/
...
void solve() {
    ...
    int diffTShirts = v.size();
    int maxMask = (1 << n) - 1;
    vector<vector<ll>> dp(diffTShirts + 1,
        vector<ll>(maxMask + 1, 0));
    dp[diffTShirts][maxMask] = 1;
    for (int i = diffTShirts - 1; i >= 0; i--) {
        
        for (int mask = 0; mask <= maxMask; mask++) {
            dp[i][mask] = (dp[i][mask] + dp[i + 1][mask]) % MOD;
            for (int j = 0; j < n; j++) {
                if (!((mask >> j) & 1) && p[j].count(v[i])) {
