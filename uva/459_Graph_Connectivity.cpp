 ...
 * Solution for: https://onlinejudge.org/index.php?option=com_onlinejudge&Itemid=8&page=show_problem&problem=400
*/
...
int n;
char c;
void dfs(int u, vector<set<int>> &g, vector<bool> &visited) {
    visited[u] = true;
    for (auto v : g[u]) {
...
void solve() {
    ...
    int cnt = 0;
    for (int i = 0; i < n; i++) {
        if (!visited[i]) dfs(i, g, visited), cnt++;
    }
    cout << cnt << nline;
}
