 ...
 * Solution for: https://atcoder.jp/contests/abc065/tasks/arc076_b
*/
...
struct DSU {
    vector<int> parent;
    vector<int> size;
    DSU(int n, bool prepopulate = true) {
        parent.resize(n + 1);
        size.resize(n + 1);
        if (prepopulate) {
...
void solve() {
    ...
    vector<pair<int,int>> point_x(n);
    vector<pair<int,int>> point_y(n);
    for (int i = 0; i < n; i++) {
        int x, y; cin >> x >> y;
        point.push_back({x,y});
        point_x[i] = {x,i};
    ...
    for (int i = 1; i < n; i++) {
        int x_x = point_x[i].first - point_x[i - 1].first;
        int y_y = point_y[i].first - point_y[i - 1].first;
        edges.push_back({x_x, point_x[i].second, point_x[i - 1].second});
        edges.push_back({y_y, point_y[i].second, point_y[i - 1].second});
    }
    sort(all(edges));
    DSU dsu(n);
    int cnt = 0;
