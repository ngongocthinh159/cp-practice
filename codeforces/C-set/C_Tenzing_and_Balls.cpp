 ...
 * Solution for: https://codeforces.com/problemset/problem/1842/C
*/
...
void solve() {
    ...
    for (int i = n - 1; i >= 0; i--) {
        if (!M.count(a[i])) {
            ...
            val = M[a[i]];
            dp[i] = val - i; // choose to cut from i -> j (j that maximize: j - i + 1 + dp[j + 1])
            dp[i] = max(dp[i], dp[i + 1]); // choose to not cut i
            new_val = i + 1 + dp[i + 1];
            if (new_val > val) M[a[i]] = new_val;
        }
    }
    cout << dp[0] << nline;
}
