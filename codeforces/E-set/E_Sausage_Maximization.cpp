 ...
 * Solution for: https://codeforces.com/problemset/problem/282/E
*/
...
void solve() {
    ...
    Node *root = new Node;
    insert(root, 0);
    ll cur_pre = 0, cur_sur = 0;
    ll ans = 0;
    for (int i = 0; i < n; i++) cur_pre ^= a[i];
    for (int i = n - 1; i >= 0; i--) {
        ans = max(ans, find_max(root, cur_pre));
        cur_pre ^= a[i];
        cur_sur ^= a[i];
        ans = max(ans, cur_sur);
        insert(root, cur_sur);
