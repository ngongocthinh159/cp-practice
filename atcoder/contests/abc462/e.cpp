...
void solve() {
    ...
    ll minDist = min(X, Y);
    ll maxDist = max(X, Y);
    ll ans = minDist * 2 * minCost;
    ll r = maxDist - minDist;

    ll firstCost = Y >= X ? B : A;
    ll secondCost = firstCost ^ A ^ B;
    if (3*minCost >= maxCost) {
        ans += r / 2 * (A + B) + (r%2) * firstCost;
    } else {
        if (firstCost == maxCost) {
            ans += r / 2 * (4 * minCost) + (r%2) * 3 * minCost;
        } else {
            ans += r / 2 * (4 * minCost) + (r%2) * minCost;
        }
    }
    cout << ans << nline;
}
