 ...
 * Solution for: https://www.spoj.com/problems/KATHTHI/ 
*/
...
void solve() {
    ...
    while (q.size()) {
        ...
        int j = cur.ss;
        if (i == n - 1 && j == m - 1) {
            cout << dist[i][j] << nline; return;
        }
        for (auto &move: moves) {
            int I = i + move[0];
            int J = j + move[1];
            if (0 <= I && I < n && 0 <= J && J < m) {
                int cost = (matrix[i][j] != matrix[I][J]);
                int w_v = dist[i][j] + cost;
                if (w_v < dist[I][J]) {
                    dist[I][J] = w_v;
                    if (cost == 1) q.push_back({I, J});
