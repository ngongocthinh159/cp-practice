 ...
 * Solution for: https://codeforces.com/contest/519/problem/E
*/
...
int n, m, xi, yi;
int par[mxn][LOG + 1];
int direct_par[mxn], height[mxn], node_cnt[mxn], out[mxn];
vector<vector<int>> g;
void dfs(int u) {
    node_cnt[u] = 1;
    for (auto &v : g[u]) {
...
void dfs2(int u) {
    for (auto &v : g[u]) {
        if (v != direct_par[u]) {
            out[v] = node_cnt[u] - node_cnt[v] + out[u];
            dfs2(v);
        }
    }
}
...
void solve() {
    ...
    for (int i = 0; i < m; i++) {
        cin >> xi >> yi;
        LCA = lca(xi, yi);
        dist = height[xi] + height[yi] - 2*height[LCA]; 
        if (dist & 1) ans.push_back(0);
        else if (xi == yi) ans.push_back(n);
        else {
            if (height[xi] == height[yi]) {
                int u1 = jump(xi, dist/2 - 1);
                int u2 = jump(yi, dist/2 - 1);
                ans.push_back(node_cnt[LCA] - node_cnt[u1] - node_cnt[u2] + out[LCA]);
            } else {
                int u = height[xi] > height[yi] ? xi : yi;
                int center = jump(u, dist/2);
                int v = jump(u, dist/2 - 1);
                ans.push_back(node_cnt[center] - node_cnt[v]);
            }
        } 
    }
    for (auto x : ans) {
