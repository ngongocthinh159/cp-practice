 ...
 * Solution for: https://github.com/google/coding-competitions-archive/blob/main/kickstart/2022/round_a/palindrome_free_strings/statement.pdf
*/
...
string s;
int n;
vector<vector<string>> v(5, vector<string>());
void pre_compute() {
...
void solve() {
    cin >> n >> s;
    vector<vector<map<string, bool>>> dp(n, vector<map<string, bool>>(2, map<string, bool>()));
    ...
    int len, e;
    string tmp1, tmp2, tmp3, lasti_1;
    for (int i = 1; i < n; i++) {
        for (int j = 0; j < 2; j++) {
            len = min(5, i);
            for (auto &lastChars : v[len - 1]) {
                if (s[i] != '?' && (((int) (s[i] - '0')) ^ j)) { // 0,1 and 1,0
                    dp[i][j][lastChars] = false;
                    continue;
                }
                
                tmp1 = lastChars + (j == 0 ? '0' : '1');
                if (tmp1.size() == 6 && pld(tmp1)) {
                    dp[i][j][lastChars] = false;
                    continue;
                }
                tmp2 = lastChars.substr(1) + (j == 0 ? '0' : '1');
                if (tmp2.size() == 5 && pld(tmp2)) {
                    dp[i][j][lastChars] = false;
                    continue;
                }

                e = lastChars.back() == '0' ? 0 : 1;
                lasti_1 = lastChars.substr(0, lastChars.size() - 1);
                if (i - 1 >= 5) {
                    dp[i][j][lastChars] = dp[i - 1][e]["0" + lasti_1] || dp[i - 1][e]["1" + lasti_1];
                } else {
                    dp[i][j][lastChars] = dp[i - 1][e][lasti_1];
                }
            }
        }
    }
