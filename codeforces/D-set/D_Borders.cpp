 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/2/5/practice/contest/269656/problem/D
*/
...
struct SuffixArray {
    ...
    int query_lcp(int i, int j) { // query largest common prefix of 2 suffixes starting at index i and j
        assert(i < n);
        assert(j < n);
...
void solve() {
    ...
    for (int i = 1; i < n; i++) {
        int cur_lcp = lcp[i - 1];
        while (st.size() && lcp[st.back().first] > cur_lcp) st.pop_back();
        ll tot;
        if (st.size()) {
            tot = st.back().second + 1LL*cur_lcp*(i - 1 - (st.back().first + 1) + 1);
        } else {
            tot = 1LL*cur_lcp*i;
        }
        ans += tot;
        st.push_back({i - 1, tot});
    }
    ans += 1LL*(n + 1)*n/2;
    cout << ans << nline;
}
