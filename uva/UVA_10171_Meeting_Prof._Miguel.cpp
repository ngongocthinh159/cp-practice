 ...
 * Solution for: https://vjudge.net/problem/UVA-10171
*/
...
vector<ll> dijkstra(int s, vector<vector<pair<int,ll>>> &g) {
    vector<ll> dist = vector<ll>(m + 1, LINF);
...
void solve() {
    ...
    id = 1;
    char under30, bidir;
    string uu, vv, srcc, dstt;
    map<string,int> nameToId;
    int w;
    int u, v;
    bool isUnder30, isBiDir;
    for (int i = 0; i < n; i++) {
        cin >> under30 >> bidir >> uu >> vv >> w;
        if (under30 == 'Y') isUnder30 = true;
        else isUnder30 = false;
        if (bidir == 'U') isBiDir = false;
        else isBiDir = true;

        if (nameToId.count(uu)) u = nameToId[uu];
        else {
            nameToId[uu] = id++;
            u = nameToId[uu];
        }
        if (nameToId.count(vv)) v = nameToId[vv];
        else {
            nameToId[vv] = id++;
            v = nameToId[vv];
        }
        ...
        if (isUnder30) {
            g1[u].push_back({v, w});
            if (isBiDir) g1[v].push_back({u, w});
        } 
        else {
            g2[u].push_back({v, w});
            if (isBiDir) g2[v].push_back({u, w});
        }
    }
    cin >> srcc >> dstt;
    if (nameToId.count(srcc)) src = nameToId[srcc];
    else {
        nameToId[srcc] = id++;
        src = nameToId[srcc];
    }
    if (nameToId.count(dstt)) dst = nameToId[dstt];
    else {
        nameToId[dstt] = id++;
        dst = nameToId[dstt];
    }
    ...
    auto dist1 = dijkstra(src, g1);
    auto dist2 = dijkstra(dst, g2);
    ll ans = LINF;
    // cout << src << " " << dst << nline;
    // cout << dist1[2] << nline;
    // cout << dist2[2] << nline;
    ...
    for (int i = 1; i <= m; i++) {
        if (dist1[i] != LINF && dist2[i] != LINF) {
            if (ans > dist1[i] + dist2[i]) {
                ...
                ver.clear();
                ver.push_back(i);
            } else if (ans == dist1[i] + dist2[i]) {
                ver.push_back(i);
            }
        }
    }
    if (ans == LINF) cout << "You will never meet." << nline;
    else {
        ...
        cout << ans << " ";
        vector<string> names;
        for (auto vertice : ver) names.push_back(idToName[vertice]);
        sort(all(names));
        for (int i = 0; i < sz(names); i++) {
            cout << names[i];
            if (i != sz(names) - 1) cout << " ";
        }
        cout << nline;
    } 
}
