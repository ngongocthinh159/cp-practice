 ...
 * Solution for: https://codeforces.com/problemset/problem/1781/C
*/
...
void fill(string &t, int f) {
    ...
    for (int i = 0; i < 26; i++) if (freq[i] != f) pq.push({freq[i], i});
    int i = 0;
    while (pq.size() && empty > 0) {
...
void solve() {
    ...
    for (int unique = 1; unique <= 26; unique++) {
        if (n%unique == 0) {
            vector<pair<int,int>> match(26);
            int c_freq = n/unique;
            int tot_match = 0;
            for (int i = 0; i < 26; i++) match[i] = {min(f[i], c_freq), i};
            sort(all(match), compare);
            vector<int> get_freq(26);
            for (int i = 0; i < unique; i++) get_freq[match[i].second] = match[i].first, tot_match += match[i].first;
            if (ans > n - tot_match) {
                ans = n - tot_match;
                string t = s;
                for (int i = 0; i < n; i++) {
                    if (get_freq[t[i] - 'a']) get_freq[t[i] - 'a']--;
                    else t[i] = EMPTY;
                }
                fill(t, c_freq);
                res = t;
