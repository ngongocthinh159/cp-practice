 ...
 * Solution for: https://codeforces.com/problemset/problem/1765/N
*/
...
struct compare {
    bool operator() (pair<int,int> &p1, pair<int,int> &p2) {
        return p2.ff < p1.ff || (p2.ff == p1.ff && p2.ss < p1.ss);
    }
};
string x;
int k;
void solve() {
    ...
    for (int digit = 1; digit <= size; digit++) {
        for (; i <= n - (size - digit) - 1; i++) {
            if (digit == 1 && x[i] == '0') {v.push_back({0, i}); continue;}
            q.push({x[i] - '0', i}); 
        }
        while (q.size() && q.top().second <= last) q.pop();
        res += q.top().first + '0';
        last = q.top().second;
        q.pop();
        if (digit == 1) {
