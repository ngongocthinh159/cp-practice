 ...
 * Solution for: https://lightoj.com/problem/road-construction
*/
...
ll prim(vector<vector<pair<int,ll>>> &g) {
    ...
    return selectedEdges.size() == n - 1 ? minMST : -1;
}
void solve() {
    cin >> m;
    ...
    for (int i = 0; i < m; i++) {
        ...
        cin >> s >> t >> w;
        int u,v;
        if (nameToId.count(s)) u = nameToId[s];
        else u = nameToId[s] = id, idToName[id] = s, id++;
        if (nameToId.count(t)) v = nameToId[t];
        else v = nameToId[t] = id, idToName[id] = t, id++;
    ...
    ll weight = prim(g);
    if (weight == -1) cout << "Impossible" << nline;
    else cout << weight << nline;
}
