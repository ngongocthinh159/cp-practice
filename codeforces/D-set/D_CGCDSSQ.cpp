 ...
 * Solution for: https://codeforces.com/contest/475/problem/D
*/
...
void solve() {
    ...
    for (int k = 0; k < n; k++) {
        cin >> x;
        for (auto &entry : st) entry.g = gcd(entry.g, x);
        st.push_back({x, k, k});
        int i = 0, j = -1;
        while (i < st.size()) {
            int start_gcd = st[i].g;
            st[++j] = st[i];
            while (i + 1 < st.size() && st[i + 1].g == start_gcd) {
                i++;
                st[j].r = st[i].r;
            }
            i++;
        ...
        st.resize(j + 1);
        for (auto &entry : st) {
            freq[entry.g] += (entry.r - entry.l + 1);
        }
    }
    int q; cin >> q;
    for (int i = 0; i < q; i++) {
