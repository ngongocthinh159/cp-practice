...
/* clang-format on */
void solve();
int maxPerimeter(vector<int>& heights);
...
void solve() {
    ...
    for (int start = n - 1; start >= 0; start--) {
        f(j,0,m) {
            int cnt = 0;
            for (int i = start; i >= 0 && nums[i][j] == 0; i--) {
                cnt++;
            }
            heights[j] = cnt;
        }
        ans = max(ans, maxPerimeter(heights));
    }
    cout << ans << "\n";
}

int firstSmallerLeft[25];
int firstSmallerRight[25];
stack<int> s;
int maxPerimeter(vector<int>& nums) {
    ...
    while (s.size()) {firstSmallerRight[s.top()] = nums.size(); s.pop();}
    f(i,0,nums.size()) {
        while (s.size() > 0 && nums[i] <= nums[s.top()]) {
            s.pop();
        }
        if (s.size()) firstSmallerLeft[i] = s.top();
        else firstSmallerLeft[i] = -1;
        s.push(i);
    }
    while (s.size()) s.pop();
    int ans = 0;
    f(i,0,nums.size()) {
        if (nums[i] > 0) ans = max(ans, 2*nums[i] + 2*(firstSmallerRight[i] - firstSmallerLeft[i] - 1));
    }
    return ans;
}
