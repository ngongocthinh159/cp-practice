 ...
 * Solution for: https://codeforces.com/problemset/problem/87/C
*/
...
void solve() {
    ...
    for (int i = 1; i <= n; i++) {
        ...
        while (last[g[i]] == i) ++g[i];

        int xr = 0;
        for (int j = i, sum = i; sum <= n && j >= 1; j--, sum += j) {
            xr ^= g[j];
            if (i - j + 1 >= 2) {
                adj[sum].push_back(xr);
            }
            if (i - j + 1 >= 2 && !xr && sum == n) {
                ans = min(ans, i - j + 1);
            } 
