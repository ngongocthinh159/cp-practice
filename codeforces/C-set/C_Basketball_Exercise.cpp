 ...
 * Solution for: https://codeforces.com/contest/1195/problem/C
*/
...
const int mxN = 1e5 + 5;
int n;
vector<vector<ll>> a(mxN, vector<ll>(2)   );
void solve() {
    cin >> n;
    for (int i = 0; i < n; i++) {
        cin >> a[i][0];
    ...
    for (int i = 1; i <= n; i++) {
        for (int state = 0; state < 3; state++) {
            if (state == 0) dp[i][0] = max(dp[i - 1][1], dp[i - 1][2]) + a[i - 1][0];
            if (state == 1) dp[i][1] = max(dp[i - 1][0], dp[i - 1][2]) + a[i - 1][1];
            if (state == 2) dp[i][2] = max(dp[i - 1][0], max(dp[i - 1][1], dp[i - 1][2]));
        }
    }
    cout << max(dp[n][0], max(dp[n][1], dp[n][2])) << nline;
}
