 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/7/2/practice/contest/289391/problem/F
*/
...
bool connected(int l, int r) {
    DSU dsu(n);
    int compo = n;
    for (int i = l; i <= r; i++) if (dsu.unite(edges[i].u, edges[i].v)) {
        compo--;
...
void solve() {
    ...
    for (int l = 0, r = 0; r < m; r++) {
        while (l <= r && connected(l, r)) {
            found = true;
            l++;
        }
        if (found) {
            ans = min(ans, edges[r].w - edges[l - 1].w);
        }
    }
    if (ans == INT_MAX) cout << "NO" << nline;
    else {
        cout << "YES" << nline;
        cout << ans << nline;
    }
