 ...
 * Solution for: https://vjudge.net/problem/UVA-10397/origin
*/
...
void dfs(int u, vector<int> &compo, vector<vector<int>> &g, vector<bool> &visited) {
    visited[u] = true;
    compo.push_back(u);
    for (auto v : g[u]) {
        if (!visited[v]) {
            dfs(v, compo, g, visited);
        }
    }
}
double dist(pair<int,int>&p1, pair<int,int>&p2) {
    ll tmp = 1LL*abs(p1.first - p2.first)*abs(p1.first - p2.first) + 1LL*abs(p1.second - p2.second)*abs(p1.second - p2.second);
    return sqrt(tmp);
}
double minDist(int u, int v, vector<vector<int>> &compos) {
    double mn = IINF;
    for (int i = 0; i < compos[u].size(); i++) {
        for (int j = 0; j < compos[v].size(); j++) {
            mn = min(mn, dist(points[compos[u][i]], points[compos[v][j]]));
        }
    }
    return mn;
}
void solve() {
    ...
    for (int i = 1; i <= n; i++) {
        if (!visited[i]) {
            vector<int> compo;
            dfs(i, compo, g1, visited);
            compos.push_back(compo);
        }
    }
    vector<bool> vis(compos.size());
    set<pair<double,int>> S;
    vector<pair<double,int>> min_e(compos.size(), {IINF,-1});
    S.insert({0,0});
    min_e[0] = {0,-1};
    double weight = 0;
    while (S.size()) {
        ...
        S.erase(S.begin());
        int u = p.second;
        double w_paru_u = p.first;
        int paru = min_e[u].second;

        weight += w_paru_u;
        vis[u] = true;

        for (int v = 0; v < compos.size(); v++) {
            if (!vis[v]) {
                double mn_edge = minDist(u, v, compos);
                if (min_e[v].first > mn_edge) {
