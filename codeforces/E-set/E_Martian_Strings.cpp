 ...
 * Solution for: https://codeforces.com/contest/149/problem/E
*/
...
string s;
int m;
int pref[MAX], surf[MAX], rmq[LOG + 1][MAXN], rrmq[LOG + 1][MAXN];
void pre_compute() {

}
int query(int l, int r, int rmq[][MAXN]) {
    int bit = 32 - __builtin_clz(r - l + 1) - 1;
    return min(rmq[bit][l], rmq[bit][r - MASK(bit) + 1]);
}
int compare(char c, int idx, int suffix_idx, string &s) {
    if (idx + suffix_idx >= SZ(s)) return 1;
    if (c > s[idx + suffix_idx]) return 1;
    if (c < s[idx + suffix_idx]) return -1;
    return 0;
}
pair<int,int> search(SuffixArray &sa, char c, int idx, int lo, int hi, string &s) {
    const auto& p = sa.get_p_array();
    int rL = -1, rR = -1;
    {  
        int l = lo - 1, r = hi + 1;
        while (r - l > 1) {
            int m = l + (r - l)/2;
            int res = compare(c, idx, p[m], s);
            if (res <= 0)
                r = m;
            else
                l = m;
...
void calc(SuffixArray &sa, int *pref, string &t, string &s, int rmq[][MAXN]) {
    int prevL = 0, prevR = SZ(s) - 1;
    for (int i = 0; i < SZ(t); i++) pref[i] = -1;
    for (int i = 0; i < SZ(t); i++) {
        auto [l, r] = search(sa, t[i], i, prevL, prevR, s);
        if (l != -1) {
            pref[i] = query(l, r, rmq);
            prevL = l;
            prevR = r;
        } else
            break;
...
void solve() {
    ...
    reverse(ALL(s));
    const auto& p = sa.get_p_array();
    const auto& rp = rsa.get_p_array();

    // rmq
    for (int i = 0; i < n; i++) rmq[0][i] = p[i], rrmq[0][i] = rp[i];
    for (int j = 1; j <= LOG; j++)
        for (int i = 0; i + MASK(j) - 1 < n; i++) 
            rmq[j][i] = min(rmq[j - 1][i], rmq[j - 1][i + MASK(j - 1)]),
            rrmq[j][i] = min(rrmq[j - 1][i], rrmq[j - 1][i + MASK(j - 1)]);

    int ans = 0;
    string t;
    for (int i = 0; i < m; i++) {
        cin >> t;
        calc(sa, pref, t, s, rmq);
        reverse(ALL(t));
        calc(rsa, surf, t, rs, rrmq);
        reverse(ALL(t));

        // for (int j = 0; j < SZ(t); j++) cout << pref[j] << " ";
        // cout << nline;
        // for (int j = 0; j < SZ(t); j++) cout << surf[j] << " ";
        // cout << nline;

        for (int j = 0; j < SZ(t) - 1; j++) {
            int idx1 = j, idx2 = SZ(t) - (idx1 + 1) - 1;
            if (pref[idx1] == -1 || surf[idx2] == -1) continue;

            if (pref[idx1] + j < n - (surf[idx2] + (SZ(t) - j - 1) - 1) - 1) {
                ans++;
                break;
            }
        }
