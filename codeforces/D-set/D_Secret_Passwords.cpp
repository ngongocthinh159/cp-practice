 ...
 * Solution for: https://codeforces.com/contest/1263/problem/D
*/
...
struct DSU {
    ...
    int find_par(char c) {
        if (par[c - 'a'] == c) return c;
        return par[c - 'a'] = find_par(par[c - 'a']);
    }
    void unite(char a, char b) {
        int par_a = find_par(a);
        int par_b = find_par(b);
        if (par_a != par_b) {
            if (size[par_a - 'a'] < size[par_b - 'a']) swap(par_a, par_b);
            size[par_a - 'a'] += size[par_b - 'a'];
            par[par_b - 'a'] = par_a;
        }
    }
    int cnt_set() {
...
void solve() {
    ...
    for (int i = 0; i < n;i++) {
        ...
        for (int j = 0; j < sz(s); j++) {
            // cout << "create_set" << j << endl;
            dsu.create_set(s[j]);
            if (j >= 1) {
                // cout << "unite" << j << " " << j - 1 << endl;
                dsu.unite(s[j], s[j - 1]);
            }
        }
    }
    cout << dsu.cnt_set() << nline;
}
