 ...
 * Solution for: https://codeforces.com/problemset/problem/598/E
*/
...
ll dfs(int row, int col, int k) {
    ...
    for (int i = 1; i <= row - 1; i++)
        for (int _k = 0; _k <= k; _k++) 
            res = min(res, dfs(i, col, _k) + dfs(row - i, col, k - _k) + col*col);
    for (int j = 1; j <= col - 1; j++)
        for (int _k = 0; _k <= k; _k++)
            res = min(res, dfs(row, j, _k) + dfs(row, col - j, k - _k) + row*row);
