 ...
 * Solution for: https://www.codechef.com/practice/course/dynamic-programming/INTDP01/problems/MAXCOMP
*/
...
void solve() {
    cin >> n;
    int MX = 49;
    vector<vector<vector<int>>> dp(MX + 1,
        vector<vector<int>>(MX + 1,
            vector<int>(MX + 1, 0)));
    for (int i = 1; i <= MX; i++) dp[0][i][i] = 0;
    for (int i = 1; i <= n; i++) {
        int u, v, w; cin >> u >> v >> w;
        u++, v++;
        dp[0][u][v] = max(dp[0][u][v], w);
    }
    for (int k = 1; k <= MX; k++)
        for (int i = 1; i <= MX; i++)
            for (int j = i; j <= MX; j++) {
                dp[k][i][j] = dp[k - 1][i][j];
                if (i <= k && k <= j) {
                    dp[k][i][j] = max(dp[k][i][j], dp[k - 1][i][k] + dp[k - 1][k][j]);
                }
            }
    int ans = -1;
    for (int i = 1; i <= MX; i++) {
