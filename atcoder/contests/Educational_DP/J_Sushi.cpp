 ...
 * Solution for: https://atcoder.jp/contests/dp/tasks/dp_j
*/
...
const int mxn = 3e2 + 5;
double dp[mxn][mxn][mxn]; // dp[i][j][k]: expected operation with number of dishes have 1, 2 and 3 food are i, j and k respectively => Number of dishes with no food is: m = n - (i + j + k)
int n, val;
void pre_compute() {
    ...
    for (int k = 0; k <= mx; k++) {
        for (int j = 0; j <= mx; j++) {
            for (int i = 0; i <= mx; i++) {
                ...
                m = n - (i + j + k);
                if (i >= 1) dp[i][j][k] += 1.00*i/(i + j + k)*(dp[i - 1][j][k] + 1);
                if (j >= 1) dp[i][j][k] += 1.00*j/(i + j + k)*(dp[i + 1][j - 1][k] + 1);
                if (k >= 1) dp[i][j][k] += 1.00*k/(i + j + k)*(dp[i][j + 1][k - 1] + 1);
                if (m >= 1) dp[i][j][k] += 1.00*m/(i + j + k);
            }
        }
    }
