 ...
 * Solution for: https://codeforces.com/contest/1944/problem/B
*/
...
void solve() {
    ...
    for (int i = 0; i < n && l.size()+2 <= len; i++) {
    ...
    int i = 0;
    while (l.size() < len && i < n) {
        if (S1.count(a[i])) {i++; continue;}
        if (M1[a[i]]==1) {
            l.push_back(a[i]);
            r.push_back(a[i]);
            S1.insert(a[i]);
    ...
    for (int i = n; i < 2*n && r.size()+2 <= len; i++) {
