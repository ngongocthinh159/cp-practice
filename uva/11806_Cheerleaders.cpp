 ...
 * Solution for: https://onlinejudge.org/index.php?option=com_onlinejudge&Itemid=8&page=show_problem&problem=2906
*/
...
void solve() {
    ...
    ans = ((ans - 2*nCk(tot - n, k) % MOD) + MOD) % MOD;
    ans = ((ans - 2*nCk(tot - m, k) % MOD) + MOD) % MOD;

    ans = (ans + 4*nCk(tot - (n + m - 1), k) % MOD) % MOD;
    ans = (ans + nCk(tot - 2*n, k)) % MOD; 
    ans = (ans + nCk(tot - 2*m, k)) % MOD;

    ans = ((ans - 2*nCk(tot - (2*n + m - 2), k) % MOD) + MOD) % MOD;
    ans = ((ans - 2*nCk(tot - (2*m + n - 2), k) % MOD) + MOD) % MOD;

    ans = (ans + nCk(tot - (2*n + 2*m - 4), k)) % MOD;
