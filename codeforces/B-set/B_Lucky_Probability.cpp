 ...
 * Solution for: https://codeforces.com/contest/109/problem/B
*/
...
void pre_compute() {
    dfs(0, 0);
    sort(lucky, lucky + top);
}
ll inter(ll l1, ll r1, ll l2, ll r2) {
    if (l1 > r2 || r1 < l2) return 0;
    return min(r2, r1) - max(l1, l2) + 1;
}
pair<ll,ll> inter2(ll l1, ll r1, ll l2, ll r2) {
    if (l1 > r2 || r1 < l2) return {-1, -1};
    return {max(l1, l2), min(r2, r1)};
}
ll cal(ll a, ll b, ll c, ll d) {
    auto it = lower_bound(lucky, lucky + top, a);
    int l = it - lucky;
    ll res = 0, s = a;
    while (l < top && l + k - 1 < top && s <= b) {
        int r = l + k - 1;
        ll upper = r + 1 < top ? lucky[r + 1] - 1 : LINF;
        res += min(lucky[l] - s + 1, b - s + 1) * inter(lucky[r], upper, c, d);
        s = lucky[l] + 1;
        l++;
    }
    return res;
...
void solve() {
    ...
    ll cnt2 = cal(c, d, a, b);
    ll cnt3 = 0;
    auto p = inter2(a, b, c, d);
    if (p.fi != -1 && k == 1) {
        auto it = lower_bound(lucky, lucky + top, p.fi);
        auto it2 = upper_bound(lucky, lucky + top, p.se);
        if (it != it2) {
            cnt3 = it2 - it;
        }
    } 
    long double ans = 1.00*(cnt1 + cnt2 - cnt3)/(b - a + 1)/(d - c + 1);
    printf("%.12Lf\n", ans);
}
