...
struct SuffixArray {
    ...
    SuffixArray(vector<int> &str, bool query_lcp_ij = false) {
        s = str;
        n =  s.size();
    ...
    vector<int> lcp_construction(vector<int> &s, vector<int> const& p) {
...
void solve() {
    cin >> m >> k;
    string t = "";
    vector<int> s, aux_len, len_ss;
    int offset = 1 + (m - 1);
    FOR(i,0,m-1) {
        cin >> t;
        if (i != 0) s.push_back(i);
        for (auto c : t) {
            s.push_back(c - 'a' + offset);
            alphabet = max(alphabet, c - 'a' + offset + 5);
        }
        aux_len.push_back(SZ(s) - 1);
        len_ss.push_back(SZ(t));
    }
    if (k == 1) {
        REP(i,m) cout << 1LL*len_ss[i]*(len_ss[i] + 1)/2 << " ";
        cout << nline;
        return;
    }
    ...
    vector<int> ss_idx(n, -1);
    FOR(i,m-1,n-1) {
        int l = -1, r = aux_len.size();
        while (r - l > 1) {
            int m = l + (r - l)/2;
            if (aux_len[m] >= p[i])
                r = m;
    ...
    vector<int> st;
    vector<int> L(n - 1, -1), R(n - 1, lcp.size());
    vector<ll> dif(n + 1), ans(m), bestL(n, -1), freq(m);
    FOR(i,0,n-2) {
        while (st.size() && lcp[st.back()] > lcp[i]) st.pop_back();
        if (st.size())
            L[i] = st.back();
        st.push_back(i);
    }
    ...
    }
    {
        ...
        FOR(r,0,n-1) {
            ...
            if (++freq[ss_idx[r]] == 1) unique++;

            while (l <= r && unique >= k) {
                found = true;
                if (--freq[ss_idx[l]] == 0) unique--;
                l++;
            }

            if (found) bestL[r] = l - 1;
        } 
    }
    
    FOR(i,0,n-2) {
        if (ss_idx[i] == -1) continue;
        if (L[i] != -1 && lcp[L[i]] == lcp[i]) continue;

        int counted_len = 0;
        if (L[i] >= 0) counted_len = max(counted_len, lcp[L[i]]);
        if (R[i] < lcp.size()) counted_len = max(counted_len, lcp[R[i]]);
        
        int start = L[i] + 1;
        int end = R[i];
        int len = end - start + 1;

        ll cur_len = lcp[i] - counted_len;
    ...
    FOR(i,1,n-1) dif[i] += dif[i - 1];
    FOR(i,0,n-1) {
        if (ss_idx[i] != -1) ans[ss_idx[i]] += dif[i];
    }
