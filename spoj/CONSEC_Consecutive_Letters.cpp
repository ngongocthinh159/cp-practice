 ...
 * Solution for: https://www.spoj.com/problems/CONSEC/
*/
...
void solve() {
    ...
    DSU dsu(n);
    for (int i = 1; i < n; i++) {
        if (s[i] == s[i - 1] && s[i] != '#') dsu.unite(i, i - 1);
    }
    vector<int> ans;
    for (int i = 0; i < q; i++) {
        if (queries[i].type == 1) {
            ans.push_back(dsu.find_size(queries[i].idx));
        } else {
            int idx = queries[i].idx;
            s[idx] = queries[i].c;
            if (idx - 1 >= 0 && s[idx - 1] == s[idx]) dsu.unite(idx, idx - 1);
            if (idx + 1 < n && s[idx + 1] == s[idx]) dsu.unite(idx, idx + 1);
        }
    }
    reverse(ALL(ans));
