 ...
 * Solution for: https://codeforces.com/problemset/problem/453/A
*/
...
double expo_double(double a, int b) {
    if (b==0) return 1.00;
    if (b==1) return a;
    double tmp = expo_double(a,b/2);
    if (b&1) {
        return 1.00*tmp*tmp*a;
    } else return 1.00*tmp*tmp;
}
...
void solve() {
    ...
    double ans = m;
    for (int i = 1; i <= m - 1; i++) {
        ans -= expo_double((double)1.00*i/m,n);
    }
    printf("%.8f\n", ans);
}
