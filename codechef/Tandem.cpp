 ...
 * Solution for: https://www.codechef.com/problems/TANDEM
*/
...
void solve() {
    ...
    reverse(ALL(s));
    SuffixArray rsa(s, true);
    ll interest = 0, boring = 0;
    for (int len = 1; 3*len <= n; len += 1) {
        for (int i = 0; i + len - 1 < n ; i += len) {
            int st = i, e = st + len - 1;
            int mxr = 0, mxl = 0;
            if (e + 1 < n) mxr = sa.query_lcp_suffix(st, e + 1);
            if (n - e - 1 + len < n) mxl = rsa.query_lcp_suffix(n - e - 1, n - e - 1 + len);
            int ss_len = 3*len;
            int extend_len = len + mxr + mxl;
            if (extend_len >= ss_len) {
                boring += (extend_len - ss_len);
                interest += 1;
                i = i + mxr;
                i = (i/len)*len;
            }
