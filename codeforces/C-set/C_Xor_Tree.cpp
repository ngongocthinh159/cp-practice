 ...
 * Solution for: https://codeforces.com/contest/1446/problem/C
*/
...
int findAns(vector<int> &v, int bit) {
    if (bit < 0) return 0;

    vector<int> v1;
    vector<int> v2;
    for (auto x : v) {
        if ((x >> bit) & 1) v1.push_back(x);
        else v2.push_back(x);
    }
    if (max(sz(v1), sz(v2)) <= 1) return 0;
    else if (sz(v1) <= 1) return findAns(v2, bit - 1);
    else if (sz(v2) <= 1) return findAns(v1, bit - 1);
    return min(findAns(v1, bit - 1) + sz(v2) - 1, findAns(v2, bit - 1) + sz(v1) - 1);
