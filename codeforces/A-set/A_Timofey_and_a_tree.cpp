 ...
 * Solution for: https://codeforces.com/contest/763/problem/A
*/
...
void solve() {
    ...
    vector<vector<int>> v;
    for (auto e : edges) {
        if (color[e[0]] != color[e[1]]) v.push_back({e[0], e[1]});
    }
    if (v.size() == 0) {
        cout << "YES" << nline;
        cout << 1 << nline;
        ...
        int common = - 1;
        if (v[0][0] == v[1][0]) common = v[0][0];
        else if (v[0][0] == v[1][1]) common = v[0][0];
        else if (v[0][1] == v[1][0]) common = v[0][1];
        else if (v[0][1] == v[1][1]) common = v[0][1];
        if (common == -1) {
            cout << "NO" << nline;
        } else {
            bool ok = true;
            for (int i = 2; i < v.size(); i++) {
                if (v[i][0] != common && v[i][1] != common) {
                    ok = false;
                    break;
                }
            }
