 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/4/1/practice/contest/273169/problem/C
*/
...
ndata make_ndata(int val, int cnt) {
    ndata res;
    res.val = val;
    res.cnt = cnt;
    return res;
...
ndata combine(ndata l, ndata r) {
    if (l.val == r.val) return make_ndata(l.val, l.cnt + r.cnt);
    if (l.val < r.val) return make_ndata(l.val, l.cnt);
    else return make_ndata(r.val, r.cnt);
}
void build(int idx, int s, int e) {
    if (s == e) {
        segtree[idx] = make_ndata(a[s], 1);
        return;
    }
    int m = (s + e)/2;
    build(2*idx, s, m);
...
ndata query(int idx, int s, int e, int l, int r) {
    if (e < l || s > r) return make_ndata(IINF, -1);
