 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/5/4/practice/contest/280801/problem/D
*/
...
Lazy lazy[4*MAX];
ll a[MAX];
Data make_data(ll sum, int cnt, ll org_sum) {
    Data res;
    res.sum = sum;
    res.cnt = cnt;
    res.org_sum = org_sum;
...
Data merge(Data &l, Data &r) {
    Data res;
    res.sum = l.sum + r.sum + r.org_sum*l.cnt;
    res.cnt = l.cnt + r.cnt;
    res.org_sum = l.org_sum + r.org_sum;
    return res;
}
void build(int idx, int s, int e) {
    if (s == e) {
        segtree[idx] = make_data(a[s], 1, a[s]);
        return;
    }
    int m = (s + e)/2;
    build(2*idx, s, m);
...
void apply(int idx, int s, int e, const Lazy &operation) {
    if (operation.type == ADD) {
        segtree[idx].sum += operation.val*(e - s + 1 + 1)*(e - s + 1)/2;
        segtree[idx].org_sum += operation.val*(e - s + 1);

        lazy[idx].type = ADD;
        lazy[idx].val += operation.val;
    }
    // 1*a1 + 2*a2 + 3*a3 ...   , l = 3 => 4*a1 + 5*a2 + 6*a3
};
