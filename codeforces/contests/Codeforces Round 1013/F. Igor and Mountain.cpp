 ...
 * Solution for: https://codeforces.com/contest/2091/problem/F
*/
...
#define MAXN 2005
int n, m, d;
ll dp[2][2][MAXN], pref[2][2][MAXN];
char a[MAXN][MAXN];

void pre_compute() {

}
pair<int,int> f(int j, int x) {
    int dist = sqrt(x);
    return {max(0, j - dist), min(m - 1, j + dist)};
}
void solve() {
    cin >> n >> m >> d;
    for (int i = 0; i < n; i++)
        ...
        for (int k = 1; k >= 0; k--)
            for (int j = 0; j < m; j++) {
                ...
                if (a[i][j] == 'X') {
                    ...
                    auto [l, r] = f(j, d*d - 1);
                    ll expand_up = (pref[iii][0][r + 1] - pref[iii][0][l]) % MOD1;
                    if (expand_up < 0) expand_up += MOD1;
                    dp[ii][k][j] = (dp[ii][k][j] + expand_up) % MOD1;

                    if (k == 0) {
                        auto [l, r] = f(j, d*d);
                        ll expand_row = (pref[ii][!k][r + 1] - pref[ii][!k][l] - dp[ii][!k][j]) % MOD1;
                        if (expand_row < 0) expand_row += MOD1;
                        dp[ii][k][j] = (dp[ii][k][j] + expand_row) % MOD1;
                    }
                }

                pref[ii][k][j + 1] = (pref[ii][k][j] + dp[ii][k][j]) % MOD1;
            }
    ll ans = 0;
    for (int j = 0; j < m; j++) ans = (ans + dp[(n - 1)%2][0][j]) % MOD1;
    cout << ans << nline;

    for (int i = 0; i < 2; i++)
        for (int k = 1; k >= 0; k--)
            for (int j = 0; j <= m; j++) dp[i][k][j] = pref[i][k][j] = 0;
}
