 ...
 * Solution for: https://codeforces.com/problemset/problem/842/D
*/
...
struct Node {
    ...
    int sub_cnt = 0;
    Node() {
        child.assign(MAX_CHILD, nullptr);
    }
};
void pre_compute() {
...
int n, m;
void insert(Node* root, int x, int bit = MAX_BIT) {
    if (bit == -1) {
        root->cnt = 1;
        root->sub_cnt = 1;
        return;
    ...
    int idx = (x >> bit)&1;
    if (!root->child[idx]) root->child[idx] = new Node;
    insert(root->child[idx], x, bit - 1);
    root->sub_cnt = 0;
    for (int i = 0; i < MAX_CHILD; i++) if (root->child[i]) 
        root->sub_cnt += root->child[i]->sub_cnt;
}
int find_ans(Node* root, int x) {
    auto p = root;
    int res = 0;
    for (int bit = MAX_BIT; bit >= 0; bit--) {
        ...
        if (p->child[idx] && p->child[idx]->sub_cnt == (1 << (bit))) {
            res |= (1 << bit);
            p = p->child[idx^1];
        } else {
            p = p->child[idx];
