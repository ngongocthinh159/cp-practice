 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/5/3/practice/contest/280799/problem/B
*/
...
void apply(int idx, int s, int e, ll val) {
    segtree[idx].val = val&1 ? ((e - s + 1) - segtree[idx].val) : segtree[idx].val; // apply update
...
Data query(int idx, int s, int e, int pos) {
    if (segtree[idx].val < pos) return make_data(-1);
    if (s == e) return make_data(s);
    push_down(idx, s, e);
    int m = (s + e)/2;
    Data l_ans = query(2*idx, s, m, pos);
    if (l_ans.val != -1) return l_ans;
    return query(2*idx + 1, m + 1, e, pos - segtree[2*idx].val);
