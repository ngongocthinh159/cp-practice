/*
    Solution for: https://codeforces.com/edu/course/2/lesson/6/4/practice/contest/285069/problem/A
*/
...
bool check(double m) {
    ...
    for (int i = 0; i < n; i++) {
        pre[i] = i == 0 ? (double) c[i] : (double) c[i] + pre[i - 1];
    }
    pair<double,int> mn = {pre[0] - c[0], 0};
    for (int i = d - 1; i < n; i++) {
        int j = i - d + 1;
        if (mn.first > pre[j] - c[j]) {
            mn = {pre[j] - c[j], j};
        }
        if (pre[i] >= mn.first) {
            _l = mn.second; _r = i;
