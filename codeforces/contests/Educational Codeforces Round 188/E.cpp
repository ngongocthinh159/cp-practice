 ...
 * Solution for: https://codeforces.com/contest/2204/problem/E
*/
...
pair<vector<int>,int> factor(int  x) {
    vector<int> res;
    int sum = 0;
    while (x) {
        int d = x % 10;
...
bool check(int x, int *f, string &s, int tar) {
    if (x <= 9) {
        ...
        if (S == tar) {
            string tmp = "";
            for (int i = 9; i >= 0; i--) if (f[i])
                for (int j = 0; j < f[i]; j++) tmp += i + '0';
            u = tmp + s;
        } else {
            ok = false;
        }
    ...
    bool ok = true;
    auto [buf, sum] = factor(x);
    for (int i = 0; i < int(buf.size()); i++) {
        if (--f[buf[i]] < 0) ok = false;
    }
    ...
    if (ok) {
        ...
        res = check(sum, f, s, tar);

        S += sum;
        while (len--) s.pop_back();
    }

    for (int i = 0; i < int(buf.size()); i++) f[buf[i]]++;
    return res;
}
void solve() {
    string s; cin >> s;
    ...
    int f[10]{};
    S = 0;
    for (int i = 1; i <= n; i++) f[s[i] - '0']++, S += s[i] - '0';
    int mx = n * 9;
    for (int i = 1; i <= mx; i++) {
        string t = "";
        if (check(i, f, t, i)) {
