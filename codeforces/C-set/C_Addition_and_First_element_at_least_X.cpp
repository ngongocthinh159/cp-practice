 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/5/3/practice/contest/280799/problem/C
*/
...
ll query(int idx, int s, int e, int l, int r, ll x) {
    if (s > r || e < l) return -1;
    if (segtree[idx].mx < x) return -1;
    if (s == e) return segtree[idx].idx;
    push_down(idx, s, e);
    int m = (s + e)/2;
    ll l_ans = query(2*idx, s, m, l, r, x);
    if (l_ans != -1) return l_ans;
    return query(2*idx + 1, m + 1, e, l, r, x);
}
void pre_compute() {
