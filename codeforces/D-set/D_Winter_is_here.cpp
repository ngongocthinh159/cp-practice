 ...
 * Solution for: https://codeforces.com/problemset/problem/839/D
*/
...
#define MAXN 200005
#define MAXA 1000005
int n, a[MAXN], cnt[MAXA], freq[MAXA];
ll g[MAXA], _2pow[MAXA];
...
void solve() {
    ...
    for (int i = 1; i < MAXA; i++) 
        for (int j = i; j < MAXA; j += i) 
            cnt[i] += freq[j];
    _2pow[0] = 1;
    for (int i = 1; i < MAXA; i++) _2pow[i] = _2pow[i - 1] * 2 % MOD;
    for (int i = MAXA - 1; i >= 2; i--) {
        if (cnt[i]) 
            g[i] = cnt[i] * _2pow[cnt[i] - 1] % MOD;
        for (int j = 2*i; j < MAXA; j += i)
            g[i] = ((g[i] - g[j]) % MOD + MOD) % MOD;
    }
    ll ans = 0;
    for (int i = 2; i < MAXA; i++) ans = (ans + g[i] * i % MOD) % MOD;
    cout << ans << nline;
}
