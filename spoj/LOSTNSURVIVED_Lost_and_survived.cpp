 ...
 * Solution for: https://www.spoj.com/problems/LOSTNSURVIVED/
*/
...
struct DSU {
    ...
    pair<pair<int,int>,pair<int,int>> unite(int a, int b) {
        int ra = find(a);
        int rb = find(b);
        if (ra != rb) {
            if (size[ra] < size[rb]) swap(ra,rb);
            pair<pair<int,int>,pair<int,int>> res = {{size[ra], ra}, {size[rb], rb}};
            size[ra] += size[rb];
            parent[rb] = ra;
            return res;
        } else {
...
void solve() {
    ...
    for (int i = 0; i < q; i++) {
        int u, v; cin >> u >> v;
        if (!dsu.same_set(u, v)) {
            auto [pa, pb] = dsu.unite(u, v);
            S.erase(pa);
            S.erase(pb);
            pair<int,int> new_p = {pa.first + pb.first, pa.second};
            S.insert(new_p);
        }
        cout << ((*S.rbegin()).first - (*S.begin()).first) << nline;
    }
}
