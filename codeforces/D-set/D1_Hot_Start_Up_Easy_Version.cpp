...
void solve() {
    ...
    for (int j = 0; j <= k + 1; j++) {
        cur_dp[j] = LONG_LONG_MAX;
        next_dp[j] = LONG_LONG_MAX;
    }
    cur_dp[0] = cold[a[1]];
    int CPU1, CPU2;
    for (int i = 2; i <= n; i++) {
        for (int j = 0; j <= k; j++) {
            if (cur_dp[j] == LONG_LONG_MAX) continue;

            CPU1 = a[i - 1];
            CPU2 = j;
            
            // Put to CPU1
            next_dp[CPU2] = min(next_dp[CPU2], cur_dp[j] + (a[i] == CPU1 ? hot[a[i]] : cold[a[i]]));

            // Put to CPU2
            next_dp[CPU1] = min(next_dp[CPU1], cur_dp[j] + (a[i] == CPU2 ? hot[a[i]] : cold[a[i]]));
        }
        for (int j = 0; j <= k; j++) {
            cur_dp[j] = next_dp[j];
            next_dp[j] = LONG_LONG_MAX;
