 ...
 * Solution for: https://codeforces.com/contest/510/problem/C
*/
...
void solve() {
    ...
    for (int i = 0;i < n;i++) {
        ...
        if (i >= 1) {
            int j = 0, k = 0;
            while (j < sz(t) && k < sz(s) && t[j] == s[k]) {
                j++; k++;
            }
            if (k == sz(s) && sz(t) > sz(s)) {
                ok = false;
            } else if (j < sz(t)) {
                g[t[j] - 'a'].insert(s[k] - 'a');
                S.insert(s[k] - 'a');
                S.insert(t[j] - 'a');
            }
        }
