 ...
 * Solution for: https://www.spoj.com/problems/NUMOFPAL/
*/
...
string s;
ll merge(int l1, int r1, int l2, int r2, Manacher &mana) {
    ll ans = 0;
    for (int i = l1; i <= r2; i++) {
        int len_take = min(i - l1 + 1, min(r2 - i + 1, mana.get_p_at(i)));
        if (i <= r1) {
            ans += max(0, i + len_take - 1 - l2 + 1);
        } else {
            ans += max(0, r1 - (i - len_take + 1) + 1);
        }
    }
    for (int i = l1 + 1; i <= r2; i++) {
        int len_take = min(i - l1, min(r2 - i + 1, mana.get_p_at(i, false)));
        if (i <= r1) {
            ans += max(0, i + len_take - 1 - l2 + 1);
        } else if (i == l2) {
            ans += len_take;
        } else {
            ans += max(0, r1 - (i - len_take) + 1);
        }
    }
    return ans;
}
ll count(int l, int r, Manacher &mana) {
    if (l == r) return 1;
    int m = (l + r)/2;
    return count(l, m, mana) + count(m + 1, r, mana) + merge(l, m, m + 1, r, mana);
}
void solve() {
    cin >> s;
    int n = SZ(s);
    Manacher mana(s);
    cout << count(0, n - 1, mana) << nline;
}
