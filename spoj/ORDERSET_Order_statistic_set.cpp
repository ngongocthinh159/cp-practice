 ...
 * Solution for: https://www.spoj.com/problems/ORDERSET/
*/
...
void insert(int idx, int s, int e, int pos) {
    if (s == e) {
        segtree[idx] = make_data(1);
        return;
    }
    int m = (s + e)/2;
    if (pos <= m) insert(2*idx, s, m, pos);
    else insert(2*idx + 1, m + 1, e, pos);
    segtree[idx] = merge(segtree[2*idx], segtree[2*idx + 1]);
}
void del(int idx, int s, int e, int pos) {
    if (s == e) {
        segtree[idx] = make_data(0);
        return;
    }
    int m = (s + e)/2;
    if (pos <= m) del(2*idx, s, m, pos);
    else del(2*idx + 1, m + 1, e, pos);
    segtree[idx] = merge(segtree[2*idx], segtree[2*idx + 1]);
}
int kth_smallest(int idx, int s, int e, int val) {
    if (s == e) return s;
    int m = (s + e)/2;
    if (segtree[2*idx].sum >= val) return kth_smallest(2*idx, s, m, val);
    if (segtree[2*idx + 1].sum >= (val - segtree[2*idx].sum))
        return kth_smallest(2*idx + 1, m + 1, e, val - segtree[2*idx].sum);
    return -1;
}
int range_sum(int idx, int s, int e, int l, int r) {
    if (s > r || e < l) return 0;
    if (l <= s && e <= r) return segtree[idx].sum;
    int m = (s + e)/2;
    int l_ans = range_sum(2*idx, s, m, l, r);
    int r_ans = range_sum(2*idx  +1, m + 1, e, l, r);
    return l_ans + r_ans;
}
void pre_compute() {
...
void normalize(vector<Query> &queries) {
    mx = -IINF;
    vector<int> aux;
    for (auto &q : queries) aux.push_back(q.val);
    sort(all(aux));
    int j = -1, i = 0;
    while (i < q) {
        int start = aux[i];
        while (i + 1 < q && aux[i + 1] == start) {
            i++;
        }
        aux[++j] = aux[i];
        i++;
    ...
    for (auto &q : queries) {
        if (q.t != 3) {
            auto it = lower_bound(all(aux), q.val);
            M[it - aux.begin() + 1] = q.val;
            q.val = it - aux.begin() + 1;
            mx = max(mx, q.val);
        }
    }
}
void solve() {
    ...
    for (auto &q : queries) {
        if (q.t == 1) {
            ...
            int ans = kth_smallest(1, 0, mx, q.val);
            if (ans == -1) cout << "invalid" << nline;
            else cout << M[ans] << nline;
        } else {
            cout << range_sum(1, 0, mx, 0, q.val - 1) << nline;
        }
    }
}
