 ...
 * Solution for: https://codeforces.com/problemset/problem/593/B
*/
...
#define MAXN 100005
#define y1 __y
int n, x1, x2, k[MAXN], b[MAXN], o1[MAXN], o2[MAXN];
...
void solve() {
    ...
    iota(o1, o1 + n, 0);
    iota(o2, o2 + n, 0);
    sort(o1, o1 + n, [&](int i, int j) {
        return y1[i] < y1[j];
    });
    sort(o2, o2 + n, [&](int i, int j) {
        return y2[i] < y2[j];
    });
    bool ok = true;
    for (int i = 0; i < n; i++) if (o1[i] != o2[i]) {
        ok = false;
        break;
    }
    if (ok)
