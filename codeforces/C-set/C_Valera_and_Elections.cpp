 ...
 * Solution for: https://codeforces.com/problemset/problem/369/C
*/
...
vector<bool> subtree_has_prob;
vector<int> v;
void pre_compute() {
    
}
void dfs2(int u, int par, bool needCheck) {
    if (needCheck && !subtree_has_prob[u]) {
        v.push_back(u);
        return;
    }
    for (auto p : g[u]) {
        if (p.first != par) {
            dfs2(p.first, u, p.second == 2);
        }
    }
}
bool dfs(int u, int par) {
    ...
    for (auto p : g[u]) {
        if (p.first != par) {
            bool has = p.second == 2 ? true : false;
            res |= has;
            res |= dfs(p.first, u);
        }
    }
    return subtree_has_prob[u] = res;
}
void solve() {
    cin >> n;
    subtree_has_prob.resize(n + 1);
    g.resize(n + 1);
    int x, y, t;
    for (int i = 1; i <= n - 1; i++) {
        cin >> x >> y >> t;
