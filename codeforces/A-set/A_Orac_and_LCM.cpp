 ...
 * Solution for: https://codeforces.com/contest/1349/problem/A
*/
...
void brute() {
    ...
    for (int i = 0; i < n -1; i++) {
        for (int j  =i +1; j < n; j++) {
            v.push_back(a[i]*a[j]/gcd(a[i],a[j]));
        }
    }
    if (v.size() == 1) {cout << v[0] <<nline; return;}
    int ans = gcd(v[0],v[1]);
    for (int i = 2; i < v.size(); i++) {
        ans = gcd(ans,v[i]);
    }
    cout << "brute: " << ans << nline;
}
void solve() {
    cin >> n;
    PF pf(PFOption(2e5 + 5, true, false));
    unordered_map<int,vector<int>> mp;
    for (int i = 0; i < n; i++) {
        cin >> a[i]; ll val = a[i];
        auto res = pf.primeFactorize(val);
        for (auto &[factor, cnt] : res) {
            mp[factor].push_back(cnt);
        }
    }
    if (n==2) {cout << a[0]*a[1]/gcd(a[0],a[1]) << nline; return;}
    ll ans = 1;
    for (auto &[key,list] : mp) {
        sort(list.begin(), list.end());
        if (((int) list.size()) + 1 >= n) {
            if (((int)list.size()) + 1 == n) ans = ans * expo(key, list[0]);
            else ans = ans * expo(key, list[1]);
        }
    }
    cout << ans << nline;
    // brute();
