 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/7/2/practice/contest/289391/problem/D
*/
...
#define MAX 300005
int parent[MAX], sz[MAX], len[MAX];
int n, m;
int find(int x) {
    if (x == parent[x]) return x;
    int rx = find(parent[x]);
    len[x] = len[x] + len[parent[x]];
    return parent[x] = rx;
}
void unite(int a, int b) {
    len[b] = max(len[b], 1 + len[a]);
    parent[b] = a;
}
void pre_compute() {
