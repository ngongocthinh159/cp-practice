 ...
 * Solution for: https://codeforces.com/contest/468/problem/B
*/
...
int ans[MAX];
set<int> g[MAX];
void dfs(int u, int par, int &edges, bool &self_loop, int &leaf, vector<bool> &visited) {
    visited[u] = true;
    if (!self_loop) leaf = u;
    for (auto v : g[u]) {
        if (v != par) {
            ...
            if (v == u) {
                ...
                dfs(v, u, edges, self_loop, leaf, visited);
            }
        }
    }
}
void get_group(int u, int par, vector<int>& group) {
    group.push_back(u);
    for (auto v : g[u]) {
        if (v != par && v != u) get_group(v, u, group);
    }
}
int get_g(int u, int v) {
    if (u + v == A) return 0;
    return 1;
}
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        if (!visited[i]) {
            ...
            int leaf = -1;
            int edges = 0;
            dfs(i, -1, edges, self_loop, leaf, visited);
            if (!self_loop && !(edges&1)) {
                cout << "NO" << nline;
                return;
            }
            vector<int> group;
            get_group(leaf, -1, group);
            int start = 0;
            if (sz(group)&1) {
                ans[group[0]] = get_g(a[group[0]], a[group[0]]);
                start = 1;
            }
            for (int j = start; j < sz(group); j += 2) {
                int _g = get_g(a[group[j]], a[group[j + 1]]);
                ans[group[j]] = ans[group[j + 1]] = _g;
            }
        }
    }
    cout << "YES" << nline;
