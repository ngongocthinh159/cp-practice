 ...
 * Solution for: https://onlinejudge.org/index.php?option=com_onlinejudge&Itemid=8&page=show_problem&problem=1006
*/
...
int ccw(const pdd& A, const pdd& B, const pdd& C) {
    pdd AB = B - A;
    pdd AC = C - A;
    return cmp(cross(AB, AC), 0);
}
void convexHull(vector<pdd>& p) {
    sort(p.begin(), p.end());
    unique((p.begin(), p.end()), p.end());
...
double signedArea(vector<pdd> &p) {
    int n = p.size();
    double res = 0;
    for (int i = 0; i < n; i++) {
...
void solve() {
    ...
    vector<pdd> hull = p;
    convexHull(hull);
    printf("Wasted Space = %.2f %%\n\n", (area(hull) - area(p)) / area(hull) * 100);
}
