...
void pre_compute() {
    ...
    ifact[MAXN - 1] = expo(fact[MAXN - 1], MOD - 2, MOD);
...
void solve() {
    ...
    vector<int> L(n - 1, -1), R(n - 1, SZ(lcp));
    vector<int> st;
    vector<ll> cnt_exact(n + 1);
    ...
    st.clear();
    for (int i = SZ(lcp) - 1; i >= 0; i--) {
        while (st.size() && lcp[st.back()] >= lcp[i]) st.pop_back();
        if (st.size())
            R[i] = st.back();
        st.push_back(i);
    }
    cnt_exact[1] = 1LL*n*(n + 1)/2 % MOD;
    for (int i = 0; i < SZ(lcp); i++) {
        cnt_exact[1] = (cnt_exact[1] - lcp[i]) % MOD;
        if (cnt_exact[1] < 0) cnt_exact[1] += MOD;

        if (L[i] != -1 && lcp[L[i]] == lcp[i]) continue;
        int counted_len = 0;
        if (L[i] != -1) counted_len = max(counted_len, lcp[L[i]]);
        if (R[i] != SZ(lcp)) counted_len = max(counted_len, lcp[R[i]]);
        cnt_exact[R[i] - L[i]] = (cnt_exact[R[i] - L[i]] + lcp[i]) % MOD;
        cnt_exact[R[i] - L[i]] = (cnt_exact[R[i] - L[i]] - counted_len) % MOD;
        if (cnt_exact[R[i] - L[i]] < 0) cnt_exact[R[i] - L[i]] += MOD;
    }
    for (int i = 2; i <= n; i++) {
        cnt_exact[1] = (cnt_exact[1] - cnt_exact[i]) % MOD;
        if (cnt_exact[1] < 0) cnt_exact[1] += MOD;
    }
    ...
    for (int k = 1; k <= n; k++) {
        for (int j = k; j <= n; j++) {
            f[k] = (f[k] + cnt_exact[j] * combination(j, k) % MOD) % MOD;
        }
    }
