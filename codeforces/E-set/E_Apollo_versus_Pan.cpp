 ...
 * Solution for: https://codeforces.com/problemset/problem/1466/E
*/
...
void pre_compute() {
    _2pow[0] = 1;
    for (int i = 1; i <= 64; i++) {
        _2pow[i] = (_2pow[i - 1] * 2) % MOD;
    }
}
void solve() {
    cin >> n;
    for (int i = 0; i <= 60; i++) cnt[i] = 0;
    for (int i = 0; i < n; i++) sumOrX[i] = 0, sumAndX[i] = 0;
    for (int i = 0; i < n; i++) {
        cin >> a[i];
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int bit = 60; bit >= 0; bit--) {
            if ((a[i] >> bit) & 1) {
                sumAnd = (sumAnd + (cnt[bit] * _2pow[bit] % MOD)) % MOD;
            }
            if ((a[i] >> bit) & 1) {
                sumOr = (sumOr + (1LL * n * _2pow[bit] % MOD)) % MOD;
            } else {
                sumOr = (sumOr + (cnt[bit] * _2pow[bit]) % MOD) % MOD;
            }
        }
        sumOrX[i] = sumOr;
        sumAndX[i] = sumAnd;
    ...
    ll ans = 0;
    for (int i = 0; i < n; i++) {
        ans = (ans + sumOrX[i] * sumAndX[i] % MOD) % MOD;
    }
    cout << ans << nline;
}
