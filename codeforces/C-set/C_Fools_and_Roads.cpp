 ...
 * Solution for: https://codeforces.com/contest/191/problem/C 
*/
...
const int mxN = 1e5 + 5;
int par[mxN][LOG + 1];
int direct_par[mxN], height[mxN], effect[mxN], ans_node[mxN], ans_edge[mxN];
int n, ui, vi, k, ai, bi;
...
void calculate_ans(int u) {
    ...
    for (auto &p : g[u]) {
        ...
        if (v != direct_par[u]) {
            calculate_ans(v);
            ans_node[u] += ans_node[v];
            ans_edge[edge_num] = ans_node[v];
        }
    }
}
void solve() {
