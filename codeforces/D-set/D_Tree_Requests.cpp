 ...
 * Solution for: https://codeforces.com/contest/570/problem/D
*/
...
int n, m, sz[MAXN], largest[MAXN], height[MAXN], h[MAXN][26], odd[MAXN], c[MAXN], tin[MAXN], tout[MAXN], ver[MAXN], T = -1;
bool ans[MAXN];
vector<int> g[MAXN];
...
void solve() {
    ...
    for (int i = 0; i < m; i++) {
        int v, h; cin >> v >> h;
        h--;
        queries[v].push_back(Query{h, i});
    }
    dfs1(1, -1);
    dfs2(1, -1, 1);
    for (int i = 0; i < m; i++) cout << (ans[i] ? "Yes" : "No") << nline;
}
