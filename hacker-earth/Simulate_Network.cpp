 ...
 * Solution for: https://assessment.hackerearth.com/challenges/hiring/globalsoft-backend-hiring-challenge/algorithm/efficient-network/
*/
...
void prim(vector<vector<int>> &g, vector<Edge> &edges, vector<int> &choose_edges, ll &min_mst) {
    vector<int> min_e(n + 1, -1); // edge index
    vector<bool> visited(n + 1);
    set<pair<ll,int>> S; // edge to u, min edge index
    S.insert({0,-1});
    while (S.size()) {
        auto p = *S.begin();
        S.erase(S.begin());
        ...
        if (edge_idx == -1) u = 1;
            ...
            if (!visited[edges[edge_idx].u]) u = edges[edge_idx].u;
            if (!visited[edges[edge_idx].v]) u = edges[edge_idx].v;
        }
        
        visited[u] = true;
        if (edge_idx != -1) choose_edges.push_back(edge_idx);
        min_mst += w_paru;

        for (auto idx : g[u]) {
            int v = edges[idx].u^edges[idx].v^u;
            int w_uv = edges[idx].w;
            if (!visited[v] && (min_e[v] == -1 || edges[min_e[v]].w > w_uv)) {
                if (min_e[v] != -1) S.erase({edges[min_e[v]].w, min_e[v]});
                min_e[v] = idx;
                S.insert({w_uv, idx});
            }
        }
...
void solve() {
    ...
    ll min_mst = 0;
    prim(g, edges, choose_edges, min_mst);
    unordered_set<int> S;
    vector<int> v;
    for (auto idx : choose_edges) v.push_back(edges[idx].w);
    sort(all(v));
    cin >> q;
    vector<int> new_cables(q);
    for (int i = 0; i < q; i++) {
        cin >> new_cables[i];
    }
    sort(all(new_cables));
    int i = choose_edges.size() - 1;
    int j = 0;
    while (i >= 0 && j < q && v[i] > new_cables[j]) {
        min_mst = min_mst - v[i] + new_cables[j];
        i--;
        j++;
    }
    cout << min_mst << nline;
