 ...
 * Solution for: https://codeforces.com/contest/1633/problem/D
*/
...
const int mxk = 1e6 + 5;
int n, k;
int b[mxn], c[mxn], dist[mxn];
ll cur[12*mxn], prv[12*mxn]; // cur[j] = dp[i][j], prv[j] = dp[i - 1][j],         dp[i][j]: 0/1 knapsack 
void pre_compute() {
...
void solve() {
    ...
    for (int i = 1; i <= n; i++) cin >> c[i], coin_sum += c[i];
    if (k >= 12*n) cout << coin_sum << nline; 
    else {
        for (int j = 0; j < 12*n; j++) prv[j] = 0;
        for (int i = 1; i <= n; i++) {
            for (int j = 0; j <= 12*n; j++) { // max dist = 12 => max k can used = 12*n
                cur[j] = prv[j];
                if (j >= dist[b[i]]) cur[j] = max(cur[j], c[i] + cur[j - dist[b[i]]]);
            }
            for (int j = 0; j <= 12*n; j++) prv[j] = cur[j];
        }
        cout << cur[k] << nline;
    }
}
