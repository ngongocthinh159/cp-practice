 ...
 * Solution for: https://codeforces.com/contest/1592/problem/C
*/
...
int dfs(int u, int par, vector<vector<int>> &g, int &compo) {
    int cur = a[u];
    for (auto v : g[u]) if (v != par) {
        int res = dfs(v, u, g, compo);
        if (res != -1) cur ^= res;
    }
    if (cur == xr) {
        compo++;
...
void solve() {
    ...
    if (xr == 0) {
        ...
        int compo = 0;
        int res = dfs(1, -1, g, compo);
        if ((res == 0 || res == -1) && compo >= 3 && k >= 3) {
            cout << "YES" << nline;
        } else
            cout << "NO" << nline;
    }
