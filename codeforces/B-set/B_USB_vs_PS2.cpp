 ...
 * Solution for: https://codeforces.com/contest/762/problem/B
*/
...
void solve() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        v.push_back({price,type});
    }
    sort(v.begin(),v.end());
    ll price = 0;
    for (int i = 0; i < v.size(); i++) {
        string type = v[i].second;
        bool ok = false;
        if (type=="USB") {
            if (a) {a--; price+=v[i].first; ok = true;}
        } else if (type=="PS/2") {
            if (b) {b--; price+=v[i].first; ok = true;}
        }
        if (!ok && c) {c--; price+=v[i].first;} 
        if (a==0&&b==0&&c==0) break;
    }
    cout << total - (a+b+c) << " " << price << nline;
}
