 ...
 * Solution for: https://www.spoj.com/problems/CNTPRIME/
*/
...
void build(int idx, int s, int e) {
    if (s == e) {
        segtree[idx] = make_data(isPrime[a[s]]);
        return;
    }
    int m = (s + e)/2;
    build(2*idx, s, m);
...
void apply(int idx, int s, int e, ll val) {
    segtree[idx].val = (isPrime[val] ? (e - s + 1) : 0); // apply update
