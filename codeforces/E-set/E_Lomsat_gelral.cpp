 ...
 * Solution for: https://codeforces.com/contest/600/problem/E
*/
...
#define MAXN 100005
int n, freq, sz[MAXN], largest[MAXN], col[MAXN], color[MAXN], tin[MAXN], tout[MAXN], ver[MAXN], T = -1;
ll ans[MAXN], sum = 0;
vector<int> g[MAXN];
void pre_compute() {
...
void dfs2(int u, int par, bool keep) {
    ...
    add(u); // add current ver to big child
    for (auto v : g[u]) if (v != par && v != largest[u]) { // merge all small child to big child
    ...
    // if current ver is not a big child of its parent, remove its contribution to parent 
    // (later we merge from small to big, if we not remove contribution then overlapping count)
    if (!keep) {
        freq = sum = 0;
