 ...
 * Solution for: https://codeforces.com/problemset/problem/1092/F
*/
...
void dfs1(int u, int par) {
    ...
    for (auto v : g[u]) {
        if (v != par) {
            dfs1(v, u);
            sub_a_sum[u] += sub_a_sum[v];
            sub_sum[u] += sub_sum[v] + sub_a_sum[v];
        }
    }
}
void changeRoot(int u, int v) {
    ...
    sub_sum[v] += (sub_sum[u] + sub_a_sum[u]);
    sub_a_sum[v] += sub_a_sum[u];
}
void dfs2(int u, int par) {
    ans = max(ans, sub_sum[u]);
    for (auto v : g[u]) {
        if (v != par) {
            changeRoot(u, v);
            dfs2(v, u);
