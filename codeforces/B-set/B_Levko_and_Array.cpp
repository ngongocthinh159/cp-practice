 ...
 * Solution for: https://codeforces.com/problemset/problem/360/B
*/
...
#define MAXN 2005
int n, k, a[MAXN], dp[MAXN]; // minimum number of changes needed from [0 -> i] and the i-th is fixed as unchanged

void pre_compute() {

}
bool check(int d) {
    for (int i = 0; i <= n; i++) dp[i] = IINF;
    dp[0] = 0;
    for (int i = 1; i <= n; i++) {
        for (int j = 0; j < i; j++) if (dp[j] != IINF) {
            if (j == 0 || abs(a[i] - a[j]) <= 1LL*(i - j) * d)
                minimize(dp[i], dp[j] + (i - j - 1));
        }
    }
    int ans = IINF;
    for (int i = 0; i <= n; i++) ans = min(ans, dp[i] + n - i); // brute force each last unchanged elements
    return ans <= k;
}
void solve() {
    cin >> n >> k;
