 ...
 * Solution for: https://codeforces.com/contest/2208/problem/D2
*/
...
int n;
vector<int> g[N], g2[N];
int mat[N][N], b[N][N], color[N], reach[N];
void pre_compute() {

}
void dfs(int u, int *c, bool &cycle) {
    c[u] = 2;
    for (auto v : g[u]) {
        if (c[v] == 2) {
            cycle = true;
...
void solve() {
    ...
    for (int i = 1; i <= n; i++) {
        int org = reach[i];
        vector<int> buf;
        while (cnt < n - 1 && reach[i] >= 1) {
            int mx = -1, idx;
            for (int j = 1; j <= n; j++) if (mat[i][j] && i != j) {
                if (maximize(mx, reach[j])) idx = j;
            }
            add(i, idx);
            for (int j = 1; j <= n; j++) if (mat[idx][j] && i != j) {
                if (mat[i][j] == 1) mat[i][j] = 0, reach[i]--, buf.push_back(j);
            }
        }
        for (auto x : buf) mat[i][x] = 1;
        reach[i] = org;
    }
    if (cnt != n - 1) {
        cout << "NO" << nline;
    ...
    for (int i = 1; i <= n; i++)
        for (int j = 1; j <= n; j++) if (mat[i][j] != b[i][j]) {
            cout << "NO" << nline;
            return;
        }
