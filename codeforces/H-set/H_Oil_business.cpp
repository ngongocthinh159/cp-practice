 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/7/2/practice/contest/289391/problem/H
*/
...
void solve() {
    ...
    reverse(ALL(tmp));
    vector<int> v;
    for (int i = 0; i < tmp.size() && s >= edges[tmp[i]].w; i++) {
        v.push_back(edges[tmp[i]].i);
        s -= edges[tmp[i]].w;
    }
    sort(ALL(v));
    cout << v.size() << nline;
