 ...
 * Solution for: https://codeforces.com/contest/1528/problem/A
*/
...
ll dfs(int u, int par, int prev) {
    ...
    ll b = 0;
    if (par != -1) {
        a = abs(l[u] - (prev == 0 ? l[par] : r[par]));
        b = abs(r[u] - (prev == 0 ? l[par] : r[par]));
    }
    ll tot_a = 0, tot_b = 0;
    ...
    res = max(a + tot_a, b + tot_b);
    return dp[u][prev] = res;
}
void solve() {
    cin >> n;
    g.clear();
