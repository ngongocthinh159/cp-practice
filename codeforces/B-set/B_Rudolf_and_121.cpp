 ...
 * Solution for: https://codeforces.com/contest/1941/problem/B
*/
...
void solve() {
    ...
    if (sum%4!=0) {cout << "no" << nline; return;}
    bool ok = true;
    for (int i = 1; i < n - 1; i++) {
        if (a[i-1]==0) continue;
        ll time = a[i-1];
        if (a[i]<time*2 || a[i+1] < time) {ok=false; break;}
        a[i-1]=0;
        a[i]-=time*2;
        a[i+1]-=time;
    }
    for (int i = 0; i<n; i++) {
        if (a[i]!=0) {cout << "no" <<nline; return;}
    }
    cout << "yes" <<nline; 
}
