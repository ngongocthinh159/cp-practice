 ...
 * Solution for: https://onlinejudge.org/index.php?option=com_onlinejudge&Itemid=8&page=show_problem&problem=3781
*/
...
struct Trie {
    ...
    void insert(string &s) {
        ...
        for (auto c : s) {
            ...
            idx = c - 'a';
            if (p->child[idx] == NULL) {
                p->child[idx] = new Node(), nodes++;
                if (level >= 2) freq[idx]++;
            }
            p = p->child[idx];
        }
    }
    int freq_level2_of(int c) {
        return freq[c];
    }
    int nodes_cnt() {
        return nodes;
...
void dfs(Node* root, Trie &t2, ll &ans, int c, int level) {
    if (c != -1 && level >= 2) {
        ans -= t2.freq_level2_of(c);
    }
    for (int i = 0; i < 26; i++) {
        if (root->child[i] != NULL) {
            dfs(root->child[i], t2, ans, i, level + 1);
        }
    }
}
void solve() {
    ...
    // cout << t2.nodes_cnt() << nline;
    ll ans = 1LL*t1.nodes_cnt()*t2.nodes_cnt();
    dfs(t1.get_root(), t2, ans, -1, 0);
    cout << ans << nline;
}
