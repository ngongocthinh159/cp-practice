 ...
 * Solution for: https://codeforces.com/contest/246/problem/E
*/
...
int n, m, sz[MAXN], big[MAXN], tin[MAXN], tout[MAXN], ver[MAXN], ans[MAXN], height[MAXN], T = -1;
ll name[MAXN], basepow[MAXL];
const ll base = 37;
unordered_map<ll,ll,custom_hash> freq[MAXN];
vector<int> g[MAXN];
vector<Query> queries[MAXN];

void pre_compute() {
    basepow[0] = 1;
    for (int i = 1; i < MAXL; i++) basepow[i] = basepow[i - 1] * base % MOD; 
}
ll get_name(string &s) {
    ll res = 0;
    for (int i = 0; i < SZ(s); i++) {
        res = (res + (s[i] - 'a' + 1) * basepow[i] % MOD) % MOD;
    }
    return res;
}
...
void del(int v, int u) {
    if (--freq[height[v]][name[v]] == 0) freq[height[v]].erase(name[v]);
}
void add(int v, int u) {
    freq[height[v]][name[v]]++;
}
...
void solve() {
    ...
    for (int i = 0; i < m; i++) {
        int v, k; cin >> v >> k;
        if (height[v] + k >= n) continue;
        queries[v].push_back(Query{height[v] + k, i});
    }
    for (auto root : roots) {
        dfs2(root, -1, false);
    }
    for (int i = 0; i < m; i++) cout << ans[i] << nline;
}
