 ...
 * Solution for: https://codeforces.com/contest/432/problem/D
*/
...
void solve() {
    ...
    FORD(i,n-1,1) pref_cnt[pref[i - 1]] += pref_cnt[i];
    FOR(i,1,n-1) pref_cnt[i]++;
    vector<pair<int,int>> res;
    res.push_back({n,1});
    int h = pref[n - 1];
    while (h) {
        res.push_back({h,pref_cnt[h]});
        h = pref[h - 1];
    }
    cout << SZ(res) << nline;
    FORD(i,SZ(res)-1,0) cout << res[i].fi << " " << res[i].se << nline;
}
