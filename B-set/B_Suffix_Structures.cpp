...
void solve() {
    ...
    f(i,0,s1.size()) cnt1[s1[i] - 'a']++;
    f(i,0,s2.size()) cnt2[s2[i] - 'a']++;
    bool alwaysEqual = true, same = true;
    f(i,0,26) {
        if (cnt1[i] < cnt2[i]) {cout << "need tree\n"; return;}
        if (cnt1[i] > cnt2[i]) alwaysEqual = false;
    }
    if (alwaysEqual) {
        cout << "array\n";
    } else {
        ...
        while (i < s1.size() && j < s2.size()) {
            int cur = s2[j];
            while (i < s1.size() && s1[i] != cur) {
                i++;
            }
            i++;
            j++;
        }
        if (i >= s1.size() && !(j >= s2.size())) cout << "both\n";
        else cout << "automaton\n";
    }
}
