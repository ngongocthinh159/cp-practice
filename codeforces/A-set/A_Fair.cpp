 ...
 * Solution for: https://codeforces.com/problemset/problem/986/A
*/
...
int n, m, k, s;
vector<vector<int>> g;
vector<vector<int>> goodDist;
vector<int> goodAt;
void pre_compute() {

}
void bfs(vector<int> &src, int good) {
    deque<int> q;
    vector<bool> visited(n + 1);
    for (auto x : src) visited[x] = true, q.push_back(x);
    int level = 0;
    while (q.size()) {
        int size = q.size();
        for (int i = 0; i < size; i++) {
            int u = q.back(); q.pop_back();
            goodDist[u].push_back(level);
            for (auto v : g[u]) {
                if (!visited[v]) {
                    visited[v] = true;
                    q.push_front(v);
...
void solve() {
    ...
    goodAt.resize(n + 1);
    goodDist.resize(n + 1);
    vector<vector<int>> srcs(k + 1, vector<int>());
    for (int i = 1; i <= n; i++) {
        cin >> goodAt[i];
        srcs[goodAt[i]].push_back(i);
    }
    for (int i = 0; i < m; i++) {
        int u,v; cin >> u >> v;
        g[u].push_back(v);
    ...
    for (int i = 1; i <= n; i++) {
        auto it = goodDist[i].begin() + (s - 1);
        nth_element(goodDist[i].begin(), it, goodDist[i].end());
        ll ans = 0;
        for (int j = 0; j < s; j++) ans += goodDist[i][j];
        cout << ans << " ";
    }
}
