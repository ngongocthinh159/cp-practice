 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/4/2/practice/contest/273278/problem/C
*/
...
int query(int idx, int s, int e, int x) {
    if (s == e) {
        if (segtree[idx].mx >= x) return s;
        return -1;
    }
    int m = (s + e)/2;
    if (segtree[2*idx].mx >= x) return query(2*idx, s, m, x);
    else if (segtree[2*idx + 1].mx >= x) return query(2*idx + 1, m + 1, e, x);
}
void pre_compute() {
