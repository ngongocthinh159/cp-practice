 ...
 * Solution for: https://codeforces.com/problemset/problem/803/F
*/
...
void solve() {
    cin >> n;
    int mx = 0;
    for (int i = 0; i < n;i++) cin >> a[i], cnt[a[i]]++, mx = max(mx, a[i]);
    for (int i = mx; i >= 1; i--) {
        ll tot = 0;
        ll accum = 0;
        for (int j = i; j <= mx; j += i) {
            tot = (tot + cnt[j]) % MOD;
            accum = (accum + dp[j]) % MOD;
        }
        dp[i] = ((_2pow[tot] - 1) % MOD + MOD) % MOD;
        dp[i] = ((dp[i] - accum) % MOD + MOD) % MOD;
    }
    cout << dp[1] << nline;
}
