 ...
 * Solution for: https://onlinejudge.org/index.php?option=com_onlinejudge&Itemid=8&page=show_problem&problem=1519
*/
...
int dfs(int c[6], int sum) {
    if (sum == 31) {
        return 0;
    }
    if (dp[c[0]][c[1]][c[2]][c[3]][c[4]][c[5]] != -1) 
        return dp[c[0]][c[1]][c[2]][c[3]][c[4]][c[5]];
    int res = 0;
    for (int i = 0; i < 6; i++) {
        if (c[i] < 4 && sum + (i + 1) <= 31) {
            int _c[6];
            for (int i = 0; i < 6; i++) _c[i] = c[i];
            _c[i]++;
            if (!dfs(_c, sum + (i + 1))) {
                res = 1;
                break;
    ...
    return dp[c[0]][c[1]][c[2]][c[3]][c[4]][c[5]] = res;
}
void solve() {
    int sum = 0;
