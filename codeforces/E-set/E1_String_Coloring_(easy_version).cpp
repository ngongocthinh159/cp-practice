 ...
 * Solution for: https://codeforces.com/contest/1296/problem/E1
*/
...
void solve() {
        ...
        for (int j = 0; j <= 26; j++)
            for (int k = 0; k <= 26; k++) {
                if (dp[i][j][k]) {
                    // cout << i << " " << j << " " << k << nline;
                    int c = s[i] - 'a' + 1;
                    if (j <= c) if (maximize(dp[i + 1][c][k], dp[i][j][k])) {
                        trace[i + 1][c][k] = (j << 6) ^ (k << 1) ^ (0 << 0);
                    }
                    if (k <= c) if (maximize(dp[i + 1][j][c], dp[i][j][k])) {
                        trace[i + 1][j][c] = (j << 6) ^ (k << 1) ^ (1 << 0);
                    }
                }
            }
    int bestI = n, bestJ = -1, bestK = -1;
    for (int j = 0; j <= 26; j++)
        for (int k = 0; k <= 26; k++) if (dp[bestI][j][k]) {
            bestJ = j, bestK = k;
            break;
        }
    if (bestJ == -1 && bestK == -1) {
    ...
    while (trace[bestI][bestJ][bestK] != -1) {
        // cout << bestI << " " << bestJ << " " << bestK << nline;
        int tmp = trace[bestI][bestJ][bestK];
        int sequence = tmp & (MASK(1) - 1); tmp >>= 1;
        int k = tmp & (MASK(5) - 1); tmp >>= 5;
        int j = tmp & (MASK(5) - 1); tmp >>= 5;

        if (sequence == 1) ans[bestI - 1] = 1;
