 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/4/2/practice/contest/273278/problem/D
*/
...
ndata combine(ndata l, ndata r) {
    if (l.mx >= r.mx) return make_ndata(l.mx);
    return make_ndata(r.mx);
}
...
int query(int idx, int s, int e, int l, int r, int x) {
    ...
    int m = (s + e)/2;
    int res1 = query(2*idx, s, m, l, r, x);
    if (res1 != -1) return res1;
    int res2 = query(2*idx + 1, m + 1, e, l, r, x);
    return res2;
}
void pre_compute() {
...
void solve() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        if (type == 1) {
            ...
            cin >> x >> l;
            cout << query(1, 0, n - 1, l, n - 1, x) << nline;
        }
    }
}
