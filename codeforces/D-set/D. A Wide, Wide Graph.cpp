 ...
 * Solution for: https://codeforces.com/problemset/problem/1805/D
*/
...
pair<int,int> dfs(int u, int p, int d) {
    ...
    for (auto v : g[u]) if (v != p) {
        auto [k, dist] = dfs(v, u, d + 1);
        if (maximize(mxdist, dist)) mxv = k;
    }
    return {mxv, mxdist};
}
struct DSU {
...
void dfs2(int u, int p, int d, vector<vector<int>>& level) {
    level[d].push_back(u);
    for (auto v : g[u]) if (v != p) {
        dfs2(v, u, d + 1, level);
    }
...
void solve() {
    ...
    int compo = n;
    DSU d(n + 1); 
    vector<vector<int>> levelu(diameter + 1);
    vector<vector<int>> levelv(diameter + 1);
    dfs2(u, -1, 0, levelu);
    dfs2(v, -1, 0, levelv);
    for (int k = n; k >= 1; k--) {
        if (k > diameter) {
            ...
            for (auto x : levelu[k]) {
                if (d.unite(u, x)) compo--;
            }
            for (auto x : levelv[k]) {
                if (d.unite(v, x)) compo--;
            }
            ans[k] = compo;
        }
    }
