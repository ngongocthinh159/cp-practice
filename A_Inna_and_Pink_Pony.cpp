...
void solve() {
    ll n, m, i, j, a, b;
    cin >> n >> m >> i >> j >> a >> b;
    vector<pair<int, int>> corners = {{1, m}, {n, 1}, {n, m}, {1, 1}};
    vector<pair<int, int>> moves = {{a, b}, {-a, b}, {a, -b}, {-a, -b}};
    ll cnt = 0, ans = LLONG_MAX;
    vector<pair<int, int>> ok;
    bool atLeastOneMove = false;
    f(k,0,corners.size()) {
        if (i == corners[k].fi && j == corners[k].se) {cout << 0 << "\n"; return;}
    }
    f(k,0,corners.size()) {
        pair<int, int>& corner = corners[k];
        if (abs(corner.fi - i)%a != 0 || abs(corner.se - j)%b != 0) cnt++;
        else if ((abs(corner.fi - i)/a + abs(corner.se - j)/b) & 1) cnt++;
        else {
            ans = min(ans, max(abs(corner.fi - i)/a, abs(corner.se - j)/b));
        }
    }
    f(k,0,moves.size()) {
        ll newX = i + moves[k].fi;
        ll newY = j + moves[k].se;
        if (1 <= newX && newX <= n && 1 <= newY && newY <= m) {
            atLeastOneMove = true;
            break;
        }
    }
    if (cnt != 4 && atLeastOneMove) cout << ans << "\n";
