 ...
 * Solution for: https://codeforces.com/problemset/problem/1985/H1
*/
...
void solve() {
    ...
    for (int i = 0 ;i < n;i++) {
        for (int j = 0;j < m; j++) {
            cin >> grid[i][j];
            if (grid[i][j] == '#') dsu.make_set(i*m + j);
        }
    }
    vector<int> row_emp(n);
    vector<int> col_emp(m);
    for (int i = 0; i < n; i++) {
        for (int j= 0;j < m; j++) {
            if (grid[i][j] == '.') row_emp[i]++, col_emp[j]++;
            if (grid[i][j] == '#') {
                for (auto &move : moves) {
                    int I = i + move[0];
                    int J = j + move[1];
                    if (0 <= I && I < n && 0 <= J && J < m && grid[I][J] == '#') {
                        dsu.unite(i*m+j, I*m+J);
                    }
                }
            }
        }
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            if (grid[i][j] == '#') {
                row[i].insert(dsu.find(i*m + j));
                col[j].insert(dsu.find(i*m + j));
            }
            for (auto &move : moves) {
                int I = i + move[0];
                int J = j + move[1];
    ...
    for (int i = 0; i < n; i++) {
        int cur = row_emp[i];
        for (auto x : row[i]) cur += dsu.get_size(x);
        ans = max(ans, cur);
    }
    for (int j = 0; j < m; j++) {
        int cur = col_emp[j];
        for (auto x : col[j]) cur += dsu.get_size(x);
        ans = max(ans, cur);
    }
    cout << ans << nline;
}
