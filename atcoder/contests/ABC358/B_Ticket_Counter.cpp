 ...
 * Solution for: https://atcoder.jp/contests/abc358/tasks/abc358_b
*/
...
int a[mxn];
int ans[mxn];
const int EVENT_COME = 0;
const int EVENT_DONE = 1;
void pre_compute() {
...
struct comparator {
    bool operator() (const vector<int>& v1, const vector<int>& v2) const {
        ...
        if (v2[0] == v1[0] && v2[1] == EVENT_DONE && v1[1] == EVENT_COME) return true;
        return false;
    }
};
struct comparator2 {
...
void solve() {
    ...
    priority_queue<vector<int>, vector<vector<int>>, comparator2> delayed;
    
    for (int i = 0; i < n; i++) {
        pq.push({a[i], EVENT_COME, i});
    }
    int time, type, idx, process_idx, process_starting_time;
    bool free = true;
    while (pq.size()) {
        ...
        if (type == EVENT_COME) {
            if (free) {
                ...
                process_starting_time = time;                    
                process_idx = idx;
                
                pq.push({process_starting_time + A, EVENT_DONE, process_idx});
                
            } else {
                delayed.push({time, idx});
            }
        } else {
            free = true;
            ...
            if (delayed.size()) {
                ...
                auto tmp = delayed.top(); delayed.pop();
                process_starting_time = time;
                process_idx = tmp[1];
