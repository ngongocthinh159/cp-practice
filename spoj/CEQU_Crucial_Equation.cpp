 ...
 * Solution for: https://www.spoj.com/problems/CEQU/
*/
...
bool diophantus_linear(ll a, ll b, ll c) {
    ll x0, y0, x, y;
    ll g = extended_gcd(abs(a), abs(b), x0, y0);
    if (c % g) 
        return false;
...
void solve() {
    cin >> a >> b >> c;
    if (diophantus_linear(a, b, c)) cout << "Yes" << nline;
    else cout << "No" << nline;
}
