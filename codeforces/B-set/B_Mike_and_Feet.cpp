 ...
 * Solution for: https://codeforces.com/contest/547/problem/B
*/
...
int a[mxn];
int n;
int nextSmallerLeft[mxn];
int nextSmallerRight[mxn];
void pre_compute() {
...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (v.size()) nextSmallerLeft[i] = v.back();
        else nextSmallerLeft[i] = -1;
        v.push_back(i);
    }
    v.clear();
    for (int i = n - 1; i >= 0; i--) {
        while (v.size() && a[v.back()] >= a[i]) v.pop_back();
        if (v.size()) nextSmallerRight[i] = v.back();
        else nextSmallerRight[i] = n;
        v.push_back(i);
    }
    for (int i = 0; i < n; i++) {
        int len = nextSmallerRight[i] - nextSmallerLeft[i] - 1;
