 ...
 * Solution for: https://codeforces.com/problemset/problem/778/B
*/
...
void solve() {
    ...
    cin.ignore();
    vector<string> v(n);
    string assigned, bin, firstOperand, secondOperand, operatorr, tmp;
    unordered_map<string,int> M;
    vector<vector<vector<bool>>> bit_at(2,
        vector<vector<bool>>(n,
        vector<bool>(m)));
    vector<vector<int>> sum(2, vector<int>(m));
    for (int i = 0; i < n; i++) {
        cin >> assigned >> tmp >> bin;
        if (bin[0] == '1' || bin[0] == '0') {
            for (int bit = 0; bit < sz(bin); bit++) {
                bit_at[0][i][bit] = bin[bin.size() - bit - 1] - '0';
                bit_at[1][i][bit] = bin[bin.size() - bit - 1] - '0';
            }
            M[assigned] = i; 
        } else {
            firstOperand = bin;
            cin >> operatorr >> secondOperand;
            ...
            for (int bit = 0; bit < m; bit++) {
                ...
                if (firstOperand == "?") {
                    ...
                    firstBit0 = bit_at[0][M[firstOperand]][bit];
                    firstBit1 = bit_at[1][M[firstOperand]][bit];
                }
                if (secondOperand == "?") {
                    secondBit0 = 0;
                    secondBit1 = 1;
                } else {
                    secondBit0 = bit_at[0][M[secondOperand]][bit];
                    secondBit1 = bit_at[1][M[secondOperand]][bit];
                }
                int resultBit0;
                int resultBit1;
                if (operatorr == "OR") {
                    resultBit0 = firstBit0 | secondBit0;
                    resultBit1 = firstBit1 | secondBit1;
                ...
                bit_at[0][i][bit] = resultBit0;
                bit_at[1][i][bit] = resultBit1;

                sum[0][bit] += resultBit0;
                sum[1][bit] += resultBit1;
    ...
    for (int bit = m - 1; bit >= 0; bit--) {
        resMn += sum[0][bit] <= sum[1][bit] ? '0' : '1';
        resMx += sum[0][bit] >= sum[1][bit] ? '0' : '1';
    }
