 ...
 * Solution for: https://atcoder.jp/contests/dp/tasks/dp_w
*/
...
#define MAXN 200005
int n, m;
vector<pair<int,int>> intervals_s[MAXN], intervals_e[MAXN];
struct Data {
    ll mx = 0;
};
Data segtree[4*MAXN];
ll lazy[4*MAXN], dp[MAXN];
...
void solve() {
    ...
    for (int i = 0; i < m; i++) {
        int l, r, a;
        cin >> l >> r >> a;
        intervals_s[l].push_back({r, a});
        intervals_e[r + 1].push_back({l, a});
    }
    ll ans = 0;
    update(1, 0, n, 0, 0, 0);
    for (int i = 1; i <= n; i++) {
        for (auto &p : intervals_s[i]) update(1, 0, n, 0, i - 1, p.second);
        for (auto &p : intervals_e[i]) update(1, 0, n, 0, p.first - 1, -p.second);
