 ...
 * Solution for: https://toph.co/p/unbelievable-array
*/
...
struct DSU {
    ...
    void change_val(int idx, int num) {
        val[find(idx)] = num;
    }
    int get_val(int idx) {
        return val[find(idx)];
...
void solve() {
    cin >> n >> q;
    DSU dsu(n);
    unordered_map<int,int> val_to_par;
    for (int i = 0; i < n; i++) {
        cin >> a[i];
        dsu.make_set(i, a[i]);
        val_to_par[a[i]] = i;
    ...
    for (int i = 0; i < q; i++) {
        ...
        if (type == 1) {
            cin >> x >> y;
            if (x == y || !val_to_par.count(x)) continue;
            int px = val_to_par[x];
            if (val_to_par.count(y)) {
                int py = val_to_par[y];
                dsu.unite(px, py);
                dsu.change_val(py, y);
            } else {
