 ...
 * Solution for: https://codeforces.com/problemset/problem/1555/D
*/
...
void solve() {
    ...
    vector<vector<int>> pref(pattern.size(), vector<int>(n + 1, 0));
    for (int i = 0; i < pattern.size(); i++) {
        for (int j = 1; j <= n; j++) {
            pref[i][j] = pref[i][j - 1] + (pattern[i][(j - 1)%3] != s[j - 1]);
        }
    }
    ...
    for (int i = 0; i < m; i++) {
        ...
        for (int j = 0; j < pattern.size(); j++) {
            cur = min(cur, pref[j][r + 1] - pref[j][l]);
        }
        cout << cur << nline;
    }
}
