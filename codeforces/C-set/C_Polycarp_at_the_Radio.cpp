 ...
 * Solution for: https://codeforces.com/problemset/problem/723/C
*/
...
void solve() {
    ...
    if (n%m==0) {
        ...
        ll oper = 0;
        for (int i = 1; i <= m; i++) {
            if (M[i] < need1) oper += need1 - M[i];
        }
        cout << need1 << " " << oper << nline;
    }
    else {
        need1 = n/m;
        need2 = need1;
    ...
    // cout << need1 << " " << need2 << nline;
    vector<int> can_changes;
    unordered_map<int,int> M2 = M;
    unordered_set<int> S;
    for (int i = 0; i < n; i++) {
        if (a[i] > m || (a[i] == largest && M[a[i]] > need2) || (a[i] != largest && M[a[i]] > need1)) {
            M[a[i]]--;
            can_changes.push_back(i);
            S.insert(i);
        }
    }
    // for (auto x : can_changes) {
    //     cout << x << " ";
    // }
    // cout <<  nline;
    int j = 0;
    for (int i = 1; i <= m; i++) {
        int need = i == largest ? need2 : need1;
        while (j < can_changes.size() && M2[i] < need) {
            M2[i]++;
            a[can_changes[j]] = i;
            j++; 
        }
