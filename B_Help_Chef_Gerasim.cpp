...
void solve() {
    ...
    sort(v.begin(), v.end());
    if (v[0].fi==v[v.size()-1].fi) {cout << "Exemplary pages.\n"; return;}
    if ((v[0].fi+v[v.size()-1].fi)%2!=0) {cout << "Unrecoverable configuration.\n"; return;}
    else {
        int tmp2 = (v[0].fi+v[v.size()-1].fi) / 2;
        int tmp3 = tmp2 - v[0].fi;
        v[0].fi = tmp2;
        v[v.size()-1].fi = tmp2;
        bool isOk = true;
        f(i,0,n-1) {
            if (v[i].fi != v[i+1].fi) {isOk=false; break;}
        }
        if (!isOk) {cout << "Unrecoverable configuration.\n"; return;}
        else {cout << tmp3 << " ml. from cup #" << v[0].se+1 << " to cup #" << v[v.size()-1].se+1 << ".\n"; return;}
    }
}
