 ...
 * Solution for: https://codeforces.com/contest/825/problem/F
*/
...
void solve() {
    ...
    n = s.size();
    dp[0] = 0;
    int len, m, mn_rotation_len, rep, rep_len;
    for (int i = 1; i <= n; i++) {
        dp[i] = INT_MAX;
        pref[i] = 0;
        int k = i;
        for (int j = i; j >= 1; j--) {
            if (j == i) {
                mn_rotation_len = rep_len = 1;
            } else {
                while (k < i && s[j - 1] != s[k - 1]) k = i - pref[k + 1];
                if (s[j - 1] == s[k - 1]) --k;
                pref[j] = i - k;

                len = (i - j + 1);
                m = len - pref[j];
                if (len % m == 0) mn_rotation_len = m;
                else mn_rotation_len = len;
                rep = len/mn_rotation_len;
                rep_len = 0;
                while (rep > 0) rep /= 10, rep_len++;
            }

            dp[i] = min(rep_len + mn_rotation_len + dp[j - 1], dp[i]);
        }
    }
    cout << dp[n] << "\n";
}
