 ...
 * Solution for: https://codeforces.com/contest/597/problem/C
*/
...
struct SegmentTree {
    ...
    void _update(int idx, int s, int e, int pos, ll val) {
        if (s == e) {
            segtree[idx] = val;
            return;
        }
        int m = (s + e)/2;
        if (pos <= m) _update(2*idx, s, m, pos, val);
        else _update(2*idx + 1, m + 1, e, pos, val);
        segtree[idx] = merge(segtree[2*idx], segtree[2*idx + 1]);
    }
    ll _query(int idx, int s, int e, int l, int r) {
        if (s > r || e < l) return 0;
...
void solve() {
    ...
    vector<ll> pre(n + 1);
    SegmentTree prev_tree(mx + 1);
    for (int i = 1; i <= mx; i++) {
        prev_tree.update(a[i], 1);
        pre[i] = prev_tree.query(0, a[i] - 1);
    }
    for (int k = 2; k <= K; k++) {
        SegmentTree tree(mx + 1);
        for (int i = 1; i <= mx; i++) {
            tree.update(a[i], pre[i]);
            pre[i] = tree.query(0, a[i] - 1);
        }
        swap(tree, prev_tree);
    }
    cout << prev_tree.query(0, mx) << nline;
}
