 ...
 * Solution for: https://onlinejudge.org/index.php?option=com_onlinejudge&Itemid=8&page=show_problem&problem=3055
*/
...
void solve() {
    ...
    for (int i = 2; i <= n; i++) {
        dp[i] = 0;
        N += unit[i];
        for (ll remain = 0; remain <= unit[i] - 1; remain++) {
            ll cur = dp[i - 1];
            cur = cur * nCk(remain + N - unit[i] - 1, remain) % MOD;
            dp[i] = (dp[i] + cur) % MOD;
        }
    }
    cout << dp[n] << nline;
