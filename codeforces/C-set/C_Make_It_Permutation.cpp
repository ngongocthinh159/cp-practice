 ...
 * Solution for: https://codeforces.com/problemset/problem/1810/C
*/
...
void solve() {
    ...
    ll ans = (v.size() - 1)*c + fixedCost + d;
    ll m = v.size() - 1;
    ll prevInsertCost = 0;
    for (int i = 1; i < v.size(); i++) {
        ll insert = v[i] - v[i - 1] - 1;
        ll del = v.size() - i - 1;
        ll cost = insert*d + del*c + fixedCost + prevInsertCost;
        ans = min(ans, cost);
        prevInsertCost += insert*d;
    }
    cout << ans << nline;
}
