...
int n, q;
string s;
vector<vector<int>> prf; // check all same substring
vector<vector<int>> prf1; // check alternate substring
bool all_same(ll _l, ll _r) {
    char c = s[_l];
    return (prf[c-'a'][_r+1] - prf[c-'a'][_l]) == (_r - _l + 1);
}
bool alternate(ll _l, ll _r) {
    char c1 = s[_l];
    char c2 = s[_l + 1];
    int r1 = (_l + _r)&1 ? _r - 1 : _r;
    int r2 = (_l+1 + _r)&1 ? _r - 1 : _r;
    int sum1 = prf1[c1-'a'][r1+2] - prf1[c1-'a'][_l];
    int sum2 = prf1[c2-'a'][r2+2] - prf1[c2-'a'][_l+1];
    int len = _r - _l + 1;
    return (sum1 + sum2 == len);
}
void solve() {
    cin >> n >> q >> s;
    Manacher manacher(s);
    prf = vector<vector<int>>(26, vector<int>(n + 1, 0));
    prf1 = vector<vector<int>>(26, vector<int>(n + 2, 0));
    for (int i = 0; i < n; i++) {
        prf[s[i] - 'a'][i + 1] = 1;
        prf1[s[i] - 'a'][i + 2] = 1;
    }
    ...
    for (ll i = 0; i < q; i++) {
        ...
        if (len > 1) {
            if (all_same(_l, _r)) {
                ans = 0;
            } else if (alternate(_l, _r)) {
                ll min = 2;
                ll max = len&1 ? len - 1 : len - 2;
                if (min <= len && max >= min) ans = 1LL*(max + min)*((max-min)/2+1)/2;
                if (!manacher.isPalindrome(_l, _r)) ans += len;
            } else {
                ans = 1LL*len*(len+1)/2 - 1 - (manacher.isPalindrome(_l, _r) ? len : 0);
            }
        }
