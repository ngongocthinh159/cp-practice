...
Edge edges[MAXN];
vector<int> g[MAXN];
ll hash_t, pref[MAXS], basepow[MAXS], ibasepow[MAXS];
void pre_compute() {
    basepow[0] = 1;
    for (int i = 1; i < MAXS; i++) basepow[i] = basepow[i - 1] * base % MOD;
    ibasepow[MAXS - 1] = expo(basepow[MAXS - 1], MOD - 2, MOD);
    for (int i = MAXS - 2; i >= 0; i--) ibasepow[i] = ibasepow[i + 1] * base % MOD;
}
void dfs(int u, int par, int idx) {
    for (auto i : g[u]) {
        int v = edges[i].u^edges[i].v^u;
        if (v != par) {
            for (int j = 0; j < edges[i].s.size(); j++) {
                pref[idx + j + 1] = (pref[idx + j] + (edges[i].s[j] - 'a' + 1)*basepow[idx + j]%MOD) % MOD;
                if (idx + j + 1 - len_t >= 0) {
                    int r = idx + j + 1;
                    int l = r - len_t;
                    ll cur = (pref[r] - pref[l]) % MOD;
                    if (cur < 0) cur += MOD;
                    cur = (cur * ibasepow[l]) % MOD;
                    if (cur == hash_t) ans++;
                }
            }
            dfs(v, u, idx + edges[i].s.size());
        }
    }
}
void solve() {
    ...
    for (int i = 0; i < len_t; i++) {
        hash_t = (hash_t + (t[i] - 'a' + 1) * basepow[i] % MOD) % MOD;
    }
    dfs(1, -1, 0);
    cout << ans << nline;
}
