 ...
 * Solution for: https://codeforces.com/contest/1660/problem/D
*/
...
void solve() {
    ...
    for (int i = 0; i < n;) {
        ...
        int r = i - 1;
        int pow = (abs(a[l]) == 2 ? 1 : 0);
        int sign = (a[l] > 0 ? 1 : -1);
        mn[l] = mx[l] = {sign, pow, l};
        if (sign == 1 && max_pow < pow) {
            max_pow = pow;
            ll = l; rr = l;
        }
        for (int j = l + 1; j <= r; j++) {
            pow = (abs(a[j]) == 2 ? 1 : 0);
            if (a[j] > 0) {
                if (mx[j - 1][0] == 1) mx[j] = {1, mx[j - 1][1] + pow, mx[j - 1][2]};
                else mx[j] = {1, pow, j};

                if (mn[j - 1][0] == -1) mn[j] = {-1, mn[j - 1][1] + pow, mn[j - 1][2]};
                else mn[j] = {1, pow, j};
            } else {
                if (mn[j - 1][0] == -1) mx[j] = {1, mn[j - 1][1] + pow, mn[j - 1][2]};
                else mx[j] = {-1, pow, j};

                if (mx[j - 1][0] == 1) mn[j] = {-1, mx[j - 1][1] + pow, mx[j - 1][2]};
                else mn[j] = {-1, pow, j};
            }

            if (mx[j][0] == 1 && max_pow < mx[j][1]) {
                max_pow = mx[j][1];
                ll = mx[j][2], rr = j;
            }
        }
