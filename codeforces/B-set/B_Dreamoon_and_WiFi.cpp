 ...
 * Solution for: https://codeforces.com/problemset/problem/476/B
*/
...
void solve() {
    ...
    if (cnt_plus <0 || cnt_sub <0) {printf("%.12f\n", 0.0); return;}
    vector<int> fact(n+1);
    fact[0] = 1;
    for (int i = 1; i <=n; i++) {
        fact[i] = fact[i-1]*i;
    }
    int total = cnt_plus + cnt_sub;
    printf("%.12f\n", 1.00*fact[total]/fact[cnt_plus]/fact[cnt_sub]/pow(2,total));
}
