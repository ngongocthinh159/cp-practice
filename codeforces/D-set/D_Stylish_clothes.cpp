 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/9/3/practice/contest/307094/problem/D
*/
...
void solve() {
    ...
    for (int j = 0; j < 4; j++) {
        int n, x;
        cin >> n;
        for (int i = 0; i < n; i++) cin >> x, v.push_back({x, j});
    }
    sort(all(v));
    ...
    int diff = 0;
    vector<int> cnt(4, 0);
    int mn = IINF, _l = -1, _r = -1;
    while (r < v.size()) {
        cnt[v[r].second]++;
        if (cnt[v[r].second] == 1) diff++;
        
        while (l < r && diff >= 4) {
            if (mn > v[r].first - v[l].first) {
                mn = v[r].first - v[l].first;
                _l = l, _r = r;
            }
            cnt[v[l].second]--;
            if (cnt[v[l].second] == 0) diff--;
            l++;
        }
    ...
    unordered_set<int> S;
    vector<int> ans(4);
    ans[v[_l].second] = v[_l].first;
    ans[v[_r].second] = v[_r].first;
    S.insert(v[_l].second);
    S.insert(v[_r].second);
    for (int i = _l; i <= _r; i++) {
