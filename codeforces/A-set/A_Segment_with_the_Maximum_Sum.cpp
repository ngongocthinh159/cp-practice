 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/4/2/practice/contest/273278/problem/A
*/
...
private:
    void _build(int idx, int s, int e) {
        if (s == e) {
            if (arr[s] >= 0) tree[idx] = Node(arr[s], arr[s], arr[s], arr[s]);
            else tree[idx] = Node(0, 0, 0, arr[s]);
            return;
        }
        int m = (s + e)/2;
        _build(2*idx, s, m);
    ...
    Node _query(int idx, int s, int e, int l, int r) {
        ...
        // cout << s << " " << e << " " << l_ans.mx_sum << " " << r_ans.mx_sum << nline;
...
struct Node {
    ...
    Node(ll _mx_sum, ll _pre_sum, ll _sur_sum, ll _sum) {
        // might change
        mx_sum = _mx_sum;
        mx_pre = _pre_sum;
        mx_sur = _sur_sum;
    ...
    void merge(Node &l, Node &r) { // merge two child nodes
        mx_sum = max(l.mx_sum, max(r.mx_sum, l.mx_sur + r.mx_pre));
        mx_sum = max(mx_sum, 0LL);
        mx_pre = max(0LL, max(l.mx_pre, l.sum + r.mx_pre));
        mx_sur = max(0LL, max(r.mx_sur, r.sum + l.mx_sur));
        sum = l.sum + r.sum;
    }
};
struct Updater {
    ...
    void apply(Node &node) { // update logic
        if (val <= 0) {
            node.mx_sum = node.mx_pre = node.mx_sur = 0;
        } else {
            node.mx_sum = node.mx_pre = node.mx_sur = val;
        }
        node.sum = val;
    }
};
...
void solve() {
    ...
    SegTree<Node, Updater> tree(n, arr);
    cout << tree.query(0, n - 1).mx_sum << nline;
    for (int i = 0; i < m; i++) {
