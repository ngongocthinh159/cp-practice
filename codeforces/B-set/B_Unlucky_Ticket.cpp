...
void solve() {
    ...
    int greater = -1;
    f(i,0,n) {
        if (nums1[i] == nums2[i]) {greater=-2; break;}
        if (i == 0) {
            if (nums1[i] > nums2[i]) greater = 1;
            else if (nums1[i] < nums2[i]) greater = 2;
        }
        if (greater == 1 && nums1[i] < nums2[i]) {greater=-2; break;}
        if (greater == 2 && nums1[i] > nums2[i]) {greater=-2; break;}
    }
    cout << (greater != -2 ? "YES\n" : "NO\n");
}
