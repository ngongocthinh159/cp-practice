 ...
 * Solution for: https://codeforces.com/problemset/problem/1778/C
*/
...
void solve() {
    ...
    int cnt = 0;
    for (auto c : S) if (order[c - 'a'] == -1) order[c - 'a'] = cnt++;

    ll ans = INT_MIN;
    for (int mask = 0; mask < (1 << sz(S)); mask++) {
        if (!(sz(S) - __builtin_popcount(mask) <= k)) continue;
        string ss = s;
        for (int i = 0; i < n; i++) {
            if (!(mask & (1 << order[ss[i] - 'a']))) ss[i] = t[i];
        }
