 ...
 * Solution for: https://codeforces.com/problemset/problem/1324/E
*/
...
const int mxa = 2e3 + 5;
int n, h, l, r;
int dp[mxn][mxh]; // dp[i][j] max number of good sleepings if: already complete the i-th sleep and the ending time of i-th sleep is j
int a[mxn];
void pre_compute() {
...
void solve() {
    ...
    dp[1][a[1]] = (l <= a[1] && a[1] <= r) ? 1 : 0;
    dp[1][a[1] - 1] = (l <= a[1] - 1 && a[1] - 1 <= r) ? 1 : 0;
    for (int i = 2; i <= n; i++) {
        for (int j = 0; j <= h - 1; j++) {
            good = (l <= j && j <= r ? 1 : 0);
            
            if (dp[i - 1][(j - a[i] + h) % h] != -1) dp[i][j] = good + dp[i - 1][(j - a[i] + h) % h];
            if (dp[i - 1][(j - (a[i] - 1) + h) % h] != -1) dp[i][j] = max(dp[i][j], good + dp[i - 1][(j - (a[i] - 1) + h) % h]);
        }
    }
    int ans = INT_MIN;
    for (int j = 0; j <= h - 1; j++) {
