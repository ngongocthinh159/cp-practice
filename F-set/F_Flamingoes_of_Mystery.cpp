/*
    Solution for: https://codeforces.com/problemset/problem/1425/F
*/
...
/* clang-format on */
struct Interactor {
    vector<int> v = {1, 4, 4, 6, 7, 8};
    Interactor() {}

    int query(int l, int r) {
        if (!(1 <= l && l <= v.size())) {cout << "invalid l: " << l << '\n'; return -1;}
        if (!(1 <= l && r <= v.size())) {cout << "invalid r: " << r << '\n'; return -1;}
        int sum = 0;
...
void solve() {
    ...
    for (int i = 2; i <= n; i++) {
        ...
        cout << "? " << l + 1 << " " << r + 1 << endl;
        cin >> cur;
        // cur = inter.query(l + 1, r + 1);
        if (i > 2) {a[i - 1] = cur - prev;}
        else save = cur;
        prev = cur;
    }
    int l = 1, r = l + (n - 1) - 1;
