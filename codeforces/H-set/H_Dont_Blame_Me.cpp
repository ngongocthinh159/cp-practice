 ...
 * Solution for: https://codeforces.com/contest/1829/problem/H
*/
...
ll dfs(int i, int mask, vector<vector<int>> &dp) {
    if (i == n) {
        return mask != (1 << 6) && __builtin_popcount(mask) == k;
    }
    if (dp[i][mask] != -1) return dp[i][mask];

    int newMask = (mask == (1 << 6) ? mask - 1 : mask) & a[i];
    int setBits = __builtin_popcount(newMask);
    bool canChoose = setBits >= k;
    ll res = 0;
    if (canChoose) {
        res = (res + dfs(i + 1, newMask, dp)) % MOD;
    }
    res = (res + dfs(i + 1, mask, dp)) % MOD;
