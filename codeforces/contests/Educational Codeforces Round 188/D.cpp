 ...
 * Solution for: https://codeforces.com/contest/2204/problem/D
*/
...
void dfs(int u, int p, int d, bool &odd, int cur, int &one, int &zero) {
    depth[u] = d;
    color[u] = 1;
    if (cur == 0) zero++;
    else one++;
    for (auto v : g[u]) if (v != p) {
        if (color[v] == 0) {
            dfs(v, u, d + 1, odd, cur ^ 1, one, zero);
        } else if (color[v] == 1) {
            int edges = depth[u] - depth[v] + 1;
            odd |= (edges & 1);
        } else {
