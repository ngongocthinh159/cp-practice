 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/5/2/practice/contest/279653/problem/A
*/
...
Data merge(Data &l, Data &r) {
    Data res;
    res.sum = l.sum + r.sum;
    res.min_pre = min(l.min_pre, l.sum + r.min_pre);
    return res;
}
...
void solve() {
    ...
    for (int i = 0; i < q; i++) {
        ...
        if (t == 1) {
            ...
            r--;
            len = r - l + 1;
            update(1, 0, n - 1, l, a[l] + val);
            if (l + len < n) update(1, 0, n - 1, l + len, a[l + len] - val);
            a[l] += val;
            if (l + len < n) a[l + len] -= val;
        } else {
            cin >> l >> r;
            r--;
            ll res = query(1, 0, n - 1, l, r).min_pre;
            ll tmp = 0;
            if (l - 1 >= 0) tmp = query(1, 0, n - 1, 0, l - 1).sum;
            cout << (res + tmp) << nline;
        }
    }                               
}
