 ...
 * Solution for: https://codeforces.com/problemset/problem/1833/E
*/
...
int n;
void dfs(int u, vector<set<int>> &g, vector<bool> &visited, vector<int> &outdeg, bool &has) {
    visited[u] = true;
    if (outdeg[u] != 2) has = true;
    for (auto v : g[u]) if (!visited[v]) dfs(v, g, visited, outdeg, has);
}
void solve() {
    cin >> n;
    ...
    for (int i = 1; i <= n; i++) {
        int v; cin >> v;
        if (!g[i].count(v)) g[i].insert(v), outdeg[i]++;
        if (!g[v].count(i)) g[v].insert(i), outdeg[v]++;
    }
    vector<bool> visited(n + 1);
    int compo = 0, non_full = 0;
    for (int i = 1; i <= n; i++) if (!visited[i]) {
        bool has = false;
        dfs(i, g, visited, outdeg, has);
        compo++;
        if (has) non_full++;
    }
    cout << (non_full == 0 ? compo : compo - non_full + 1) << " " << compo << nline;
}
