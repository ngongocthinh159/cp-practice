 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/5/2/practice/contest/279653/problem/D
*/
...
int inter_len(int l1, int r1, int l2, int r2) {
    if (!is_intersect(l1, r1, l2, r2)) return 0;
...
void update(int idx, int s, int e, int l, int r, ll val) {
    ...
    Data new_sum = merge(segtree[2*idx], segtree[2*idx + 1]);
    new_sum.sum += lazy[idx]*(e - s + 1);
    segtree[idx] = new_sum;
}
Data query(int idx, int s, int e, int l, int r) {
    ...
    Data new_sum = merge(l_ans, r_ans);
    new_sum.sum += lazy[idx]*inter_len(s, e, l, r);
    return new_sum;
}
void pre_compute() {
