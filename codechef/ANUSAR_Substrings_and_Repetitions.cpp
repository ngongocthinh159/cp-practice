 ...
 * Solution for: https://www.codechef.com/problems/ANUSAR
*/
...
void solve() {
    ...
    vector<ll> cnt_exact(n + 2), cnt_atleast(n + 2);
    for (int i = 0; i < n - 1; i++) {
        ...
        cnt_exact[R[i] - L[i]] += 1LL*(lcp[i] - counted_len)*(R[i] - L[i]);
    }
    cnt_exact[1] = 1LL*n*(n + 1)/2;
    for (int i = 2; i <= n; i++) cnt_exact[1] -= cnt_exact[i];
    for (int i = n; i >= 1; i--) cnt_atleast[i] = cnt_exact[i] + cnt_atleast[i + 1];
    for (int i = 0; i < q; i++) {
        int f; cin >> f;
        if (f > n)
            cout << 0 << nline;
        else
            cout << cnt_atleast[f] << nline;
    }
}
