 ...
 * Solution for: https://codeforces.com/contest/2070/problem/D
*/
...
void dfs(int u, int p, int d) {
    height = max(height, d);
    if (d == level.size()) level.push_back({});
    level[d].push_back(u);
...
void solve() {
    ...
    for (int cur_h = height; cur_h >= 1; cur_h--) {
        ll level_sum = 0;
        for (auto u : level[cur_h]) {
            if (lowest) dp[u] = 1;
            else {
                dp[u] = (1 + next) % MOD1;
                for (auto v : g[u]) if (v != par[u]) 
                    dp[u] = ((dp[u] - dp[v]) % MOD1 + MOD1) % MOD1;
            }
            level_sum = (level_sum + dp[u]) % MOD1;
        }
        next = level_sum;
        lowest = false;
    }
    ll ans = 0;
    for (auto v : g[1]) ans = (ans + dp[v]) % MOD1;
    ans = (ans + 1) % MOD1;
    cout << ans << nline;
}
