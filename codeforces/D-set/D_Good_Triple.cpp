 ...
 * Solution for: https://codeforces.com/contest/1169/problem/D
*/
...
/* 
https://www.cut-the-knot.org/proofs/2ColorsOnLine.shtml

Lemma: exist at least one 3 points of same color with equal distance in a segment of length at least 9

Proof: Points on a straight line are colored in two colors. Prove that it is always possible to find three points of the same color with one being the midpoint of the other two.
...
void solve() {
    ...
    ll ans = 0;
    while (r < n) {
        for (int k = 1; r - 2*k > last_l; k++) {
            if (s[r] == s[r - k] && s[r] == s[r - 2*k]) {
                last_l = r - 2*k;
            }
        }
        
        if (last_l != -1) ans += (last_l + 1);
