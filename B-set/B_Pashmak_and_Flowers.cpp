...
int main() {
    ...
    ll n, min = INFF, max = -INFF, minCount = 0, maxCount = 0;
    cin >> n;
    f(i,0,n) {
        ll tmp;
        cin >> tmp;
        if (min > tmp) {
            min = tmp;
            minCount = 1;
        } else if (min == tmp) minCount++;
        if (max < tmp) {
            max = tmp;
            maxCount = 1;
        } else if (max == tmp) maxCount++;
    }
    cout << max - min << " " << (min != max ? minCount*maxCount : minCount*(minCount - 1)/2) << "\n";
}
