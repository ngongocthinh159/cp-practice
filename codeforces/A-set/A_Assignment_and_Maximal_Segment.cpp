 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/5/3/practice/contest/280799/problem/A
*/
...
struct Data {
    ll mx_sum, mx_pre, mx_sur, sum;
};
Data segtree[4*MAX];
ll lazy[4*MAX];
ll a[MAX];
Data make_data(ll mx_sum, ll mx_pre, ll mx_sur, ll sum) {
    Data res;
    res.mx_sum = mx_sum;
    res.mx_pre = mx_pre;
    res.mx_sur = mx_sur;
...
Data merge(Data &l, Data &r) {
    Data res;
    res.mx_sum = max(l.mx_sum, max(r.mx_sum, l.mx_sur + r.mx_pre));
    res.mx_pre = max(l.mx_pre, l.sum + r.mx_pre);
    res.mx_sur = max(r.mx_sur, r.sum + l.mx_sur);
    res.sum = l.sum + r.sum;
    return res;
}
void build(int idx, int s, int e) {
    if (s == e) {
        segtree[idx] = make_data(0, 0, 0, 0);
        return;
    }
    int m = (s + e)/2;
    build(2*idx, s, m);
...
void apply(int idx, int s, int e, ll val) {
    segtree[idx].mx_sum = val > 0 ? val*(e - s + 1) : val;
    segtree[idx].mx_pre = val > 0 ? val*(e - s + 1) : val;
    segtree[idx].mx_sur = val > 0 ? val*(e - s + 1) : val;
    segtree[idx].sum = val*(e - s + 1);
...
Data query(int idx, int s, int e, int l, int r) {
    if (s > r || e < l) return make_data(0, 0, 0, 0);
...
void solve() {
    ...
    for (int i = 0; i < q; i++) {
        ...
        ll ans = query(1, 0, n - 1, 0, n - 1).mx_sum;
        cout << (ans < 0 ? 0 : ans) << nline;
        // if (t == 1) {
        //     cin >> l >> r;
        //     r--;
        ...
        //     cin >> val;
        //     val++;
        //     cout << query(1, 0, n - 1, val).val << nline;
        // }
    }
}
