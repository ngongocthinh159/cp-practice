 ...
 * Solution for: https://onlinejudge.org/index.php?option=com_onlinejudge&Itemid=8&page=show_problem&problem=3785
*/
...
#define N 10005
#define LOG 17
int R, C, Q, dp[N], up[N], down[N], color[N], depth[N], pref[N], buf[N], compo[N];
int par[LOG + 1][N], top;
vector<vector<int>> g;
void pre_compute() {
...
bool one_simple_path(int u, int v) {
    return (pref[u] - pref[v]) == 0;
}
void solve() {
    g.assign(R + 1, vector<int>());
    for (int i = 0; i <= R; i++) color[i] = 0, up[i] = 0, down[i] = 0, pref[i] = 0;
    for (int i = 0; i < C; i++) {
        int u, v;
        cin >> u >> v;
        g[u].push_back(v);
    ...
    int id = 0;
    for (int i = 1; i <= R; i++) if (color[i] == 0) dfs(i, -1, id++);
    for (int i = 0; i <= R; i++) color[i] = 0;
    for (int i = 1; i <= R; i++) if (color[i] == 0) {
        pref[i] = dp[i] != 0;
        dfs2(i, -1);
    }
    for (int i = 0; i < Q; i++) {
        ...
        int A = lca(u, v);
        // u -> v has only 1 simple path, if after buidling the dfs tree, the path from u->v on dfs tree contains
        // all the edges that are bridges of the the graph
        if (one_simple_path(u, A) && one_simple_path(v, A)) {
            cout << 'Y' << nline;
        } else
            cout << 'N' << nline;
    }
