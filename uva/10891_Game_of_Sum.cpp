 ...
 * Solution for: https://onlinejudge.org/index.php?option=com_onlinejudge&Itemid=8&page=show_problem&problem=1832
*/
...
#define MAXN 105
ll dp[MAXN][MAXN], n, a[MAXN], pref[MAXN];
void pre_compute() {
...
ll dfs(int l, int r) {
    ...
    ll res = LLONG_MIN;
    for (int i = l; i <= r; i++) {
        res = max(res, pref[r + 1] - pref[l] - dfs(i + 1, r));
    }
    for (int i = r; i >= l; i--) {
        res = max(res, pref[r + 1] - pref[l] - dfs(l, i - 1));
    }
    return dp[l][r] = res;
}
void solve() {
