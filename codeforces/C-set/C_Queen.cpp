 ...
 * Solution for: https://codeforces.com/problemset/problem/1143/C
*/
...
void dfs(int u, int par, vector<int>&deleted) {

    bool shouldDelete = c[u];
    for (auto v : g[u]) {
        if (v != par) {
            shouldDelete &= c[v];
        }
    }
    if (shouldDelete) deleted.push_back(u);
    for (auto  v : g[u]) {
        if (v != par) {
            dfs(v, u, deleted);
        }
