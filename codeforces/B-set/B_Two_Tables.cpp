...
void solve() {
    ...
    int mx = INT_MIN;
    int mxX = -1, mxY = -1;
    cf(shiftX,-(max(na,nb)-1),max(na,nb)-1) {
        cf(shiftY,-(max(ma,mb)-1),max(ma,mb)-1) {
            int sum = 0;
            bool ok = false;
            f(i,0,na) {
                f(j,0,ma) {
                    if (0 <= i + shiftX && i + shiftX <= nb - 1 && 0 <= j + shiftY && j + shiftY <= mb - 1) {
                        if (matrixA[i][j] == matrixB[i + shiftX][j + shiftY]) {
                            sum += matrixA[i][j]*matrixB[i + shiftX][j + shiftY];
                            ok = true;
                        }
                    }
                }
