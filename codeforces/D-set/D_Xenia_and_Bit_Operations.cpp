 ...
 * Solution for: https://codeforces.com/contest/339/problem/D
*/
...
ndata segtree[4*mxn];
ndata combine(ndata l, ndata r, int pow) {
    ndata res;
    if (pow&1) res.val = (l.val|r.val);
    else res.val = (l.val^r.val);
    return res;
}
void build(int idx, int s, int e, int pow) {
    if (s == e) {
        segtree[idx] = make_ndata(a[s]);
        return;
    }
    int m = (s + e)/2;
    build(2*idx, s, m, pow - 1);
    build(2*idx + 1, m + 1, e, pow - 1);
    segtree[idx] = combine(segtree[2*idx], segtree[2*idx + 1], pow);
}
void update(int idx, int s, int e, int pos, int val, int pow) {
    if (s == e) {
        segtree[idx] = make_ndata(val);
        return;
    }
    int m = (s + e)/2;
    if (pos <= m) update(2*idx, s, m, pos, val, pow - 1);
    else update(2*idx + 1, m + 1, e, pos, val, pow - 1);
    segtree[idx] = combine(segtree[2*idx], segtree[2*idx + 1], pow);
}
ndata query(int idx, int s, int e, int l, int r, int pow) {
    if (e < l || s > r) return make_ndata(0);
    if (l <= s && e <= r) return segtree[idx];
    int m = (s + e)/2;
    return combine(query(2*idx, s, m, l, r, pow - 1), query(2*idx + 1, m + 1, e, l, r, pow - 1), pow);
}
void pre_compute() {
...
void solve() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        pos--;
        update(1, 0, N - 1, pos, val, n);
        cout << query(1, 0, N - 1, 0, N - 1, n).val << nline;
    }
}
