 ...
 * Solution for: https://codeforces.com/problemset/problem/276/D
*/
...
void solve() {
    ...
    for (int bit = 60; bit >= 0; bit--) {
        if (((r >> bit) & 1) && !((l >> bit) & 1)) { // first left most bit: set in r and unset in l
            ans = (1LL << (bit + 1)) - 1;
            break;
        }
    }
    cout << (ans == -1 ? 0 : ans) << nline; // ans == -1 mean l == r then l^r = 0
}
