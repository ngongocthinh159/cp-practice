...
int dist(int u, int v) {
    if (u == v) return 0;
    int lcauv = lca(u, v);
    return path_sum[u] + path_sum[v] - 2*path_sum[lcauv];
}
int kth_node(int u, int v, int k) {
    int lcauv = lca(u, v);
    int u_to_lca = depth[u] - depth[lcauv];
    int v_to_lca = depth[v] - depth[lcauv];
    k--;
    if (k <= u_to_lca) return jump(u, k);
    else {
        return jump(v, v_to_lca - (k - u_to_lca));
    }
}
void pre_compute() {
