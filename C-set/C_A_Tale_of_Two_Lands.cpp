/*
    Solution for: https://codeforces.com/problemset/problem/1166/C
*/
...
bool check(int x, int y) {
    return (min(abs(x-y),abs(x+y)) <= abs(x) && abs(x) <= max(abs(x-y),abs(x+y)) && 
            min(abs(x-y),abs(x+y)) <= abs(y) && abs(y) <= max(abs(x-y),abs(x+y)));
}
void solve() {
    cin >> n;
    for (int i = 0; i < n; i++) {
