 ...
 * Solution for: https://codeforces.com/contest/1517/problem/D
*/
...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m - 1; j++) {
            cin >> val;
            g[i][j].push_back({i, j + 1, val});
            g[i][j + 1].push_back({i, j, val});
        }
    }
    for (int i = 0; i < n - 1; i++) {
        for (int j = 0; j < m; j++) {
            cin >> val;
            g[i][j].push_back({i + 1, j, val});
            g[i + 1][j].push_back({i, j, val});
        } 
    }
    if (k&1) {
        for (int i = 0; i < n; i++) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                ...
                for (auto &nei: g[i][j]) {
                    ...
                    vj = nei[1];
                    boredness = nei[2];
                    dp[p][i][j] = min(dp[p][i][j], 2*boredness + dp[p - 2][vi][vj]);
                }
            }
        }
    }
