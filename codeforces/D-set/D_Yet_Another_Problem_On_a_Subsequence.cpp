 ...
 * Solution for: https://codeforces.com/contest/1000/problem/D
*/
...
void pre_compute() {
    ...
    ifact[mxn - 1] = expo(fact[mxn - 1], MOD1 - 2, MOD1);
    for (int i = mxn - 2; i >= 0; i--) {
        ifact[i] = ifact[i + 1] * (i + 1) % MOD1;
    }
}
ll comb(int N, int K) {
...
void solve() {
    ...
    for (int i = n - 1; i >= 0; i--) {
        ...
        for (int j = i + a[i] + 1; j <= n; j++) {
            dp[i] = (dp[i] + comb(j - i - 1, a[i]) * dp[j] % MOD1) % MOD1;
        }
    }
    ll ans = 0;
    for (int i = 0; i < n; i++) {
