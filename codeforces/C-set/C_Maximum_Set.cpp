 ...
 * Solution for: https://codeforces.com/problemset/problem/1796/C
*/
...
void solve() {
    ...
    ll len = cnt + 1;
    ll mxl1 = r / _2pow[len-1];
    ll mxl2 = r / (_2pow[len-2]*3);
    ll ans = 0;
    if (mxl1 >= l) {
        ans = mod_add(ans,mxl1-l+1,mod);
    }
    if (mxl2 >= l) {
        ans = mod_add(ans,mod_mul(mxl2-l+1,len-1,mod),mod);
    }
    cout << len << " " << ans << nline;
}
