...
ll dfs(int i, bool started, int cnt, bool prev_smaller, string &upper) {
    if (i == sz(upper)) {
        return started;
    }
    if (dp[i][started][cnt][prev_smaller] != -1) return dp[i][started][cnt][prev_smaller];

    ll res = 0;
    if (!started) {
        // chose zero
        res += dfs(i + 1, false, cnt, true, upper);

        // choose number
        int hi = prev_smaller ? 9 : upper[i] - '0';
        for (int j = 1; j <= hi; j++) {
            res += dfs(i + 1, true, cnt + 1, prev_smaller | (j < upper[i] - '0'), upper);
        }
    } else {
        if (cnt >= 3) {
            res += dfs(i + 1, true, cnt, prev_smaller | (0 < upper[i] - '0'), upper);
        } else {
            int hi = prev_smaller ? 9 : upper[i] - '0';
            for (int j = 0; j <= hi; j++) {
                res += dfs(i + 1, true, cnt + (j != 0), prev_smaller | (j < upper[i] - '0'), upper);
            }
        }
    }
    return dp[i][started][cnt][prev_smaller] = res;
}
void solve() {
    cin >> l >> r;
    l--;
    ...
    ll tmp1 = dfs(0, 0, 0, 0, sr);
    memset(dp, -1, sizeof dp);
    ll tmp2 = dfs(0, 0, 0, 0, sl);
    cout << tmp1 - tmp2 << nline;
}
