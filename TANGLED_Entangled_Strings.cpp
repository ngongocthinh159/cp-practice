...
void solve() {
    ...
    vector<int> L(n - 1, -1), R(n - 1, lcp.size()), cnt_s1(n + 1);
    vector<int> st;
    FOR(i,0,n-2) {
    ...
    FOR(i,0,n-1) {
        cnt_s1[i + 1] = cnt_s1[i] + (p[i] < SZ(s1));
    }

    int maxL = max(SZ(s1), SZ(s2));
    vector<ll> f(maxL + 2);
    FOR(i,0,n-2) {
        ...
        int l = L[i] + 1;
        int r = R[i];
        int s1_occur = cnt_s1[r + 1] - cnt_s1[l];
        int s2_occur = len - s1_occur;
        ll tuples = 1LL*s1_occur*s2_occur;

        int counted_len = 0;
        if (L[i] >= 0) counted_len = max(counted_len, lcp[L[i]]);
        if (R[i] < lcp.size()) counted_len = max(counted_len, lcp[R[i]]);

        // printf("i: %d, l: %d, r: %d, s1_occur: %d, s2_occur: %d, counted_len: %d, lcp[i]: %d\n", i, l, r, s1_occur, s2_occur, counted_len, lcp[i]);
        // printf("plus to %d, extract to %d   value: %d\n\n", counted_len + 1, lcp[i] + 1, tuples);

        f[counted_len + 1] += tuples;
        f[lcp[i] + 1] -= tuples;
    }
    FOR(i,2,maxL) f[i] += f[i - 1];
    FOR(i,1,K) {
        if (i > maxL)
            cout << 0 << " ";
        else
