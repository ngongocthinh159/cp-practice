 ...
 * Solution for: https://codeforces.com/gym/102694/problem/F
*/
...
int n, q;
int tin[2*MAX], tout[2*MAX], cnt = -1, height[MAX];
struct Data {
    ll seeds;
    ll pods;
};
Data segtree[8*MAX];
Data make_data(ll seeds, ll pods) {
    Data res;
    res.seeds = seeds;
    res.pods = pods;
    return res;
...
Data merge(Data &l, Data &r) {
    Data res;
    res.seeds = l.seeds + r.seeds;
    res.pods = l.pods + r.pods;
    return res;
}
void build(int idx, int s, int e) {
    if (s == e) {
        segtree[idx] = make_data(0, 0);
        return;
    }
    int m = (s + e)/2;
    build(2*idx, s, m);
...
void update(int idx, int s, int e, int pos, int delta, int type) {
    if (s == e) {
        if (type == 1) segtree[idx].seeds += delta;
        else segtree[idx].pods += delta;
        return;
    };
    int m = (s + e)/2;
    if (pos <= m) update(2*idx, s, m, pos, delta, type);
    else update(2*idx + 1, m + 1, e, pos, delta, type);
    segtree[idx] = merge(segtree[2*idx], segtree[2*idx + 1]);
}
Data query(int idx, int s, int e, int l, int r) {
    if (s > r || e < l) return make_data(0, 0);
...
void dfs(int u, int par, vector<vector<int>> &g) {
    tin[u] = ++cnt;
    for (auto v : g[u]) {
        if (v != par) height[v] = height[u] + 1, dfs(v, u, g);
    }
    tout[u] = ++cnt;
}
void solve() {
    ...
    for (int i = 0; i < q; i++) {
        ...
        if (x == 0) {
            ...
            Data res = query(1, 0, cnt, tin[u], tout[u]);
            cout << abs(res.seeds - res.pods) << nline;
        } else {
            update(1, 0, cnt, tin[u], x, 1);
            update(1, 0, cnt, tin[v], x, 2);
        }
    }
}
