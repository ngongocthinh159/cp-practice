...
void pre_compute_base_pow() {
    ...
    base_pow = vector<vector<ll>>(prime_count, vector<ll>(globalMxN + 5));
    i_base_pow = vector<vector<ll>>(prime_count, vector<ll>(globalMxN + 5));
...
void solve() {
    ...
    for (int r = k - 1; r < s.size(); r++) {
        S.insert(hash.hash_in_range(r - k + 1, r));
    }
    cout << S.size() << nline;
}
