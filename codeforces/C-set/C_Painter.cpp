 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/5/4/practice/contest/280801/problem/C
*/
...
enum LazyType { COLOR, NONE };
struct Lazy {
    LazyType type = NONE;
    int val = 0; // 0 white, 1 black
};
struct Data {
    int sum = 0;
    int cnt = 0;
...
Data make_data(int sum, int cnt, bool pre, bool sur) {
    Data res;
    res.sum = sum;
    res.cnt = cnt;
    res.pre = pre;
...
void merge(Data *root, Data *l, Data *r) {
    ...
    if (l->sur && r->pre) root->cnt--;
    root->pre = l->pre;
    root->sur = r->sur;
}
void apply(Data *root, int s, int e, const Lazy &operation) {
    if (operation.type == COLOR) {
        if (operation.val == 0) { // white
            root->sum = 0;
            root->cnt = 0;
            root->pre = false;
            root->sur = false;
        ...
        root->lazy.type = COLOR;
    }
};
void push_down(Data *root, int s, int e) {
    if (root->left == NULL) root->left = new Data();
    if (root->right == NULL) root->right = new Data();
    int m = (s + e)/2;
    apply(root->left, s, m, root->lazy); // apply left child
    apply(root->right, m + 1, e, root->lazy); // apply right child
    root->lazy = Lazy(); // reset lazy
}
void update(Data *root, int s, int e, int l, int r, const Lazy &operation) { // range update + val
    if (s > r || e < l) return;
    if (l <= s && e <= r) {
        apply(root, s, e, operation);
        return;
    }
    push_down(root, s, e);
    int m = (s + e)/2;
    update(root->left, s, m, l, r, operation);
    update(root->right, m + 1, e, l, r, operation);
...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        cin >> c >> l >> len;
        l += 500000; // what the fuck?
        int r = l + len - 1;
        if (c == 'W') {
            update(ROOT, 0, MAX - 1, l, r, Lazy({COLOR, 0}));
        } else {
            update(ROOT, 0, MAX - 1, l, r, Lazy({COLOR, 1}));
        }
        cout << ROOT->cnt << " " << ROOT->sum << nline;
    }
}
