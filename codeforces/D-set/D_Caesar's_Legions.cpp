 ...
 * Solution for: https://codeforces.com/contest/118/problem/D
*/
...
void solve() {
    ...
    for (int i = 0; i <= n1; i++) {
        for (int j = 0; j <= n2; j++) {
            for (int k = 0; k <= 1; k++) {
                for (int t = 0; t <= (k == F ? k1 : k2); t++) {
                    if (i < n1 && (k == H || (k == F && t < k1))) {
                        (dp[i + 1][j][F][k == F ? t + 1 : 1] += dp[i][j][k][t]) %= mod;
                    }
                    if (j < n2 && (k == F || (k == H && t < k2))) {
                        (dp[i][j + 1][H][k == H ? t + 1 : 1] += dp[i][j][k][t]) %= mod;
                    }
                }
            }
        }
    ...
    int ans = 0;
    for (int t = 1; t <= k1; t++) {
        ans = (ans + dp[n1][n2][F][t]) % mod;
    }
    for (int t = 1; t <= k2; t++) {
        ans = (ans + dp[n1][n2][H][t]) % mod;
    }
    cout << ans << nline;
}
