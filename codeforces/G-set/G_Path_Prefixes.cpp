 ...
 * Solution for: https://codeforces.com/problemset/problem/1714/G
*/
...
void dfs(int u, int par, ll sumA, vector<ll> &pathB, vector<int> &ans, vector<vector<vector<int>>> &g) {
    auto it = upper_bound(all(pathB), sumA);
    if (it != pathB.begin()) {
        it--;
        int idx = it - pathB.begin();
        ans[u] = idx + 1;
    }
    for (auto &vec : g[u]) {
        int v = vec[0];
        if (v != par) {
            int a = vec[1];
            int b = vec[2];
            pathB.push_back(pathB.size() ? pathB.back() + b : b);
            dfs(v, u, sumA + a, pathB, ans, g);
            pathB.pop_back();
        }
    }
}
void solve() {
    ...
    for (int i = 2; i <= n; i++) {
        int par, a, b;
        cin >> par >> a >> b;
        g[par].push_back({i, a, b});
    }
    dfs(1, -1, 0, pathB, ans, g);
    for (int i = 2; i <=n; i++) {
        cout << ans[i] << " ";
