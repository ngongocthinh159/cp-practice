 ...
 * Solution for: https://codeforces.com/contest/1632/problem/D
*/
...
#define LOG 20
#define MASK(i) (1 << (i))
#define RELAX(x, y) gcd(x, y)


int a[MAX], rmq_gcd[LOG + 1][MAX];
...
int n;
void build_rmq() {
    for (int i = 0; i < n; i++) rmq_gcd[0][i] = a[i];
    for (int j = 1; j <= LOG; j++)
        for (int i = 0; i < n; i++) {
            if (i + MASK(j) - 1 < n) {
                rmq_gcd[j][i] = RELAX(rmq_gcd[j - 1][i], rmq_gcd[j - 1][i + MASK(j - 1)]);
            } else break;
        }
}
int query_gcd(int l, int r) {
    int msb = 32 - __builtin_clz(r - l + 1) - 1;
    return RELAX(rmq_gcd[msb][l], rmq_gcd[msb][r - MASK(msb) + 1]);
}
void solve() {
    cin >> n;
    ...
    int cnt = 0;
    while (r < n) {
        while (l < r && query_gcd(l, r) < (r - l + 1)) {
            l++;
        }
        if (query_gcd(l, r) == (r - l + 1)) {
            if (mx_point < l) {
                cnt++;
                mx_point = r;
            }
