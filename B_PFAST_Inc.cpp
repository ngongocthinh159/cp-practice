...
/* clang-format on */
void solve();
void generateSubSet(vector<string>& v, vector<string>& cur, vector<vector<string>>& res, int i);
...
void solve() {
    ...
    f(i,0,m) {
        cin >> name1 >> name2;
        unordered_set<string>& list1 = g[name1];
        unordered_set<string>& list2 = g[name2];
        list1.insert(name2);
        list2.insert(name1);
    }
    generateSubSet(v, cur, subsets, 0);

    int maxSize = -1;
    int maxIndex = -1;
    f(k,0,subsets.size()) {
        vector<string>& subset = subsets[k];

        bool shouldTake = true;
        for (int i = 0; i < (int) subset.size() - 1; i++) {
            for (int j = i + 1; j < subset.size(); j++) {
                unordered_set<string>& list = g[subset[i]];
                if (list.find(subset[j]) != list.end()) {shouldTake = false; break;}
            }
            if (!shouldTake) break;
        }
        if (shouldTake && maxSize < (int)subset.size()) {
            maxSize = (int) subset.size();
            maxIndex = k;
        }
    }
    cout << maxSize << "\n";
    vector<string>& ans = subsets[maxIndex];
