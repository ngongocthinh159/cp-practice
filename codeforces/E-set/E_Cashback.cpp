 ...
 * Solution for: https://codeforces.com/contest/940/problem/E
*/
...
void solve() {
    ...
    deque<int> dq;
    for (int r = 0; r < c; r++) {
        while (dq.size() && a[dq.back()] >= a[r]) dq.pop_back();
        dq.push_back(r);
    }
    mn[c - 1] = a[dq.front()];
    for (int r = c; r < n; r++) {
        while (dq.size() && a[dq.back()] >= a[r]) dq.pop_back();
        dq.push_back(r);
        while (dq.size() && !((r - c + 1) <= dq.front() && dq.front() <= r)) dq.pop_front();
        mn[r] = a[dq.front()];
    }
    prf[0] = 0;
    for (int i = 1; i <= n; i++) {
    ...
    for (int i = 1; i <= n; i++) {
        dp[i] = dp[i - 1] + a[i - 1];
        if (i - c >= 0) dp[i] = min(dp[i], dp[i - c] + prf[i] - prf[i - c] - mn[i - 1]);
    }
    cout << dp[n] << nline;
}
