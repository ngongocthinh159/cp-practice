 ...
 * Solution for: https://codeforces.com/contest/63/problem/E
*/
...
void build_substate() {
    ...
    v.push_back({0,1,2});
    v.push_back({3,4,5,6});
    v.push_back({7,8,9,10,11});
    v.push_back({12,13,14,15});
    v.push_back({16,17,18});

    v.push_back({0,3,7});
    v.push_back({1,4,8,12});
    v.push_back({2,5,9,13,16});
    v.push_back({6,10,14,17});
    v.push_back({11,15,18});

    v.push_back({2,6,11});
    v.push_back({1,5,10,15});
    v.push_back({0,4,9,14,18});
    v.push_back({3,8,13,17});
    v.push_back({7,12,16});

    for (vector<int> &row : v) {
        for (int len = 1; len <= SZ(row); len++) {
            int state = 0;
            for (int r = 0; r < len; r++) {
                state |= (1 << row[r]);
            }
            sub_states[top++] = state;
            for (int r = len; r < SZ(row); r++) {
                state |= (1 << row[r]);
                state &= ~(1 << row[r - len]);
                sub_states[top++] = state;
            }
...
void solve() {
    ...
    build_substate();
    for (int state = 1; state < (1 << MAXN); state++) {
        for (int j = 0; j < top; j++) {
            if ((sub_states[j] & state) == sub_states[j]) {
                if (!dp[state ^ sub_states[j]]) {
                    dp[state] = 1;
                    break;
                }
            }
    ...
    cout << (dp[cur] ? "Karlsson" : "Lillebror") << nline;
}
