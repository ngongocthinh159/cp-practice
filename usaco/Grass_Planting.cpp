 ...
 * Solution for: https://usaco.org/index.php?page=viewproblem2&cpid=102&lang=en
...
struct HLD {
    int n, T;
    vector<int> big, chain, par, depth, pos;
    vector<ll> euler;
    SegmentTree segtree;
...
public:
    ...
    int query(int u, int v) {
        ...
        if (depth[u] > depth[v]) swap(u, v);
        res += segtree.query(pos[u] + 1, pos[v]).sum;
    ...
    void update(int u, int v, int val) {
        ...
        if (depth[u] > depth[v]) swap(u, v);
        segtree.update(pos[u] + 1, pos[v], val, Node::PLUS);
...
void solve() {
    ...
    HLD<SegmentTree<Node, ll>> hld(g, val, 1);
    char t;
    int u, v;
    for (int i = 0; i < m; i++) {
        cin >> t >> u >> v;
