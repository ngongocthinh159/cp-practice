 ...
 * Solution for: https://acm.hdu.edu.cn/showproblem.php?pid=6397
*/
...
#define MAXN 100005
int n, m, k;
ll fact[2*MAXN], ifact[2*MAXN];


// reference: https://cp-algorithms.com/combinatorics/inclusion-exclusion.html#number-of-upper-bound-integer-sums

// m boxes (m - 1 bars), k stars

// Total = (m - 1) C (m + k - 1)   (xi >= 0)

// De gioi han xi <= n - 1
// Thi ta phai lay total tru di nhung truong hop xi >= n
// => Union cua x1 >=n, x2 >= n
...
void pre_compute() {
    fact[0] = 1;
    for (int i = 1; i < 2*MAXN; i++) fact[i] = fact[i - 1] * i % MOD1;
    ifact[2*MAXN - 1] = expo(fact[2*MAXN - 1], MOD1 - 2, MOD1);
    for (int i = 2*MAXN - 2; i >= 0; i--) ifact[i] = ifact[i + 1] * (i + 1) % MOD1;
}
ll nCk(int n, int k) {
    if (k > n) return 0;
...
void solve() {
    cin >> n >> m >> k;
    ll ans = nCk(m + k - 1, m - 1);
    for (int len = 1; len <= m; len++) {
        int sign = (len & 1) ? -1 : 1;
        ll cur = nCk(m, len) * nCk(m + k - 1 - len*n, m - 1) % MOD1;
        ans = (ans + (cur * sign % MOD1 + MOD1) % MOD1) % MOD1;
    }
    cout << ans << nline;
}
