 ...
 * Solution for: https://www.spoj.com/problems/CLFLARR/
*/
...
void solve() {
    ...
    vector<int> ans(n);
    for (int i = 0; i < q; i++) {
        for (int j = find(queries[i].l); j <= queries[i].r; j = find(j)) {
            ans[j] = queries[i].c;
            if (next[j] == j) next[j] = j + 1;
        }
    }
    for (int i = 0; i < n; i++) cout << ans[i] << nline;
};
