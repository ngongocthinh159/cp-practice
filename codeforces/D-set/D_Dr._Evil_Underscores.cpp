 ...
 * Solution for: https://codeforces.com/problemset/problem/1285/D
*/
...
int find_ans(node *root, int B) {
    node* p = root;
    int mn = 0;
    for (int bit = B; bit >= 0; bit--) {
        if (p->child[0] && p->child[1]) {
            auto l = find_ans(p->child[0], bit - 1);
            auto r = find_ans(p->child[1], bit - 1);
            mn ^= (1 << bit);
            mn += min(l, r);
            break;
        }
...
void solve() {
    ...
    node* root = new node;
    int a[n];
    for (int i =0; i <n; i++) cin >> a[i], insert(root, a[i]);
    cout << find_ans(root, BIT - 1) << nline;
}
