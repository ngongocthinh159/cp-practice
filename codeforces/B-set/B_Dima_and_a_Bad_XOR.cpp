 ...
 * Solution for: https://codeforces.com/contest/1151/problem/B
*/
...
#define MAXN 505
#define MSB 10
int n, m, a[MAXN][MAXN], trace[MAXN][1 << MSB + 1];
bool dp[MAXN][1 << MSB + 1];
...
void solve() {
    ...
    dp[0][0] = true;
    for (int i = 0; i < n; i++)
        for (int j = 0; j < (1 << (MSB + 1)); j++) if (dp[i][j]) {
            for (int k = 1; k <= m; k++) {
                dp[i + 1][j ^ a[i + 1][k]] = true;
                trace[i + 1][j ^ a[i + 1][k]] = (k << 11) ^ j;
            }
        }
    bool ans = false;
    int bestI, bestJ;
    for (int j = 1; j < (1 << (MSB + 1)); j++) if (dp[n][j]) {
        ans = true;
        bestI = n;
        bestJ = j;
        break;
    ...
    if (!ans)
        ...
        vector<int> v;
        while (bestI != 0) {
            int nextJ = trace[bestI][bestJ] & ((1 << 11) - 1);
            int k = trace[bestI][bestJ] >> 11;
            v.push_back(k);
            bestI--;
            bestJ = nextJ;
        }
