 ...
 * Solution for: https://vjudge.net/problem/UVA-11475/origin
*/
...
void solve() {
    ...
    int len = n - 1;
    for (int i = 0; i < n; i++) {
        if (i + mana.get_p_at(i) == n) {
            len = min(len, n - mana.get_longest(i));
        }
        if (i >= 1 && i + mana.get_p_at(i, false) == n) {
            len = min(len, n - mana.get_longest(i, false));
        }
    }
