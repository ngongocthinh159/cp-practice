 ...
 * Solution for: https://codeforces.com/contest/2075/problem/C
*/
...
void solve() {
    cin >> n >> m;
    pbds s;
    ll ans = 0, cnt_equal_n = 0;
    for (int i = 0; i < m; i++) {
        cin >> a[i];
        if (a[i] == n) cnt_equal_n++;
    }
    sort(a, a + m);
    for (int i = 1; i <= n - 1; i++) {
        int remain = n - i;
        auto it1 = lower_bound(a, a + m, i);
        auto it2 = lower_bound(a, a + m, remain);
        if (it1 != a + m && it2 != a + m) {
            int cnt1 = m - (it1 - a);
            int cnt2 = m - (it2 - a);
            ans += 1LL*cnt1*cnt2 - min(cnt1, cnt2);
        }
    }
    cout << ans << nline;
}
