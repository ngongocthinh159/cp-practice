 ...
 * Solution for: https://codeforces.com/contest/471/problem/D
*/
...
#define MAX 200005
int n, m, bear[MAX], ele[MAX], pref[MAX];
void pre_compute() {
...
void solve() {
    ...
    int j = 1;
    for (int i = 2; i < m; i++) {
        while (j > 1 && ele[i] != ele[j]) j = pref[j - 1] + 1;
        pref[i] = ele[i] == ele[j] ? (++j - 1) : 0;
    }
    j = 1;
    int ans = 0;
    for (int i = 1; i < n; i++) {
        while (j > 1 && bear[i] != ele[j]) j = pref[j - 1] + 1;
        if (bear[i] == ele[j]) ++j;
        if (j - 1 == m - 1) {
            ans++;
        }
