 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/7/2/practice/contest/289391/problem/J
*/
...
struct DSU {
    ...
    vector<int> parity;
    vector<bool> bipartile;
    DSU(int n) {
        parent.resize(n + 1);
        rank.resize(n + 1);
        parity.resize(n + 1);
        bipartile.resize(n + 1);
        iota(ALL(parent), 0);
        fill(ALL(rank), 0);
        fill(ALL(parity), 0);
        fill(ALL(bipartile), true);
    }
    int find(int x) {
    ...
    void unite(int a, int b) {
        ...
        if (ra != rb) {
            ...
            bipartile[rb] = bipartile[rb] & bipartile[ra];
            parent[rb] = ra;
        } else {
    ...
    bool is_bipartile(int x) {
        return bipartile[find(x)];
    }
};
ll n, m;
void pre_compute() {
