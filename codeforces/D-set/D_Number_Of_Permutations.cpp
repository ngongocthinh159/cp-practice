 ...
 * Solution for: https://codeforces.com/contest/1207/problem/D
*/
...
ll cnt(int mode = 0) {
    ...
    while (i < n) {
        ...
        int y = s[i].se;
        if (mode == 0) 
            while (i < n && s[i].fi == x) i++;
        else 
            while (i < n && s[i].fi == x && s[i].se == y) i++;
        int len = i - j;
        bad = (bad * fact[len]) % MOD1;
    }
    return bad;
}
void solve() {
    ...
    ll res = fact[n];
    sort(s, s + n);
    res = ((res - cnt()) % MOD1 + MOD1) % MOD1;
    bool inter = true;
    for (int i = 1; i < n; i++) if (s[i].se < s[i - 1].se) {
        inter = false;
        break;
    }
    if (inter) res = ((res + cnt(1)) % MOD1 + MOD1) % MOD1;
