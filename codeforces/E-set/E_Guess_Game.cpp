 ...
 * Solution for: https://codeforces.com/problemset/problem/1864/E
*/
...
struct Trie {
    ...
    void insert(int &x) {
        ...
        for (int i = 31; i >= 0; i--) {
            ...
            if (p->child[idx] == NULL) {
                ...
                p->child[idx]->one_bit_cnt = p->one_bit_cnt;
                if (idx == 1) p->child[idx]->one_bit_cnt++;
            }
            p = p->child[idx];
            p->passed++;
        }
    ...
    ll get_ans() {
        ...
        ll rep = n;
        rep = (rep + (n*(n - 1)) % m) % m;
        res = (res*expo(rep, m - 2, m)) % m;
        return res;
    }

    void dfs(Node* root) {
        bool hasLeft = (root->child[0] != NULL);
        bool hasRight = (root->child[1] != NULL);
        if (hasLeft && hasRight) {
            Node* left = root->child[0];
            Node* right = root->child[1];
            ll len = left->one_bit_cnt + 1;
            ll rep = (1LL * left->passed * right->passed) % m;
            ll val1 = len + !(len&1);
            ll val2 = len + (len&1);
            res = (res + (rep*val1)%m) % m;
            res = (res + (rep*val2)%m) % m;
            dfs(root->child[0]);
            dfs(root->child[1]);
        } else if (hasLeft) dfs(root->child[0]);
        else if (hasRight) dfs(root->child[1]);
        else {
            ll passed = root->passed;
            ll rep = (passed*(passed - 1))%m;
            rep = (rep + passed) % m;
            res = (res + (rep*(root->one_bit_cnt + 1)) % m) % m;
        }
    }
};
