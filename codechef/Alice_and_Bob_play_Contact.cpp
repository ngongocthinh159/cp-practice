 ...
 * Solution for: https://www.codechef.com/practice/course/icpc/ICPCTR14/problems/CHN16I
*/
...
struct Node {
    vector<Node*> child;
    int cnt = 0; // cnt word ending
    int cnt_sub = 0; // total cnt word ending in node subtree
    Node() {
...
void dfs(Node* root) {
    ...
    for (int i = 0; i < 26; i++) {
        dfs(root->child[i]);
        if (root->child[i]) res += root->child[i]->cnt_sub;
    }
    res += root->cnt;
    root->cnt_sub = res;
}
