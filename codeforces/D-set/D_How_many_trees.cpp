 ...
 * Solution for: https://codeforces.com/problemset/problem/9/D
*/
...
#define MAXN 40
ll c[MAXN][MAXN], n, h;
// idea: the same idea with divide and conquer of catalan number on binary tree
...
void solve() {
    ...
    for (int i = 1; i <= n; i++)
        for (int j = 1; j <= n; j++)
            for (int k = 1; k <= i; k++) {
                for (int m = 0; m <= j - 1; m++)
                    c[i][j] += c[k - 1][j - 1] * c[i - k][m];
                for (int m = 0; m <= j - 2; m++)
                    c[i][j] += c[k - 1][m] * c[i - k][j - 1];
            }
    ll ans = 0;
    for (int i = h; i <= n; i++) ans += c[n][i];
    cout << ans << nline;
}
