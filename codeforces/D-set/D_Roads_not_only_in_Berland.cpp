 ...
 * Solution for: https://codeforces.com/contest/25/problem/D
*/
...
void solve() {
    ...
    for (int i = 1; i <= n; i++) re.insert(dsu.find(i));
     // for (auto x : v) cout << x << " " ;
    // cout << nline;
    vector<int> rep(ALL(re));
    int j = 0;
    cout << v.size() << nline; 
    for (int i = 0; i < SZ(v); i++) {
        cout << edges[v[i]].u << " " << edges[v[i]].v << " " << rep[i] << " " << rep[j + 1] << nline;
        j++; 
    }
}
