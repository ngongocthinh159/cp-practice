 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/2/5/practice/contest/269656/problem/F
 * Reference: Problem 10 in https://web.stanford.edu/class/cs97si/suffix-array.pdf
*/
...
void solve() {
    ...
    reverse(ALL(s));
    SuffixArray sar(s, true);
    int k = 1;
    for (int len = 1; len <= n; len++)
        for (int i = 0; i + len - 1 < n; i += len) {
            int mxr = 0, mxl = 0;
            if (i + len < n) mxr = sa.query_lcp_substr(i, i + len);
            int eidx = i + len - 1;
            int reidx = n - eidx - 1;
            if (reidx + len < n) mxl = sar.query_lcp_substr(reidx, reidx + len);
            int tot = mxr + mxl + len;
            k = max(k, tot/len);
        }
    cout << k << nline;
