 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/7/1/practice/contest/289390/problem/E
*/
...
void dfs(int u, vector<int> &ans, int m) {
    ans[u] = m;
    for (auto v : g3[u]) {
        if (ans[v] == -1) {
            dfs(v, ans, m);
...
void solve() {
    ...
    for (int i = 0; i < m; i++) {
        int idx, hand;
        cin >> idx >> hand;
        v.push_back({i,{idx, hand}});
        if (hand == 1) g[idx].first = -1;
        else g[idx].second = -1;
    }
    DSU dsu(n);
    ...
    for (int i = m - 1; i >= 0; i--) {
        int sec, idx, hand;
        sec = v[i].first;
        idx = v[i].second.first;
        hand = v[i].second.second;
        int a = idx;
        int b = -1;
        if (hand == 1) {
            b = g2[idx].first;
