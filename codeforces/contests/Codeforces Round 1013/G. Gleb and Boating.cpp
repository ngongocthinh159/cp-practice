 ...
 * Solution for: https://codeforces.com/contest/2091/problem/G
*/
...
void solve() {
    ...
    if (s >= k * k) {
        cout << max(1, k - 2) << nline;
        return;
    }
    vector<bool> dp(s + 1);
    dp[0] = 1;
    int dir = 1;
    for (int power = k; ; power = max(1, power - 1)) {
        vector<bool> ndp(s + 1);
        if (dir == 1) {
            for (int i = 0; i <= s - power; i++)
                ndp[i + power] = dp[i] || ndp[i];
        } else {
            for (int i = s; i >= power; i--)
                ndp[i - power] = dp[i] || ndp[i];
        }
        swap(dp, ndp);
        if (dp[s]) {
