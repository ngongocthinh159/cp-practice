 ...
 * Solution for: https://codeforces.com/contest/208/problem/E
*/
...
vector<vector<int>> g;
int n, m, x, v1, p1;
int par_dir[mxN], height[mxN], sta[mxN], fin[mxN], node_cnt[mxN];
int cnt;
int mx_height;
void dfs(int u) {
    node_cnt[u] = 1;
    ...
    for (auto v : g[u]) {
        if (v != par_dir[u]) {
            height[v] = height[u] + 1;
            mx_height = max(mx_height, height[v]);
...
void solve() {
    ...
    vector<vector<int>> M(mx_height + 1);
    for (int i = 1; i <= n; i++) {
        M[height[i]].push_back(sta[i]);
    }
    for (auto &vec : M) {
        sort(all(vec));
    }
    ...
    for (int i = 0; i < m; i++) {
        cin >> v1 >> p1;
        int parent = jump(v1, p1);
        if (parent == 0) cout << 0 << nline;
        else {
            auto &vec = M[height[v1]];
            auto it1 = lower_bound(all(vec), sta[parent] + 1);
            auto it2 = upper_bound(all(vec), fin[parent]);
            int l = it1 - vec.begin();
            int r = it2 - vec.begin() - 1;
            cout << (r - l) << nline;
        }
    }
}
