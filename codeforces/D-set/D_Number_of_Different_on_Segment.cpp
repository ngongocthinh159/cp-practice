 ...
 * Solution for: https://codeforces.com/edu/course/2/lesson/4/4/practice/contest/274684/problem/D
*/
...
Data make_data(int _freq[MAX]) {
    Data res;
    for (int i = 0; i < MAX; i++) res.freq[i] = _freq[i];
    return res;
}
Data make_default() {
    Data res;
    for (int i = 0; i < MAX; i++) res.freq[i] = 0;
    return res;
}
Data segtree[4*MAXN];
void build(int idx, int s, int e, vector<int> &arr) {
    if (s == e) {
        Data res = make_default();
        res.freq[arr[s]] = 1;
        segtree[idx] = res;
        return;
    }
    ...
    for (int i = 0; i < MAX; i++) segtree[idx].freq[i] = segtree[2*idx].freq[i] + segtree[2*idx + 1].freq[i];
}
void update(int idx, int s, int e, int pos, int val, int old_val) {
    if (s == e) {
        segtree[idx].freq[val]++;
        segtree[idx].freq[old_val]--;
        return;
    }
    int m = (s + e)/2;
    if (pos <= m) update(2*idx, s, m, pos, val, old_val);
    else update(2*idx + 1, m + 1, e, pos, val, old_val);
    for (int i = 0; i < MAX; i++) segtree[idx].freq[i] = segtree[2*idx].freq[i] + segtree[2*idx + 1].freq[i];
}
Data query(int idx, int s, int e, int l, int r) {
    ...
    Data ans = make_default();
    for (int i = 0; i < MAX; i++) ans.freq[i] = l_ans.freq[i] + r_ans.freq[i];
    return ans;
}
void pre_compute() {
...
void solve() {
    ...
    for (int i = 0; i < q; i++) {
        ...
        if (t == 1) {
            ...
            int cnt = 0;
            for (int i = 0; i < MAX; i++) if (ans.freq[i]) cnt++;
            cout << cnt << nline;
        } else {
            cin >> pos >> val;
            pos--;
            update(1, 0, n - 1, pos, val, arr[pos]);
            arr[pos] = val;
        }
    }
}
