 ...
 * Solution for: https://codeforces.com/contest/1923/problem/C
*/
...
ll query(int l, int r, ll *arr) {
    return arr[r + 1] - arr[l];
}
void solve() {
    cin >> n >> q;;
    for (int i = 0; i < n; i++) cin >> a[i], pref[i + 1] = pref[i] + a[i], cnt[i + 1] = cnt[i] + (a[i] == 1);
    for (int i = 0; i < q; i++) {
        int l, r; cin >> l >> r;
        l--, r--;
        ll can = query(l, r, pref) - (r - l + 1);
        if (l != r && can >= query(l, r, cnt)) 
            cout << "YES" << nline;
        else 
            cout << "NO" << nline;
    }
