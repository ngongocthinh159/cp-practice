 ...
 * Solution for: https://codeforces.com/problemset/problem/1391/D
*/
...
bool isAlternativePattern(int x, int M) {
    for (int bit = M - 2; bit >= 0; bit--) {
        if (!(((x >> bit) & 1) ^ ((x >> (bit + 1) & 1)))) return false;
    }
    return true;
}
void solve() {
    ...
    for (int i = 1; i <= n; i++) {
        for (int prevMask = 0; prevMask <= maxMask; prevMask++) {
            for (int mask = 0; mask <= maxMask; mask++) {
                if (i != 1) {
                    if (!isAlternativePattern(mask ^ prevMask, m)) continue;
                }

                int dif = __builtin_popcount(mask ^ rows[i - 1]);
                dp[i][mask] = min(dp[i][mask], dp[i - 1][prevMask] + dif);
                if (i == n) {
